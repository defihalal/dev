...
contract("Echidna debugger", async accounts => {
  ...
  before(async () => {
    ...
    usdhToken = await USDHToken.at(await echidnaTester.usdhToken());
  ...
  it.only("USDH balance", async () => {
    await echidnaTester.openTroveExt("0", "0", "4210965169908805439447313562489173090");

    const totalSupply = await usdhToken.totalSupply();
    const gasPoolBalance = await usdhToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getUSDHDebt();
    const defaultPoolBalance = await defaultPool.getUSDHDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalUSDHDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = usdhToken.balanceOf(currentTrove);
