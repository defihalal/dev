...
def get_usdh_amount_from_net_debt(contracts, net_debt):
...
def logGlobalState(contracts):
    ...
    activePoolDebt = contracts.activePool.getUSDHDebt()
    defaultPoolColl = contracts.defaultPool.getETH()
    defaultPoolDebt = contracts.defaultPool.getUSDHDebt()
    ...
    SP_USDH = contracts.stabilityPool.getTotalUSDHDeposits().to("ether")
    SP_ETH = contracts.stabilityPool.getETH().to("ether")
    print('SP USDH         ', SP_USDH)
    print('SP ETH          ', SP_ETH)
    ...
    return [ETH_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_USDH, SP_ETH]
