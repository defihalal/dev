...
USDH_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for USDH. The ether price $P_t^e$ follows 
...
"""# HALAL price
In the first month, the price of HALAL follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_HALAL) represents HALAL price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_HALAL, so that the expected HALAL price increases from price_HALAL_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_HALAL_initial$ \cdot (1+$ drift_HALAL$)^{720}$

The HALAL price from the second month on is endogenously determined.
"""

#HALAL price & airdrop
price_HALAL_initial = 0.4
price_HALAL = [price_HALAL_initial]
sd_HALAL=0.005
drift_HALAL = 0.0035
supply_HALAL=[0]
HALAL_total_supply=100000000

"""**HALAL Endogenous Price**

The staked HALAL pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{HALAL Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ HALAL_total_supply

Therefore, the HALAL price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{HALAL\_total\_supply}E_t$$

Interpretation: The denominator implies that with more HALAL tokens issued, HALAL price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop HALAL gain.
...
> (collateral in ether, debt in USDH, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of USDH.

$N_t^o$ is rounded to an integer.

---

The amount of USDH tokens generated by a new trove is
...
#sensitivity to USDH price & issuance fee
alpha = 0.3
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of HALAL token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of USDH deposited in the Stability Pool (see below)
...
"""HALAL Price - First Month"""

#HALAL price
for i in range(1, month):
    random.seed(2+13*i)
    shock_HALAL = random.normalvariate(0,sd_HALAL)  
    price_HALAL.append(price_HALAL[i-1]*(1+shock_HALAL)*(1+drift_HALAL))
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalUSDHDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_HALAL_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, price_HALAL_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalUSDHDeposits() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalUSDHDeposits() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_USDH
    airdrop_gain = price_HALAL_current * quantity_HALAL_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_USDH, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_USDH, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalUSDHDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_USDH * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_USDH)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_usdh_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def transfer_from_to(contracts, from_account, to_account, amount):
    balance = contracts.usdhToken.balanceOf(from_account)
    transfer_amount = min(balance, amount)
    if transfer_amount == 0:
        return amount
    if from_account == to_account:
        return amount
    contracts.usdhToken.transfer(to_account, transfer_amount, { 'from': from_account })
    pending = amount - transfer_amount

    return pending

def get_usdh_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    usdhBalance = contracts.usdhToken.balanceOf(account)
    if debt > usdhBalance:
        pending = debt - usdhBalance
        ...
        # try with active accounts, which are more likely to hold USDH
        ...
        if pending > 0:
            print(f"\n ***Error: not enough USDH to repay! {debt / 1e18} USDH for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_usdh_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayUSDH(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawUSDH(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_USDH_adjust = issuance_USDH_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_USDH_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + USDH_GAS_COMPENSATION
    usdh = get_usdh_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, usdh, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, index):
    random.seed(2019*index)
    shock_opentroves = random.normalvariate(0,sd_opentroves)
    n_troves = len(active_accounts)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0
    issuance_USDH_open = 0

    if index <= 0:
        number_opentroves = initial_open
    elif price_USDH <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_USDH - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_USDH_open = issuance_USDH_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_USDH_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    supply = contracts.usdhToken.totalSupply() / 1e18
    stability_pool_previous = contracts.stabilityPool.getTotalUSDHDeposits() / 1e18
    ...
    if stability_pool > stability_pool_previous:
        ...
        while remaining > 0 and i < len(active_accounts):
          ...
          balance = contracts.usdhToken.balanceOf(account) / 1e18
        ...
        current_deposit = contracts.stabilityPool.getCompoundedUSDHDeposit(accounts[0])
...
"""USDH Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of USDH tokens defined above, the price of USDH at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of USDH.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_USDH, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_USDH_current= price_USDH * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_USDH_current

""" **Stabilizers**

There are two stabilizers to attenuate USDH price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the USDH price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of USDH in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of USDH price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    usdh_balance = contracts.usdhToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedUSDHamount] = contracts.hintHelpers.getRedemptionHints(usdh_balance, price_ether_current, 70)
    if truncatedUSDHamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"USDH Token:    {contracts.usdhToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"USDH bal: {usdh_balance / 1e18}")
        print(f"truncated: {truncatedUSDHamount / 1e18}")
        ...
        print(f"amount: {truncatedUSDHamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, index):

    stability_pool = contracts.stabilityPool.getTotalUSDHDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_USDH_stabilizer = 0
    ...
    #Calculating Price
    price_USDH_current = calculate_price(price_USDH, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_USDH_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_USDH)**delta
        ...
        issuance_USDH_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_USDH_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_USDH_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_USDH)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_USDH_current = 1 - rate_redemption
        else:
    ...
    return [price_USDH_current, redemption_pool, redemption_fee, issuance_USDH_stabilizer]

"""# HALAL Market"""

def HALAL_market(index, data):
    #quantity_HALAL = (HALAL_total_supply/3)*(1-0.5**(index/period))
    np.random.seed(2+3*index)
    if index <= month:
        price_HALAL_current = price_HALAL[index-1]
        ...
        price_HALAL_current = discount * PE_ratio * annualized_earning / HALAL_total_supply

    #MC_HALAL_current = price_HALAL_current * quantity_HALAL

    return [price_HALAL_current, annualized_earning]
