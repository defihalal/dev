...
def setAddresses(contracts):
    ...
    contracts.troveManager.setAddresses(
        ...
        contracts.usdhToken.address,
        contracts.sortedTroves.address,
        contracts.halalToken.address,
        contracts.halalStaking.address,
        { 'from': accounts[0] }
    )
    ...
    # HALAL
    contracts.halalStaking.setAddresses(
        contracts.halalToken.address,
        contracts.usdhToken.address,
        contracts.troveManager.address,
        contracts.borrowerOperations.address,
...
def contracts():
    ...
    contracts.usdhToken = USDHToken.deploy(
        contracts.troveManager.address,
        contracts.stabilityPool.address,
        contracts.borrowerOperations.address,
        { 'from': accounts[0] }
    ...
    # HALAL
    contracts.halalStaking = HALALStaking.deploy({ 'from': accounts[0] })
    ...
    contracts.halalToken = HALALToken.deploy(
        contracts.communityIssuance.address,
        contracts.halalStaking.address,
...
def print_expectations():
    ...
    print("Expected HALAL price at the end of first month: $", price_HALAL_initial * (1 + drift_HALAL)**720)
...
* trove liquidation
* return of the previous period's stability pool determined (liquidation gain & airdropped HALAL gain)
* trove closure
* trove adjustment
* open troves
* issuance fee
* trove pool formed
* USDH supply determined
* USDH stability pool demand determined
* USDH liquidity pool demand determined
* USDH price determined
* redemption & redemption fee
* HALAL pool return determined
"""
def test_run_simulation(add_accounts, contracts, print_expectations):
    USDH_GAS_COMPENSATION = contracts.troveManager.USDH_GAS_COMPENSATION() / 1e18
    ...
    with open('tests/simulation.csv', 'w', newline='') as csvfile:
        ...
        datawriter.writerow(['iteration', 'ETH_price', 'price_USDH', 'price_HALAL', 'num_troves', 'total_coll', 'total_debt', 'TCR', 'recovery_mode', 'last_ICR', 'SP_USDH', 'SP_ETH', 'total_coll_added', 'total_coll_liquidated', 'total_usdh_redempted'])
        ...
        for index in range(1, n_sim):
            ...
            result_liquidation = liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, price_HALAL_current, data, index)
            total_coll_liquidated = total_coll_liquidated + result_liquidation[0]
            return_stability = result_liquidation[1]

            #close troves
            result_close = close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, index)

            #adjust troves
            [coll_added_adjust, issuance_USDH_adjust] = adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index)

            #open troves
            [coll_added_open, issuance_USDH_open] = open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, index)
            ...
            [price_USDH, redemption_pool, redemption_fee, issuance_USDH_stabilizer] = price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDH, index)
            total_usdh_redempted = total_usdh_redempted + redemption_pool
            print('USDH price', price_USDH)
            print('HALAL price', price_HALAL_current)

            issuance_fee = price_USDH * (issuance_USDH_adjust + issuance_USDH_open + issuance_USDH_stabilizer)
            data['issuance_fee'][index] = issuance_fee
            data['redemption_fee'][index] = redemption_fee

            #HALAL Market
            result_HALAL = HALAL_market(index, data)
            price_HALAL_current = result_HALAL[0]
            #annualized_earning = result_HALAL[1]
            #MC_HALAL_current = result_HALAL[2]

            [ETH_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_USDH, SP_ETH] = logGlobalState(contracts)
            print('Total redempted ', total_usdh_redempted)
            ...
            datawriter.writerow([index, ETH_price, price_USDH, price_HALAL_current, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_USDH, SP_ETH, total_coll_added, total_coll_liquidated, total_usdh_redempted])
