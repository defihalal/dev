...
* 
*
*  --- Functionality added specific to the HALALToken ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Liquity contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending HALAL directly to a Liquity
* core contract, when they should rather call the right function.
*
* 2) sendToHALALStaking(): callable only by Liquity core contracts, which move HALAL tokens from user -> HALALStaking contract.
*
...
* -sendToHALALStaking() reverts when the multisig is the sender, blocking the multisig from staking its HALAL.
* 
* After one year has passed since deployment of the HALALToken, the restrictions on multisig operations are lifted
* and the multisig has the same rights as any other address.
*/

contract HALALToken is CheckContract, IHALALToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "HALAL";
    string constant internal _SYMBOL = "HALAL";
    ...
    address public immutable halalStakingAddress;
        ...
        public 
    {
        ...
        // Allocate the remainder to the HALAL Multisig: (100 - 2 - 32 - 1.33) million = 64.66 million
    ...
    function sendToHALALStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsHALALStaking();
        if (_isFirstYear()) { _requireSenderIsNotMultisig(_sender); }  // Prevent the multisig from staking HALAL
        _transfer(_sender, halalStakingAddress, _amount);
    }
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'HALAL: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'HALAL: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "HALAL: Cannot transfer tokens directly to the HALAL token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != halalStakingAddress,
            "HALAL: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "HALALToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "HALALToken: caller must not be the multisig");
    }

    function _requireCallerIsHALALStaking() internal view {
         require(msg.sender == halalStakingAddress, "HALALToken: caller must be the HALALStaking contract");
    }
