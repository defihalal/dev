...
* Within the first year from deployment, the deployer of the HALALToken (Liquity AG's address) may transfer HALAL only to valid 
* LockupContracts, and no other addresses (this is enforced in HALALToken.sol's transfer() function).
* 
* The above two restrictions ensure that until one year after system deployment, HALAL tokens originating from Liquity AG cannot 
...
contract LockupContract {
    ...
    event LockupContractEmptied(uint _HALALwithdrawal);
    ...
    function withdrawHALAL() external {
        _requireCallerIsBeneficiary();
        _requireLockupDurationHasPassed();

        IHALALToken halalTokenCached = halalToken;
        uint HALALBalance = halalTokenCached.balanceOf(address(this));
        halalTokenCached.transfer(beneficiary, HALALBalance);
        emit LockupContractEmptied(HALALBalance);
    }
