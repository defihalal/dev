...
contract EchidnaTester {
    ...
    uint private MCR;
    uint private CCR;
    uint private USDH_GAS_COMPENSATION;
    ...
    constructor() public payable {
        ...
        troveManager.setAddresses(address(borrowerOperations), 
            ...
            address(priceFeedTestnet), address(usdhToken), 
        ...
        stabilityPool.setAddresses(address(borrowerOperations), 
            address(troveManager), address(activePool), address(usdhToken), 
        ...
        for (uint i = 0; i < NUMBER_OF_ACTORS; i++) {
            echidnaProxies[i] = new EchidnaProxy(troveManager, borrowerOperations, stabilityPool, usdhToken);
        ...
        USDH_GAS_COMPENSATION = borrowerOperations.USDH_GAS_COMPENSATION();
        require(MCR > 0);
        require(CCR > 0);
    ...
    function redeemCollateralExt(
        ...
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].redeemCollateralPrx(_USDHAmount, _firstRedemptionHint, _upperPartialRedemptionHint, _lowerPartialRedemptionHint, _partialRedemptionHintNICR, 0, 0);
    }
    ...
    function getAdjustedETH(uint actorBalance, uint _ETH, uint ratio) internal view returns (uint) {
        ...
        require(price > 0);
        uint minETH = ratio.mul(USDH_GAS_COMPENSATION).div(price);
        require(actorBalance > minETH);
        uint ETH = minETH + _ETH % (actorBalance - minETH);
        return ETH;
    }

    function getAdjustedUSDH(uint ETH, uint _USDHAmount, uint ratio) internal view returns (uint) {
        uint price = priceFeedTestnet.getPrice();
        uint USDHAmount = _USDHAmount;
        uint compositeDebt = USDHAmount.add(USDH_GAS_COMPENSATION);
        uint ICR = LiquityMath._computeCR(ETH, compositeDebt, price);
        if (ICR < ratio) {
            compositeDebt = ETH.mul(price).div(ratio);
            USDHAmount = compositeDebt.sub(USDH_GAS_COMPENSATION);
        }
        return USDHAmount;
    }

    function openTroveExt(uint _i, uint _ETH, uint _USDHAmount) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        ...
        uint USDHAmount = getAdjustedUSDH(ETH, _USDHAmount, CCR);

        //console.log('ETH', ETH);
        //console.log('USDHAmount', USDHAmount);

        echidnaProxy.openTrovePrx(ETH, USDHAmount, address(0), address(0), 0);
    ...
    function openTroveRawExt(uint _i, uint _ETH, uint _USDHAmount, address _upperHint, address _lowerHint, uint _maxFee) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].openTrovePrx(_ETH, _USDHAmount, _upperHint, _lowerHint, _maxFee);
    }
    ...
    function withdrawUSDHExt(uint _i, uint _amount, address _upperHint, address _lowerHint, uint _maxFee) external {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].withdrawUSDHPrx(_amount, _upperHint, _lowerHint, _maxFee);
    }

    function repayUSDHExt(uint _i, uint _amount, address _upperHint, address _lowerHint) external {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].repayUSDHPrx(_amount, _upperHint, _lowerHint);
    }
    ...
    function adjustTroveExt(uint _i, uint _ETH, uint _collWithdrawal, uint _debtChange, bool _isDebtIncrease) external payable {
        ...
        if (_isDebtIncrease) {
            ...
            debtChange = getAdjustedUSDH(ETH, uint(_debtChange), MCR);
        }
    ...
    function echidna_trove_properties() public view returns(bool) {
        ...
        while (currentTrove != address(0)) {
            ...
            // Minimum debt (gas compensation)
            if (troveManager.getTroveDebt(currentTrove) < USDH_GAS_COMPENSATION) {
                return false;
            }
            // Uncomment to check that the condition is meaningful
            //else return false;
    ...
    function echidna_ETH_balances() public view returns(bool) {
        ...
        if (address(usdhToken).balance > 0) {
            return false;
        }
    ...
    // Total USDH matches
    function echidna_USDH_global_balances() public view returns(bool) {
        uint totalSupply = usdhToken.totalSupply();
        uint gasPoolBalance = usdhToken.balanceOf(address(gasPool));

        uint activePoolBalance = activePool.getUSDHDebt();
        uint defaultPoolBalance = defaultPool.getUSDHDebt();
        if (totalSupply != activePoolBalance + defaultPoolBalance) {
            return false;
        }

        uint stabilityPoolBalance = stabilityPool.getTotalUSDHDeposits();
        address currentTrove = sortedTroves.getFirst();
        uint trovesBalance;
        while (currentTrove != address(0)) {
            trovesBalance += usdhToken.balanceOf(address(currentTrove));
            currentTrove = sortedTroves.getNext(currentTrove);
        }
