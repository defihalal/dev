...
$ hardhat test
  ...
  Contract: Access Control: Liquity functions with the caller restricted to Liquity contract(s)
    ...
    ActivePool
      ...
      ✓ increaseUSDHDebt(): reverts when called by an account that is not BO nor TroveM
      ✓ decreaseUSDHDebt(): reverts when called by an account that is not BO nor TroveM nor SP
    ...
    DefaultPool
      ...
      ✓ increaseUSDHDebt(): reverts when called by an account that is not TroveManager
      ✓ decreaseUSDH(): reverts when called by an account that is not TroveManager
    ...
    LockupContract
      ✓ withdrawHALAL(): reverts when caller is not beneficiary (68ms)
    HALALStaking
      ✓ increaseF_USDH(): reverts when caller is not TroveManager
    HALALToken
      ✓ sendToHALALStaking(): reverts when caller is not the HALALSstaking (49ms)
    CommunityIssuance
      ✓ sendHALAL(): reverts when caller is not the StabilityPool
      ✓ issueHALAL(): reverts when caller is not the StabilityPool
  ...
  Contract: BorrowerOperations
    Without proxy
      ...
      ✓ addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_USDHDebt snapshots (736ms)
      ...
      ✓ withdrawColl(): applies pending rewards and updates user's L_ETH, L_USDHDebt snapshots (1205ms)
      ✓ withdrawUSDH(): reverts when withdrawal would leave trove with ICR < MCR (516ms)
      ✓ withdrawUSDH(): decays a non-zero base rate (1192ms)
      ✓ withdrawUSDH(): reverts if max fee > 100% (668ms)
      ✓ withdrawUSDH(): reverts if max fee < 0.5% in Normal mode (639ms)
      ✓ withdrawUSDH(): reverts if fee exceeds max fee percentage (938ms)
      ✓ withdrawUSDH(): succeeds when fee is less than max fee percentage (1272ms)
      ✓ withdrawUSDH(): doesn't change base rate if it is already zero (1044ms)
      ✓ withdrawUSDH(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation (899ms)
      ✓ withdrawUSDH(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity (852ms)
      ✓ withdrawUSDH(): borrowing at non-zero base rate sends USDH fee to HALAL staking contract (1004ms)
      ✓ withdrawUSDH(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (923ms)
      ✓ withdrawUSDH(): Borrowing at non-zero base rate increases the HALAL staking contract USDH fees-per-unit-staked (986ms)
      ✓ withdrawUSDH(): Borrowing at non-zero base rate sends requested amount to the user (1562ms)
      ✓ withdrawUSDH(): Borrowing at zero base rate changes USDH fees-per-unit-staked (839ms)
      ✓ withdrawUSDH(): Borrowing at zero base rate sends debt request to user (790ms)
      ✓ withdrawUSDH(): reverts when calling address does not have active trove (370ms)
      ✓ withdrawUSDH(): reverts when requested withdrawal amount is zero USDH (459ms)
      ✓ withdrawUSDH(): reverts when system is in Recovery Mode (649ms)
      ✓ withdrawUSDH(): reverts when withdrawal would bring the trove's ICR < MCR (347ms)
      ✓ withdrawUSDH(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR (439ms)
      ✓ withdrawUSDH(): reverts if system is in Recovery Mode (312ms)
      ✓ withdrawUSDH(): increases the Trove's USDH debt by the correct amount (208ms)
      ✓ withdrawUSDH(): increases USDH debt in ActivePool by correct amount (230ms)
      ✓ withdrawUSDH(): increases user USDHToken balance by correct amount (243ms)
      ✓ repayUSDH(): reverts when repayment would leave trove with ICR < MCR (392ms)
      ✓ repayUSDH(): Succeeds when it would leave trove with net debt >= minimum net debt (435ms)
      ✓ repayUSDH(): reverts when it would leave trove with net debt < minimum net debt (256ms)
      ✓ repayUSDH(): reverts when calling address does not have active trove (431ms)
      ✓ repayUSDH(): reverts when attempted repayment is > the debt of the trove (418ms)
      ✓ repayUSDH(): reduces the Trove's USDH debt by the correct amount (403ms)
      ✓ repayUSDH(): decreases USDH debt in ActivePool by correct amount (406ms)
      ✓ repayUSDH(): decreases user USDHToken balance by correct amount (404ms)
      ✓ repayUSDH(): can repay debt in Recovery Mode (557ms)
      ✓ repayUSDH(): Reverts if borrower has insufficient USDH balance to cover his debt repayment (1122ms)
      ...
      ✓ adjustTrove(): borrowing at non-zero base rate sends USDH fee to HALAL staking contract (889ms)
      ✓ adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (1013ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate increases the HALAL staking contract USDH fees-per-unit-staked (837ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user (895ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes USDH balance of HALAL staking contract (783ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes HALAL staking contract USDH fees-per-unit-staked (881ms)
      ✓ adjustTrove(): Borrowing at zero base rate sends total requested USDH to the user (783ms)
      ...
      ✓ adjustTrove(): reverts when USDH repaid is > debt of the trove (451ms)
      ...
      ✓ adjustTrove(): changes USDHToken balance by the requested decrease (529ms)
      ✓ adjustTrove(): changes USDHToken balance by the requested increase (577ms)
      ...
      ✓ adjustTrove(): Changes the USDH debt in ActivePool by requested decrease (541ms)
      ✓ adjustTrove(): Changes the USDH debt in ActivePool by requested increase (397ms)
      ...
      ✓ adjustTrove(): Reverts if borrower has insufficient USDH balance to cover his debt repayment (496ms)
      ...
      ✓ closeTrove(): subtracts the debt of the closed Trove from the Borrower's USDHToken balance (405ms)
      ✓ closeTrove(): applies pending rewards (1267ms)
      ✓ closeTrove(): reverts if borrower has insufficient USDH balance to repay his entire debt (347ms)
      ...
      ✓ openTrove(): borrowing at non-zero base rate sends USDH fee to HALAL staking contract (923ms)
      ✓ openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct (865ms)
      ✓ openTrove(): Borrowing at non-zero base rate increases the HALAL staking contract USDH fees-per-unit-staked (794ms)
      ✓ openTrove(): Borrowing at non-zero base rate sends requested amount to the user (832ms)
      ✓ openTrove(): Borrowing at zero base rate changes the HALAL staking contract USDH fees-per-unit-staked (602ms)
      ...
      ✓ openTrove(): records up-to-date initial snapshots of L_ETH and L_USDHDebt (639ms)
      ✓ openTrove(): allows a user to open a Trove, then close it, then re-open it (687ms)
      ✓ openTrove(): increases the Trove's USDH debt by the correct amount (129ms)
      ✓ openTrove(): increases USDH debt in ActivePool by the debt of the trove (162ms)
      ✓ openTrove(): increases user USDHToken balance by correct amount (85ms)
  ...
  Contract: Deployment script - Sets correct contract addresses dependencies after deployment
    ...
    ✓ Sets the correct USDHToken address in TroveManager
    ...
    ✓ Sets the correct HALALStaking address in TroveManager
    ...
    ✓ Sets the correct USDHToken address in StabilityPool
    ...
    ✓ Sets the correct HALALStaking address in BorrowerOperations
    ✓ Sets the correct HALALToken address in HALALStaking
    ✓ Sets the correct ActivePool address in HALALStaking
    ✓ Sets the correct ActivePool address in HALALStaking
    ✓ Sets the correct ActivePool address in HALALStaking
    ✓ Sets the correct BorrowerOperations address in HALALStaking
    ✓ Sets the correct CommunityIssuance address in HALALToken
    ✓ Sets the correct HALALStaking address in HALALToken
    ✓ Sets the correct LockupContractFactory address in HALALToken
    ✓ Sets the correct HALALToken address in LockupContractFactory
    ✓ Sets the correct HALALToken address in CommunityIssuance
...
TCR: 11.892415157517211309
  ...
  Contract: HALAL Token
    ...
    ✓ sendToHALALStaking(): changes balances of HALALStaking and calling account by the correct amounts (48ms)
  ...
  Contract: Deploying and funding One Year Lockup Contracts
    Deploying LCs
      ✓ HALAL Deployer can deploy LCs through the Factory (121ms)
      ✓ Anyone can deploy LCs through the Factory (71ms)
      ✓ HALAL Deployer can deploy LCs directly (43ms)
    ...
    Funding LCs
      ✓ HALAL transfer from HALAL deployer to their deployed LC increases the HALAL balance of the LC (397ms)
      ✓ HALAL Multisig can transfer HALAL to LCs deployed through the factory by anyone (168ms)
    Withdrawal attempts on funded, inactive LCs immediately after funding
      ...
      ✓ HALAL multisig can't withraw from a LC which it funded (271ms)
      ✓ No one can withraw from a LC (78ms)

  Contract: Deploying the HALAL contracts: LCF, CI, HALALStaking, and HALALToken 
    CommunityIssuance deployment
      ✓ Stores the deployer's address
    HALALStaking deployment
      ✓ Stores the deployer's address
    HALALToken deployment
      ✓ Stores the multisig's address
      ✓ Stores the CommunityIssuance address
      ✓ Stores the LockupContractFactory address
      ✓ Mints the correct HALAL amount to the multisig's address: (64.66 million)
      ✓ Mints the correct HALAL amount to the CommunityIssuance contract address: 32 million
      ✓ Mints the correct HALAL amount to the bountyAddress EOA: 2 million
      ✓ Mints the correct HALAL amount to the lpRewardsAddress EOA: 1.33 million
    Community Issuance deployment
      ✓ Stores the deployer's address
      ✓ Has a supply cap of 32 million
      ✓ Liquity AG can set addresses if CI's HALAL balance is equal or greater than 32 million  (393ms)
      ✓ Liquity AG can't set addresses if CI's HALAL balance is < 32 million  (367ms)
    Connecting HALALToken to LCF, CI and HALALStaking
      ✓ sets the correct HALALToken address in HALALStaking (1866ms)
      ✓ sets the correct HALALToken address in LockupContractFactory
      ✓ sets the correct HALALToken address in CommunityIssuance (203ms)

  Contract: During the initial lockup period
    HALAL transfer during first year after HALAL deployment
      ✓ Liquity multisig can not transfer HALAL to a LC that was deployed directly (105ms)
      ✓ Liquity multisig can not transfer to an EOA or Liquity system contracts (302ms)
      ✓ Liquity multisig can not approve any EOA or Liquity system contract to spend their HALAL (546ms)
      ...
      ✓ Liquity multisig can not stake their HALAL in the staking contract
      ✓ Anyone (other than Liquity multisig) can transfer HALAL to LCs deployed by anyone through the Factory (359ms)
      ✓ Anyone (other than Liquity multisig) can transfer HALAL to LCs deployed by anyone directly (141ms)
      ✓ Anyone (other than liquity multisig) can transfer to an EOA (106ms)
      ✓ Anyone (other than liquity multisig) can approve any EOA or to spend their HALAL
      ...
      ✓ Anyone (other than liquity AG) can stake their HALAL in the staking contract
    Lockup Contract Factory negative tests
      ✓ deployLockupContract(): reverts when HALAL token address is not set (197ms)
    Transferring HALAL to LCs
      ✓ Liquity multisig can transfer HALAL (vesting) to lockup contracts they deployed (198ms)
      ✓ Liquity multisig can transfer HALAL to lockup contracts deployed by anyone (1798ms)
    Deploying new LCs
      ✓ HALAL Deployer can deploy LCs through the Factory (46ms)
      ✓ Liquity multisig can deploy LCs through the Factory
      ✓ Anyone can deploy LCs through the Factory (80ms)
      ✓ HALAL Deployer can deploy LCs directly (44ms)
  ...
  Contract: After the initial lockup period has passed
    Deploying new LCs
      ✓ HALAL Deployer can deploy new LCs
    ...
    Beneficiary withdrawal from initial LC
      ...
      ✓ Beneficiaries can withraw full HALAL balance of LC if it has increased since lockup period ended (234ms)
    Withdrawal attempts from LCs by non-beneficiaries
      ✓ HALAL Multisig can't withdraw from a LC they deployed through the Factory
      ✓ HALAL Multisig can't withdraw from a LC that someone else deployed
      ✓ Non-beneficiaries cannot withdraw from a LC (189ms)
    Transferring HALAL
      ✓ HALAL multisig can transfer HALAL to LCs they deployed (176ms)
      ✓ HALAL multisig can transfer tokens to LCs deployed by anyone (104ms)
      ✓ HALAL multisig can transfer HALAL directly to any externally owned account (141ms)
      ✓ Anyone can transfer HALAL to LCs deployed by anyone (179ms)
      ✓ Anyone can transfer to an EOA (421ms)
      ✓ Anyone can approve any EOA to spend their HALAL
      ...
      ✓ Anyone can stake their HALAL in the staking contract (55ms)
    Withdrawal Attempts on new LCs before unlockTime has passed
      ✓ HALAL Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, before the unlockTime (44ms)
      ✓ HALAL Deployer can't withdraw from a funded LC that someone else deployed, before the unlockTime (448ms)
    ...
    Withdrawals from new LCs after unlockTime has passed
      ✓ HALAL Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, after the unlockTime (100ms)
      ✓ HALAL multisig can't withdraw from a funded LC when they are not the beneficiary, after the unlockTime (529ms)
  ...
  Contract: HALAL community issuance arithmetic tests
...
issuance fraction after: 949066037374286
    ...
    ✓ Total HALAL tokens issued is 42.20 after a minute (38ms)
    ✓ Total HALAL tokens issued is 2,531.94 after an hour (41ms)
    ✓ Total HALAL tokens issued is 60,711.40 after a day (136ms)
    ✓ Total HALAL tokens issued is 422,568.60 after a week (331ms)
    ✓ Total HALAL tokens issued is 1,772,113.21 after a month
    ✓ Total HALAL tokens issued is 5,027,363.22 after 3 months (40ms)
    ✓ Total HALAL tokens issued is 9,264,902.04 after 6 months (47ms)
    ✓ Total HALAL tokens issued is 16,000,000 after a year (47ms)
    ✓ Total HALAL tokens issued is 24,000,000 after 2 years (47ms)
    ✓ Total HALAL tokens issued is 28,000,000 after 3 years (75ms)
    ✓ Total HALAL tokens issued is 30,000,000 after 4 years (40ms)
    ✓ Total HALAL tokens issued is 31,968,750 after 10 years (69ms)
    ✓ Total HALAL tokens issued is 31,999,969.48 after 20 years (68ms)
    ✓ Total HALAL tokens issued is 31,999,999.97 after 30 years (87ms)
  ...
  Contract: HALALStaking revenue share tests
    ✓ stake(): reverts if amount is zero (74ms)
    ✓ ETH fee per HALAL staked increases when a redemption fee is triggered and totalStakes > 0 (1562ms)
    ✓ ETH fee per HALAL staked doesn't change when a redemption fee is triggered and totalStakes == 0 (3540ms)
    ✓ USDH fee per HALAL staked increases when a redemption fee is triggered and totalStakes > 0 (1757ms)
    ✓ USDH fee per HALAL staked doesn't change when a redemption fee is triggered and totalStakes == 0 (1705ms)
    ✓ HALAL Staking: A single staker earns all ETH and HALAL fees that occur (2143ms)
    ✓ stake(): Top-up sends out all accumulated ETH and USDH gains to the staker (2138ms)
    ✓ getPendingETHGain(): Returns the staker's correct pending ETH gain (1856ms)
    ✓ getPendingUSDHGain(): Returns the staker's correct pending USDH gain (1965ms)
    ✓ HALAL Staking: Multiple stakers earn the correct share of all ETH and HALAL fees, based on their stake size (3133ms)
  ...
  Contract: All Liquity functions with onlyOwner modifier
    ...
    LockupContractFactory
      ✓ setHALALAddress(): reverts when called by non-owner, with wrong address, or twice (216ms)

  Contract: StabilityPool
    ✓ getETH(): gets the recorded ETH balance
    ✓ getTotalUSDHDeposits(): gets the recorded USDH balance

  Contract: ActivePool
    ✓ getETH(): gets the recorded ETH balance
    ✓ getUSDHDebt(): gets the recorded USDH balance
    ✓ increaseUSDH(): increases the recorded USDH balance by the correct amount
    ✓ decreaseUSDH(): decreases the recorded USDH balance by the correct amount
    ✓ sendETH(): decreases the recorded ETH balance by the correct amount

  Contract: DefaultPool
    ✓ getETH(): gets the recorded USDH balance
    ✓ getUSDHDebt(): gets the recorded USDH balance
    ✓ increaseUSDH(): increases the recorded USDH balance by the correct amount
    ✓ decreaseUSDH(): decreases the recorded USDH balance by the correct amount (57ms)
  ...
  Contract: BorrowerWrappers
    ...
    ✓ claimStakingGainsAndRecycle(): with only USDH gain (821ms)
    ✓ claimStakingGainsAndRecycle(): with both ETH and USDH gains (1593ms)
  ...
  Contract: StabilityPool - HALAL Rewards
    HALAL Rewards
totalHALALIssued_1: 30370113195977152000000
totalHALALIssued_2: 30370113195977152000000
      ✓ liquidation < 1 minute after a deposit does not change totalHALALIssued (743ms)
      ✓ withdrawFromSP(): reward term G does not update when no HALAL is issued (634ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct HALAL gain. No liquidations. No front end. (1479ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct HALAL gain. No liquidations. No front end. (1345ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct HALAL gain. No liquidations. No front end. (1925ms)
      ✓ withdrawFromSP(): Depositor withdraws correct HALAL gain after serial pool-emptying liquidations. No front-ends. (5752ms)
      ✓ HALAL issuance for a given period is not obtainable if the SP was empty during the period (551ms)
      ✓ withdrawFromSP(): Several deposits of 100 USDH span one scale factor change. Depositors withdraw correct HALAL gains (6075ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct HALAL gain. No liquidations. Front ends and kickback rates. (1660ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct HALAL gain. Front ends and kickback rates (2813ms)
      ✓ withdrawFromSP(): Several deposits of 10k USDH span one scale factor change. Depositors withdraw correct HALAL gains (4788ms)

  Contract: Pool Manager: Sum-Product rounding errors
    - Rounding errors: 100 deposits of 100USDH into SP, then 200 liquidations of 49USDH
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ...
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing USDH (965ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing USDH (1152ms)
      ...
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain (1512ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain (1637ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct USDH deposit and ETH Gain (1645ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain (1704ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct USDH deposit and ETH Gain (1753ms)
      ✓ withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 USDH. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct USDH deposit and ETH Gain (1537ms)
...
alice deposit: 0
      ...
      ✓ withdrawFromSP(): Several deposits of 10000 USDH span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (1676ms)
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ...
      ✓ withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing USDH (1597ms)
      ✓ withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing USDH (1711ms)
      ...
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain (2168ms)
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain (5085ms)
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct USDH deposit and ETH Gain (2304ms)
      ✓ withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain (2335ms)
      ✓ withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct USDH deposit and ETH Gain (2145ms)
      ✓ withdrawETHGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 USDH. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct USDH deposit and ETH Gain (2543ms)
...
alice deposit: 0
      ...
      ✓ withdrawETHGainToTrove(): Several deposits of 10000 USDH span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (2361ms)
  ...
  Contract: StabilityPool
    Stability Pool Mechanisms
      ✓ provideToSP(): increases the Stability Pool USDH balance (354ms)
      ✓ provideToSP(): updates the user's deposit record in StabilityPool (355ms)
      ✓ provideToSP(): reduces the user's USDH balance by the correct amount (321ms)
      ✓ provideToSP(): increases totalUSDHDeposits by correct amount (283ms)
      ...
      ✓ provideToSP(): reverts if user tries to provide more than their USDH balance (1030ms)
      ✓ provideToSP(): reverts if user tries to provide 2^256-1 USDH, which exceeds their balance (508ms)
      ...
      ✓ provideToSP(): providing 0 USDH reverts (1091ms)
      ✓ provideToSP(), new deposit: when SP > 0, triggers HALAL reward event - increases the sum G (1072ms)
      ...
      ✓ provideToSP(), new deposit: depositor does not receive any HALAL rewards (671ms)
      ✓ provideToSP(), new deposit after past full withdrawal: depositor does not receive any HALAL rewards (1679ms)
      ✓ provideToSP(), new eligible deposit: tagged front end receives HALAL rewards (1605ms)
      ...
      ✓ provideToSP(), topup: triggers HALAL reward event - increases the sum G (1007ms)
      ✓ provideToSP(), topup from different front end: doesn't change the front end tag (1511ms)
      ✓ provideToSP(), topup: depositor receives HALAL rewards (1487ms)
      ✓ provideToSP(), topup: tagged front end receives HALAL rewards (1036ms)
      ...
      ✓ withdrawFromSP(): partial retrieval - retrieves correct USDH amount and the entire ETH Gain, and updates deposit (1102ms)
      ✓ withdrawFromSP(): partial retrieval - leaves the correct amount of USDH in the Stability Pool (1128ms)
      ✓ withdrawFromSP(): full retrieval - leaves the correct amount of USDH in the Stability Pool (1091ms)
      ...
      ✓ withdrawFromSP(): it correctly updates the user's USDH and ETH snapshots of entitled reward per unit staked (1049ms)
      ...
      ✓ withdrawFromSP(): increases depositor's USDH token balance by the expected amount (2098ms)
      ...
      ✓ withdrawFromSP(): withdrawing 0 USDH doesn't alter the caller's deposit or the total USDH in the Stability Pool (993ms)
      ...
      ✓ withdrawFromSP(): Request to withdraw 2^256-1 USDH only withdraws the caller's compounded deposit (1229ms)
      ...
      ✓ withdrawFromSP(): triggers HALAL reward event - increases the sum G (982ms)
      ✓ withdrawFromSP(), partial withdrawal: doesn't change the front end tag (1421ms)
      ✓ withdrawFromSP(), partial withdrawal: depositor receives HALAL rewards (1155ms)
      ✓ withdrawFromSP(), partial withdrawal: tagged front end receives HALAL rewards (1245ms)
      ...
      ✓ withdrawETHGainToTrove(): Applies USDHLoss to user's deposit, and redirects ETH reward to user's Trove (932ms)
      ...
      ✓ withdrawETHGainToTrove(): triggers HALAL reward event - increases the sum G (1298ms)
      ✓ withdrawETHGainToTrove(), partial withdrawal: doesn't change the front end tag (4778ms)
      ✓ withdrawETHGainToTrove(), eligible deposit: depositor receives HALAL rewards (1607ms)
      ✓ withdrawETHGainToTrove(), eligible deposit: tagged front end receives HALAL rewards (1701ms)
...
B stake after A11: 39999999999999999998008332745
  ...
  Contract: TroveManager - in Recovery Mode
    ...
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool USDH > debt to liquidate: offsets the trove entirely with the pool (1265ms)
    ✓ liquidate(), with ICR% = 110 < TCR, and StabilityPool USDH > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus (1382ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool USDH > debt to liquidate: removes stake and updates totalStakes (1361ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool USDH > debt to liquidate: updates system snapshots (1287ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool USDH > debt to liquidate: closes the Trove (6532ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool USDH > debt to liquidate: can liquidate troves out of order (2667ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool USDH < liquidated debt: Trove remains active (985ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool USDH < liquidated debt: Trove remains in TroveOwners array (1134ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool USDH < liquidated debt: nothing happens (1146ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool USDH < liquidated debt: updates system shapshots (1284ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool USDH < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves (1266ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool USDH < liquidated debt: ICR of non liquidated trove does not change (2161ms)
    ✓ liquidate() with ICR > 110%, and StabilityPool USDH < liquidated debt: total liquidated coll and debt is correct (1912ms)
...
gasUsed:  636956
    ...
    ✓ batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool USDH > debt to liquidate: can liquidate troves out of order (1357ms)
  ...
  Contract: TroveManager
    ...
    ✓ liquidate(): decreases ActivePool ETH and USDHDebt by correct amounts (567ms)
    ✓ liquidate(): increases DefaultPool ETH and USDH debt by correct amounts (553ms)
    ...
    ✓ liquidate(): updates the L_ETH and L_USDHDebt reward-per-unit-staked totals (984ms)
    ...
    ✓ liquidate(): when SP > 0, triggers HALAL reward event - increases the sum G (1081ms)
    ...
    ✓ liquidateTroves(): when SP > 0, triggers HALAL reward event - increases the sum G (5057ms)
    ...
    ✓ batchLiquidateTroves: when SP > 0, triggers HALAL reward event - increases the sum G (1428ms)
    ...
    ✓ redeemCollateral(): cancels the provided USDH with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether (838ms)
    ...
    ✓ redeemCollateral(): caller can redeem their entire USDHToken balance (1086ms)
    ✓ redeemCollateral(): reverts when requested redemption amount exceeds caller's USDH token balance (1320ms)
    ✓ redeemCollateral(): value of issued ETH == face value of redeemed USDH (assuming 1 USDH has value of $1) (1353ms)
    ...
    ✓ redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to HALAL staking contract (5136ms)
    ✓ redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-HALAL-staked in HALAL Staking contract (1207ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to HALAL staking contract (1730ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-HALAL-staked in the staking contract (5971ms)
    ...
    ✓ getPendingUSDHDebtReward(): Returns 0 if there is no pending USDHDebt reward (618ms)
    ✓ getPendingETHReward(): Returns 0 if there is no pending ETH reward (620ms)
    ✓ computeICR(): Returns 0 if trove's coll is worth 0
    ✓ computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 USDH
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 USDH
    ✓ computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 USDH
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 USDH
