...
contract("SortedTroves", async accounts => {
  ...
  describe("SortedTroves", () => {
    ...
    it("contains(): returns false for addresses that opened and then closed a trove", async () => {
      ...
      await usdhToken.transfer(alice, dec(1000, 18), { from: whale });
      await usdhToken.transfer(bob, dec(1000, 18), { from: whale });
      await usdhToken.transfer(carol, dec(1000, 18), { from: whale });
