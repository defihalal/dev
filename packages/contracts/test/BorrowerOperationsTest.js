...
const USDHTokenTester = artifacts.require("./USDHTokenTester");
...
/* NOTE: Some of the borrowing tests do not test for specific USDH fee values. They only test that the
 * fees are non-zero when they should occur, and that they decay over time.
 *
 * Specific USDH fee values will depend on the final fee schedule used, and the final choice for
...
contract("BorrowerOperations", async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      contracts = await deploymentHelper.deployUSDHTokenTester(contracts);
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_USDHDebt snapshots", async () => {
      // --- SETUP ---

      const { collateral: aliceCollBefore, totalDebt: aliceDebtBefore } = await openTrove({
        extraUSDHAmount: toBN(dec(15000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      });
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(5000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // price drops to 1ETH:100USDH, reducing Carol's ICR below MCR
      ...
      const L_USDHDebt = await troveManager.L_USDHDebt();
      ...
      const alice_USDHDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1];
      ...
      const bob_USDHDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1];

      assert.equal(alice_ETHrewardSnapshot_Before, 0);
      assert.equal(alice_USDHDebtRewardSnapshot_Before, 0);
      assert.equal(bob_ETHrewardSnapshot_Before, 0);
      assert.equal(bob_USDHDebtRewardSnapshot_Before, 0);
      ...
      const alicePendingUSDHDebtReward = await troveManager.getPendingUSDHDebtReward(alice);
      const bobPendingUSDHDebtReward = await troveManager.getPendingUSDHDebtReward(bob);
      for (reward of [
        alicePendingETHReward,
        bobPendingETHReward,
        alicePendingUSDHDebtReward,
        bobPendingUSDHDebtReward
      ]) {
      ...
      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePendingUSDHDebtReward)));
      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)));
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingUSDHDebtReward)));

      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ETH and L_USDHDebt */
      ...
      const alice_USDHDebtRewardSnapshot_After = alice_rewardSnapshot_After[1];
      ...
      const bob_USDHDebtRewardSnapshot_After = bob_rewardSnapshot_After[1];

      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After, L_ETH), 100);
      assert.isAtMost(th.getDifference(alice_USDHDebtRewardSnapshot_After, L_USDHDebt), 100);
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After, L_ETH), 100);
      assert.isAtMost(th.getDifference(bob_USDHDebtRewardSnapshot_After, L_USDHDebt), 100);
    });
    ...
    //   // --- SETUP ---
    //   // A,B,C add 15/5/5 ETH, withdraw 100/100/900 USDH
    ...
    //   // close Carol's Trove, liquidating her 5 ether and 900USDH.
    ...
    it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:150USDH, reducing TCR below 150%
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_USDHDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180USDH.
      await troveManager.liquidate(carol, { from: owner });
      ...
      const pendingDebtReward_A = await troveManager.getPendingUSDHDebtReward(alice);
      const pendingCollReward_B = await troveManager.getPendingETHReward(bob);
      const pendingDebtReward_B = await troveManager.getPendingUSDHDebtReward(bob);
      for (reward of [
        pendingCollReward_A,
        pendingDebtReward_A,
        pendingCollReward_B,
    ...
    it("withdrawUSDH(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } });
      ...
      const USDHwithdrawal = 1; // withdraw 1 wei USDH

      await assertRevert(
        borrowerOperations.withdrawUSDH(th._100pct, USDHwithdrawal, alice, alice, { from: alice }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
      );
    });

    it("withdrawUSDH(): decays a non-zero base rate", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });

      await openTrove({
        extraUSDHAmount: toBN(dec(20, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      const A_USDHBal = await usdhToken.balanceOf(A);
      ...
      // D withdraws USDH
      await borrowerOperations.withdrawUSDH(th._100pct, dec(1, 18), A, A, { from: D });
      ...
      // E withdraws USDH
      await borrowerOperations.withdrawUSDH(th._100pct, dec(1, 18), A, A, { from: E });

      const baseRate_3 = await troveManager.baseRate();
      assert.isTrue(baseRate_3.lt(baseRate_2));
    });

    it("withdrawUSDH(): reverts if max fee > 100%", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(40, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
      await openTrove({
      ...
      await assertRevert(
        borrowerOperations.withdrawUSDH(dec(2, 18), dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawUSDH("1000000000000000001", dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
    });

    it("withdrawUSDH(): reverts if max fee < 0.5% in Normal mode", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      ...
      await assertRevert(
        borrowerOperations.withdrawUSDH(0, dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawUSDH(1, dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
      await assertRevert(
        borrowerOperations.withdrawUSDH("4999999999999999", dec(1, 18), A, A, { from: A }),
        "Max fee percentage must be between 0.5% and 100%"
      );
    });

    it("withdrawUSDH(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(60, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(70, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
      await openTrove({
        extraUSDHAmount: toBN(dec(80, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D }
      });
      await openTrove({
        extraUSDHAmount: toBN(dec(180, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E }
      });

      const totalSupply = await usdhToken.totalSupply();
      ...
      await assertRevert(
        borrowerOperations.withdrawUSDH(lessThan5pct, dec(3, 18), A, A, { from: A }),
      ...
      await assertRevert(
        borrowerOperations.withdrawUSDH(dec(1, 16), dec(1, 18), A, A, { from: B }),
        "Fee exceeded provided maximum"
      );
      ...
      await assertRevert(
        borrowerOperations.withdrawUSDH(dec(3754, 13), dec(1, 18), A, A, { from: C }),
        "Fee exceeded provided maximum"
      );
      ...
      await assertRevert(
        borrowerOperations.withdrawUSDH(dec(5, 15), dec(1, 18), A, A, { from: D }),
        "Fee exceeded provided maximum"
      );
    });

    it("withdrawUSDH(): succeeds when fee is less than max fee percentage", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(60, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      ...
      const tx1 = await borrowerOperations.withdrawUSDH(moreThan5pct, dec(1, 18), A, A, { from: A });
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawUSDH(dec(5, 16), dec(1, 18), A, A, { from: B });
      ...
      const tx3 = await borrowerOperations.withdrawUSDH(dec(1, 17), dec(1, 18), A, A, { from: C });
      ...
      const tx4 = await borrowerOperations.withdrawUSDH(dec(37659, 13), dec(1, 18), A, A, {
        from: D
      });
      assert.isTrue(tx4.receipt.status);

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.withdrawUSDH(dec(1, 18), dec(1, 18), A, A, { from: E });
      assert.isTrue(tx5.receipt.status);
    });

    it("withdrawUSDH(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });

      await openTrove({
        extraUSDHAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(50, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
      await openTrove({
      ...
      // D withdraws USDH
      await borrowerOperations.withdrawUSDH(th._100pct, dec(37, 18), A, A, { from: D });
      ...
      // E opens trove
      await borrowerOperations.withdrawUSDH(th._100pct, dec(12, 18), A, A, { from: E });

      const baseRate_3 = await troveManager.baseRate();
      assert.equal(baseRate_3, "0");
    });

    it("withdrawUSDH(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      ...
      await borrowerOperations.withdrawUSDH(th._100pct, dec(1, 18), C, C, { from: C });
    ...
    it("withdrawUSDH(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraUSDHAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
    ...
    it("withdrawUSDH(): borrowing at non-zero base rate sends USDH fee to HALAL staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 HALAL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
      await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
      await halalStaking.stake(dec(1, 18), { from: multisig });

      // Check HALAL USDH balance before == 0
      const halalStaking_USDHBalance_Before = await usdhToken.balanceOf(halalStaking.address);
      assert.equal(halalStaking_USDHBalance_Before, "0");
      ...
      // D withdraws USDH
      await borrowerOperations.withdrawUSDH(th._100pct, dec(37, 18), C, C, { from: D });

      // Check HALAL USDH balance after has increased
      const halalStaking_USDHBalance_After = await usdhToken.balanceOf(halalStaking.address);
      assert.isTrue(halalStaking_USDHBalance_After.gt(halalStaking_USDHBalance_Before));
    });

    if (!withProxy) {
      // TODO: use rawLogs instead of logs
      it("withdrawUSDH(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        // time fast-forwards 1 year, and multisig stakes 1 HALAL
        await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
        await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
        await halalStaking.stake(dec(1, 18), { from: multisig });
        ...
        const withdrawalTx = await borrowerOperations.withdrawUSDH(
          th._100pct,
          toBN(dec(37, 18)),
          D,
          D,
        ...
        const emittedFee = toBN(th.getUSDHFeeFromUSDHBorrowingEvent(withdrawalTx));
    ...
    it("withdrawUSDH(): Borrowing at non-zero base rate increases the HALAL staking contract USDH fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 HALAL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
      await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
      await halalStaking.stake(dec(1, 18), { from: multisig });

      // Check HALAL contract USDH fees-per-unit-staked is zero
      const F_USDH_Before = await halalStaking.F_USDH();
      assert.equal(F_USDH_Before, "0");
      ...
      // D withdraws USDH
      await borrowerOperations.withdrawUSDH(th._100pct, toBN(dec(37, 18)), D, D, { from: D });

      // Check HALAL contract USDH fees-per-unit-staked has increased
      const F_USDH_After = await halalStaking.F_USDH();
      assert.isTrue(F_USDH_After.gt(F_USDH_Before));
    });

    it("withdrawUSDH(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 HALAL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
      await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
      await halalStaking.stake(dec(1, 18), { from: multisig });

      // Check HALAL Staking contract balance before == 0
      const halalStaking_USDHBalance_Before = await usdhToken.balanceOf(halalStaking.address);
      assert.equal(halalStaking_USDHBalance_Before, "0");
      ...
      const D_USDHBalanceBefore = await usdhToken.balanceOf(D);

      // D withdraws USDH
      const D_USDHRequest = toBN(dec(37, 18));
      await borrowerOperations.withdrawUSDH(th._100pct, D_USDHRequest, D, D, { from: D });

      // Check HALAL staking USDH balance has increased
      const halalStaking_USDHBalance_After = await usdhToken.balanceOf(halalStaking.address);
      assert.isTrue(halalStaking_USDHBalance_After.gt(halalStaking_USDHBalance_Before));

      // Check D's USDH balance now equals their initial balance plus request USDH
      const D_USDHBalanceAfter = await usdhToken.balanceOf(D);
      assert.isTrue(D_USDHBalanceAfter.eq(D_USDHBalanceBefore.add(D_USDHRequest)));
    });

    it("withdrawUSDH(): Borrowing at zero base rate changes USDH fees-per-unit-staked", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraUSDHAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // A artificially receives HALAL, then stakes it
      await halalToken.unprotectedMint(A, dec(100, 18));
      await halalStaking.stake(dec(100, 18), { from: A });
      ...
      // D withdraws USDH
      await borrowerOperations.withdrawUSDH(th._100pct, dec(37, 18), D, D, { from: D });

      // Check HALAL USDH balance after > 0
      const F_USDH_After = await halalStaking.F_USDH();
      assert.isTrue(F_USDH_After.gt("0"));
    });

    it("withdrawUSDH(): Borrowing at zero base rate sends debt request to user", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraUSDHAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // Check D's USDH balance now equals their requested USDH
      const D_USDHBalanceAfter = await usdhToken.balanceOf(D);

      // Check D's trove debt == D's USDH balance + liquidation reserve
      assert.isTrue(D_USDHBalanceAfter.eq(D_USDHBalanceBefore.add(D_USDHRequest)));
    });

    it("withdrawUSDH(): reverts when calling address does not have active trove", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });

      // Bob successfully withdraws USDH
      const txBob = await borrowerOperations.withdrawUSDH(th._100pct, dec(100, 18), bob, bob, {
        from: bob
      });
      assert.isTrue(txBob.receipt.status);

      // Carol with no active trove attempts to withdraw USDH
      try {
        const txCarol = await borrowerOperations.withdrawUSDH(
          th._100pct,
          dec(100, 18),
          carol,
          carol,
    ...
    it("withdrawUSDH(): reverts when requested withdrawal amount is zero USDH", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });

      // Bob successfully withdraws 1e-18 USDH
      const txBob = await borrowerOperations.withdrawUSDH(th._100pct, 1, bob, bob, { from: bob });
      assert.isTrue(txBob.receipt.status);

      // Alice attempts to withdraw 0 USDH
      try {
        const txAlice = await borrowerOperations.withdrawUSDH(th._100pct, 0, alice, alice, {
          from: alice
        });
    ...
    it("withdrawUSDH(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } });
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawUSDH(th._100pct, dec(100, 18), alice, alice, {
        from: alice
      });
      assert.isTrue(txAlice.receipt.status);
      ...
      //Check USDH withdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdrawUSDH(th._100pct, 1, bob, bob, { from: bob });
        assert.isFalse(txBob.receipt.status);
      } catch (err) {
    ...
    it("withdrawUSDH(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(11, 17)), extraParams: { from: bob } });

      // Bob tries to withdraw USDH that would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdrawUSDH(th._100pct, 1, bob, bob, { from: bob });
        assert.isFalse(txBob.receipt.status);
      } catch (err) {
    ...
    it("withdrawUSDH(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      await priceFeed.setPrice(dec(100, 18));
      const price = await priceFeed.getPrice();
      ...
      // Bob attempts to withdraw 1 USDH.
      // System TCR would be: ((3+3) * 100 ) / (200+201) = 600/401 = 149.62%, i.e. below CCR of 150%.
      try {
        const txBob = await borrowerOperations.withdrawUSDH(th._100pct, dec(1, 18), bob, bob, {
          from: bob
        });
        assert.isFalse(txBob.receipt.status);
      } catch (err) {
    ...
    it("withdrawUSDH(): reverts if system is in Recovery Mode", async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } });
      ...
      try {
        const txData = await borrowerOperations.withdrawUSDH(th._100pct, "200", alice, alice, {
          from: alice
        });
        assert.isFalse(txData.receipt.status);
      } catch (err) {
    ...
    it("withdrawUSDH(): increases the Trove's USDH debt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      ...
      await borrowerOperations.withdrawUSDH(
        th._100pct,
    ...
    it("withdrawUSDH(): increases USDH debt in ActivePool by correct amount", async () => {
      await openTrove({
        ICR: toBN(dec(10, 18)),
        extraParams: { from: alice, value: toBN(dec(100, "ether")) }
      });
      ...
      // check before
      const activePool_USDH_Before = await activePool.getUSDHDebt();
      assert.isTrue(activePool_USDH_Before.eq(aliceDebtBefore));
      ...
      // check after
      const activePool_USDH_After = await activePool.getUSDHDebt();
      th.assertIsApproximatelyEqual(
        activePool_USDH_After,
        activePool_USDH_Before.add(toBN(dec(10000, 18)))
      );
    });

    it("withdrawUSDH(): increases user USDHToken balance by correct amount", async () => {
      await openTrove({ extraParams: { value: toBN(dec(100, "ether")), from: alice } });

      // check before
      const alice_USDHTokenBalance_Before = await usdhToken.balanceOf(alice);
      assert.isTrue(alice_USDHTokenBalance_Before.gt(toBN("0")));

      await borrowerOperations.withdrawUSDH(th._100pct, dec(10000, 18), alice, alice, {
        from: alice
      });

      // check after
      const alice_USDHTokenBalance_After = await usdhToken.balanceOf(alice);
      assert.isTrue(
        alice_USDHTokenBalance_After.eq(alice_USDHTokenBalance_Before.add(toBN(dec(10000, 18))))
      );
    });

    // --- repayUSDH() ---
    it("repayUSDH(): reverts when repayment would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } });
      ...
      const USDHRepayment = 1; // 1 wei repayment

      await assertRevert(
        borrowerOperations.repayUSDH(USDHRepayment, alice, alice, { from: alice }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
      );
    });

    it("repayUSDH(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      // Make the USDH request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      ...
      const repayTxA = await borrowerOperations.repayUSDH(1, A, A, { from: A });
      ...
      const repayTxB = await borrowerOperations.repayUSDH(dec(19, 25), B, B, { from: B });
      assert.isTrue(repayTxB.receipt.status);
    });

    it("repayUSDH(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      // Make the USDH request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(
        th._100pct,
        await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN("2"))),
      ...
      const repayTxAPromise = borrowerOperations.repayUSDH(2, A, A, { from: A });
      await assertRevert(
        repayTxAPromise,
    ...
    it("repayUSDH(): reverts when calling address does not have active trove", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      ...
      // Bob successfully repays some USDH
      const txBob = await borrowerOperations.repayUSDH(dec(10, 18), bob, bob, { from: bob });
      assert.isTrue(txBob.receipt.status);

      // Carol with no active trove attempts to repayUSDH
      try {
        const txCarol = await borrowerOperations.repayUSDH(dec(10, 18), carol, carol, {
          from: carol
        });
        assert.isFalse(txCarol.receipt.status);
      } catch (err) {
    ...
    it("repayUSDH(): reverts when attempted repayment is > the debt of the trove", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      ...
      try {
        const txAlice = await borrowerOperations.repayUSDH(
    ...
    //repayUSDH: reduces USDH debt in Trove
    it("repayUSDH(): reduces the Trove's USDH debt by the correct amount", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      ...
      await borrowerOperations.repayUSDH(aliceDebtBefore.div(toBN(10)), alice, alice, {
        from: alice
    ...
    it("repayUSDH(): decreases USDH debt in ActivePool by correct amount", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      ...
      // Check before
      const activePool_USDH_Before = await activePool.getUSDHDebt();
      assert.isTrue(activePool_USDH_Before.gt(toBN("0")));
      ...
      th.assertIsApproximatelyEqual(
        activePool_USDH_After,
        activePool_USDH_Before.sub(aliceDebtBefore.div(toBN(10)))
      );
    });

    it("repayUSDH(): decreases user USDHToken balance by correct amount", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      ...
      const alice_USDHTokenBalance_After = await usdhToken.balanceOf(alice);
      th.assertIsApproximatelyEqual(
        alice_USDHTokenBalance_After,
        alice_USDHTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10)))
      );
    });

    it("repayUSDH(): can repay debt in Recovery Mode", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      ...
      const tx = await borrowerOperations.repayUSDH(aliceDebtBefore.div(toBN(10)), alice, alice, {
        from: alice
      });
    ...
    it("repayUSDH(): Reverts if borrower has insufficient USDH balance to cover his debt repayment", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      ...
      const bobBalBefore = await usdhToken.balanceOf(B);
      assert.isTrue(bobBalBefore.gt(toBN("0")));

      // Bob transfers all but 5 of his USDH to Carol
      await usdhToken.transfer(C, bobBalBefore.sub(toBN(dec(5, 18))), { from: B });

      //Confirm B's USDH balance has decreased to 5 USDH
      const bobBalAfter = await usdhToken.balanceOf(B);

      assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))));

      // Bob tries to repay 6 USDH
      const repayUSDHPromise_B = borrowerOperations.repayUSDH(toBN(dec(6, 18)), B, B, { from: B });

      await assertRevert(repayUSDHPromise_B, "Caller doesnt have enough USDH to make repayment");
    });
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(th._100pct, 0, USDHRepayment, false, alice, alice, {
          from: alice,
          value: collTopUp
        }),
        "BorrowerOps: An operation that would result in ICR < MCR is not permitted"
    ...
    it("adjustTrove(): borrowing at non-zero base rate sends USDH fee to HALAL staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 HALAL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
      await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
      await halalStaking.stake(dec(1, 18), { from: multisig });
      ...
      // D adjusts trove
      await openTrove({
        extraUSDHAmount: toBN(dec(37, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D }
      });
    ...
    if (!withProxy) {
      ...
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const emittedFee = toBN(th.getUSDHFeeFromUSDHBorrowingEvent(adjustmentTx));
        assert.isTrue(emittedFee.gt(toBN("0")));
    ...
    it("adjustTrove(): Borrowing at non-zero base rate increases the HALAL staking contract USDH fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 HALAL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
      await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
      await halalStaking.stake(dec(1, 18), { from: multisig });
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D adjusts trove
      const USDHRequest_D = toBN(dec(40, 18));
      await borrowerOperations.adjustTrove(th._100pct, 0, USDHRequest_D, true, D, D, { from: D });

      // Check HALAL staking USDH balance has increased
      const halalStaking_USDHBalance_After = await usdhToken.balanceOf(halalStaking.address);
      assert.isTrue(halalStaking_USDHBalance_After.gt(halalStaking_USDHBalance_Before));

      // Check D's USDH balance has increased by their requested USDH
      const D_USDHBalanceAfter = await usdhToken.balanceOf(D);
      assert.isTrue(D_USDHBalanceAfter.eq(D_USDHBalanceBefore.add(USDHRequest_D)));
    });

    it("adjustTrove(): Borrowing at zero base rate changes USDH balance of HALAL staking contract", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
      await openTrove({
        extraUSDHAmount: toBN(dec(30, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // Check staking USDH balance before > 0
      const halalStaking_USDHBalance_Before = await usdhToken.balanceOf(halalStaking.address);
      assert.isTrue(halalStaking_USDHBalance_Before.gt(toBN("0")));

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D });

      // Check staking USDH balance after > staking balance before
      const halalStaking_USDHBalance_After = await usdhToken.balanceOf(halalStaking.address);
      assert.isTrue(halalStaking_USDHBalance_After.gt(halalStaking_USDHBalance_Before));
    });

    it("adjustTrove(): Borrowing at zero base rate changes HALAL staking contract USDH fees-per-unit-staked", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(20000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: toBN(dec(100, "ether")) }
      });
      await openTrove({
        extraUSDHAmount: toBN(dec(40000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await openTrove({
      ...
      // Check staking USDH balance before == 0
      const F_USDH_Before = await halalStaking.F_USDH();
      assert.isTrue(F_USDH_Before.eq(toBN("0")));

      // D adjusts trove
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, { from: D });

      // Check staking USDH balance increases
      const F_USDH_After = await halalStaking.F_USDH();
      assert.isTrue(F_USDH_After.gt(F_USDH_Before));
    });

    it("adjustTrove(): Borrowing at zero base rate sends total requested USDH to the user", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(20000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: toBN(dec(100, "ether")) }
      ...
      const D_USDHBalBefore = await usdhToken.balanceOf(D);
      // Check baseRate is zero
      const baseRate_1 = await troveManager.baseRate();
      assert.equal(baseRate_1, "0");
      ...
      const DUSDBalanceBefore = await usdhToken.balanceOf(D);

      // D adjusts trove
      const USDHRequest_D = toBN(dec(40, 18));
      await borrowerOperations.adjustTrove(th._100pct, 0, USDHRequest_D, true, D, D, { from: D });

      // Check D's USDH balance increased by their requested USDH
      const USDHBalanceAfter = await usdhToken.balanceOf(D);
      assert.isTrue(USDHBalanceAfter.eq(D_USDHBalBefore.add(USDHRequest_D)));
    });
    ...
    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      ...
      // Alice coll and debt increase(+1 ETH, +50USDH)
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(200000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: bob }
      });
      ...
      // B stakes HALAL
      await halalToken.unprotectedMint(bob, dec(100, 18));
      await halalStaking.stake(dec(100, 18), { from: bob });

      const halalStakingUSDHBalanceBefore = await usdhToken.balanceOf(halalStaking.address);
      assert.isTrue(halalStakingUSDHBalanceBefore.gt(toBN("0")));
      ...
      const emittedFee = toBN(
        await th.getEventArgByName(txAlice, "USDHBorrowingFeePaid", "_USDHFee")
      );
      ...
      const halalStakingUSDHBalanceAfter = await usdhToken.balanceOf(halalStaking.address);
      assert.equal(
        halalStakingUSDHBalanceAfter.toString(),
        halalStakingUSDHBalanceBefore.toString()
      );
    });
    ...
    it("adjustTrove(): reverts when USDH repaid is > debt of the trove", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      ...
      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, "USDHBorrowingFeePaid", 1));
      assert.isTrue(bobFee.gt(toBN("0")));

      // Alice transfers USDH to bob to compensate borrowing fees
      await usdhToken.transfer(bob, bobFee, { from: alice });

      const remainingDebt = (await troveManager.getTroveDebt(bob)).sub(USDH_GAS_COMPENSATION);
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      ...
      // Bob attempts to increase debt by 100 USDH and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      // Alice adjusts trove. No coll change, and a debt increase (+50USDH)
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
        value: 0
      });
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice);
      const activePoolDebtBefore = await activePool.getUSDHDebt();
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice);
      const activePoolDebtAfter = await activePool.getUSDHDebt();
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ETH, +50USDH)
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50USDH)
      await borrowerOperations.adjustTrove(
        th._100pct,
        dec(500, "finney"),
        dec(50, 18),
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50USDH)
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10USDH)
      await borrowerOperations.adjustTrove(
        th._100pct,
        dec(1, 17),
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      // Alice adjusts trove - coll and debt increase (+1 ETH, +50 USDH)
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, {
        from: alice,
        value: dec(1, "ether")
      });
    ...
    it("adjustTrove(): changes USDHToken balance by the requested decrease", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      const alice_USDHTokenBalance_After = await usdhToken.balanceOf(alice);
      assert.isTrue(
        alice_USDHTokenBalance_After.eq(alice_USDHTokenBalance_Before.sub(toBN(dec(10, 18))))
      );
    });

    it("adjustTrove(): changes USDHToken balance by the requested increase", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      const alice_USDHTokenBalance_After = await usdhToken.balanceOf(alice);
      assert.isTrue(
        alice_USDHTokenBalance_After.eq(alice_USDHTokenBalance_Before.add(toBN(dec(100, 18))))
      );
    });
    ...
    it("adjustTrove(): Changes the USDH debt in ActivePool by requested decrease", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      const activePool_USDHDebt_Before = await activePool.getUSDHDebt();
      assert.isTrue(activePool_USDHDebt_Before.gt(toBN("0")));
      ...
      const activePool_USDHDebt_After = await activePool.getUSDHDebt();
      assert.isTrue(activePool_USDHDebt_After.eq(activePool_USDHDebt_Before.sub(toBN(dec(30, 18)))));
    });

    it("adjustTrove(): Changes the USDH debt in ActivePool by requested increase", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      th.assertIsApproximatelyEqual(
        activePool_USDHDebt_After,
        activePool_USDHDebt_Before.add(toBN(dec(100, 18)))
      );
    });
    ...
    it("adjustTrove(): Reverts if borrower has insufficient USDH balance to cover his debt repayment", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Bob transfers some USDH to carol
      await usdhToken.transfer(C, dec(10, 18), { from: B });

      //Confirm B's USDH balance is less than 50 USDH
      const B_USDHBal = await usdhToken.balanceOf(B);
      assert.isTrue(B_USDHBal.lt(bobDebt));

      const repayUSDHPromise_B = borrowerOperations.adjustTrove(
        th._100pct,
        0,
        bobDebt,
        false,
      ...
      await assertRevert(repayUSDHPromise_B, "revert");
    });
    ...
    it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
      ...
      await openTrove({
        ICR: toBN(dec(120, 16)),
        extraUSDHAmount: toBN(dec(300, 18)),
        extraParams: { from: bob }
      });
      ...
      await usdhToken.transfer(alice, dec(300, 18), { from: bob });
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(100000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: alice }
      });
      await openTrove({
      ...
      // Alice transfers her USDH to Bob and Carol so they can cover fees
      const aliceBal = await usdhToken.balanceOf(alice);
      await usdhToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice });
      await usdhToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice });
    ...
    it("closeTrove(): reverts when trove is the only one in the system", async () => {
      ...
      await usdhToken.unprotectedMint(alice, dec(100000, 18));

      // Check she has more USDH than her trove debt
      const aliceBal = await usdhToken.balanceOf(alice);
      const aliceDebt = await getTroveEntireDebt(alice);
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice);
      const dennisUSDH = await usdhToken.balanceOf(dennis);
      assert.isTrue(aliceCollBefore.gt(toBN("0")));
      assert.isTrue(dennisUSDH.gt(toBN("0")));

      // To compensate borrowing fees
      await usdhToken.transfer(alice, dennisUSDH.div(toBN(2)), { from: dennis });
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_USDHDebt_A_Snapshot = (await troveManager.rewardSnapshots(alice))[1];
      assert.isTrue(L_ETH_A_Snapshot.gt(toBN("0")));
      assert.isTrue(L_USDHDebt_A_Snapshot.gt(toBN("0")));
      ...
      const L_USDHDebt_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[1];

      assert.isTrue(L_ETH_Snapshot_A_AfterLiquidation.gt(toBN("0")));
      assert.isTrue(L_USDHDebt_Snapshot_A_AfterLiquidation.gt(toBN("0")));

      // to compensate borrowing fees
      await usdhToken.transfer(alice, await usdhToken.balanceOf(dennis), { from: dennis });
      ...
      const L_USDHDebt_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[1];

      assert.equal(L_ETH_Snapshot_A_afterAliceCloses, "0");
      assert.equal(L_USDHDebt_Snapshot_A_afterAliceCloses, "0");
    });
    ...
    it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
      ...
      // Check before
      const activePool_Debt_before = await activePool.getUSDHDebt();
      ...
      // Check after
      const activePool_Debt_After = (await activePool.getUSDHDebt()).toString();
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's USDHToken balance", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: dennis }
      ...
      const alice_USDHBalance_Before = await usdhToken.balanceOf(alice);
      assert.isTrue(alice_USDHBalance_Before.gt(toBN("0")));

      // close trove
      await borrowerOperations.closeTrove({ from: alice });

      // check alice USDH balance after
      const alice_USDHBalance_After = await usdhToken.balanceOf(alice);
      th.assertIsApproximatelyEqual(
        alice_USDHBalance_After,
        alice_USDHBalance_Before.sub(aliceDebt.sub(USDH_GAS_COMPENSATION))
      );
    });
    ...
    it("closeTrove(): applies pending rewards", async () => {
      // --- SETUP ---
      await openTrove({
        extraUSDHAmount: toBN(dec(1000000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      });
      ...
      await usdhToken.transfer(alice, dec(10000, 18), { from: whale });
      await usdhToken.transfer(bob, dec(10000, 18), { from: whale });
      ...
      const defaultPool_USDHDebt = await defaultPool.getUSDHDebt();

      // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100);
      assert.isAtMost(th.getDifference(defaultPool_USDHDebt, liquidatedDebt_C), 100);
      ...
      const defaultPool_USDHDebt_afterAliceCloses = await defaultPool.getUSDHDebt();
      ...
      assert.isAtMost(
        th.getDifference(
          defaultPool_USDHDebt_afterAliceCloses,
          defaultPool_USDHDebt.sub(pendingDebtReward_A)
        ),
        1000
      );
      ...
      const defaultPool_USDHDebt_afterBobCloses = await defaultPool.getUSDHDebt();

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses, 0), 100000);
      assert.isAtMost(th.getDifference(defaultPool_USDHDebt_afterBobCloses, 0), 100000);
    });

    it("closeTrove(): reverts if borrower has insufficient USDH balance to repay his entire debt", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(15000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      ...
      //Confirm Bob's USDH balance is less than his trove debt
      const B_USDHBal = await usdhToken.balanceOf(B);
      const B_troveDebt = await getTroveEntireDebt(B);

      assert.isTrue(B_USDHBal.lt(B_troveDebt));
      ...
      await assertRevert(
        closeTrovePromise_B,
        "BorrowerOps: Caller doesnt have enough USDH to make repayment"
      );
    });
    ...
    if (!withProxy) {
      ...
      it("openTrove(): emits a TroveUpdated event with the correct collateral and debt", async () => {
        ...
        const txC = (
          await openTrove({
            extraUSDHAmount: toBN(dec(3000, 18)),
            ICR: toBN(dec(2, 18)),
            extraParams: { from: C }
          })
        ).tx;
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(30000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: B }
      });
      await openTrove({
      ...
      // E opens trove
      await openTrove({
        extraUSDHAmount: toBN(dec(12, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: E }
      });
    ...
    it("openTrove(): borrowing at non-zero base rate sends USDH fee to HALAL staking contract", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 HALAL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
      await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
      await halalStaking.stake(dec(1, 18), { from: multisig });
    ...
    if (!withProxy) {
      ...
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        const D_USDHRequest = toBN(dec(20000, 18));
        ...
        const emittedFee = toBN(th.getUSDHFeeFromUSDHBorrowingEvent(openTroveTx));
        ...
        th.assertIsApproximatelyEqual(
          newDebt,
          D_USDHRequest.add(emittedFee).add(USDH_GAS_COMPENSATION),
          100000
        );
      });
    }

    it("openTrove(): Borrowing at non-zero base rate increases the HALAL staking contract USDH fees-per-unit-staked", async () => {
      // time fast-forwards 1 year, and multisig stakes 1 HALAL
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
      await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
      await halalStaking.stake(dec(1, 18), { from: multisig });
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      // D opens trove
      const USDHRequest_D = toBN(dec(40000, 18));
      await borrowerOperations.openTrove(th._100pct, USDHRequest_D, D, D, {
        from: D,
        value: dec(500, "ether")
      });
      ...
      // Check D's USDH balance now equals their requested USDH
      const USDHBalance_D = await usdhToken.balanceOf(D);
      assert.isTrue(USDHRequest_D.eq(USDHBalance_D));
    });

    it("openTrove(): Borrowing at zero base rate changes the HALAL staking contract USDH fees-per-unit-staked", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(5000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      ...
      // Check USDH reward per HALAL staked == 0
      const F_USDH_Before = await halalStaking.F_USDH();
      assert.equal(F_USDH_Before, "0");
      ...
      // Check USDH reward per HALAL staked > 0
      const F_USDH_After = await halalStaking.F_USDH();
      assert.isTrue(F_USDH_After.gt(toBN("0")));
    });
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const USDHRequest = toBN(dec(10000, 18));
      const txC = await borrowerOperations.openTrove(
        th._100pct,
        USDHRequest,
        ZERO_ADDRESS,
      ...
      const _USDHFee = toBN(th.getEventArgByName(txC, "USDHBorrowingFeePaid", "_USDHFee"));

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(USDHRequest)).div(toBN(dec(1, 18)));
      assert.isTrue(_USDHFee.eq(expectedFee));
    });
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:100USDH, reducing TCR below 150%
      await priceFeed.setPrice("100000000000000000000");
      const price = await priceFeed.getPrice();
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      const USDHRequest = MIN_NET_DEBT;
      borrowerOperations.openTrove(th._100pct, MIN_NET_DEBT, carol, carol, {
        from: alice,
        value: dec(100, "ether")
      });

      // Get the expected debt based on the USDH request (adding fee and liq. reserve on top)
      const expectedDebt = USDHRequest.add(await troveManager.getBorrowingFee(USDHRequest)).add(
        USDH_GAS_COMPENSATION
      );
    ...
    it("openTrove(): records up-to-date initial snapshots of L_ETH and L_USDHDebt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ETH should equal 1/10 ether per-ether-staked,
       and L_USDH should equal 18 USDH per-ether-staked. */

      const L_ETH = await troveManager.L_ETH();
      const L_USDH = await troveManager.L_USDHDebt();

      assert.isTrue(L_ETH.gt(toBN("0")));
      assert.isTrue(L_USDH.gt(toBN("0")));
      ...
      // Check Bob's snapshots of L_ETH and L_USDH equal the respective current values
      const bob_rewardSnapshot = await troveManager.rewardSnapshots(bob);
      const bob_ETHrewardSnapshot = bob_rewardSnapshot[0];
      const bob_USDHDebtRewardSnapshot = bob_rewardSnapshot[1];

      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_ETH), 1000);
      assert.isAtMost(th.getDifference(bob_USDHDebtRewardSnapshot, L_USDH), 1000);
    });
    ...
    it("openTrove(): increases the Trove's USDH debt by the correct amount", async () => {
      // check before
      const alice_Trove_Before = await troveManager.Troves(alice);
    ...
    it("openTrove(): increases USDH debt in ActivePool by the debt of the trove", async () => {
      const activePool_USDHDebt_Before = await activePool.getUSDHDebt();
      assert.equal(activePool_USDHDebt_Before, 0);
      ...
      const activePool_USDHDebt_After = await activePool.getUSDHDebt();
      assert.isTrue(activePool_USDHDebt_After.eq(aliceDebt));
    });

    it("openTrove(): increases user USDHToken balance by correct amount", async () => {
      // check before
      const alice_USDHTokenBalance_Before = await usdhToken.balanceOf(alice);
      assert.equal(alice_USDHTokenBalance_Before, 0);
      ...
      // check after
      const alice_USDHTokenBalance_After = await usdhToken.balanceOf(alice);
      assert.equal(alice_USDHTokenBalance_After, dec(10000, 18));
    });
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      assert.equal(res1, USDH_GAS_COMPENSATION.toString());

      const res2 = await borrowerOperations.getCompositeDebt(dec(90, 18));
      th.assertIsApproximatelyEqual(res2, USDH_GAS_COMPENSATION.add(toBN(dec(90, 18))));

      const res3 = await borrowerOperations.getCompositeDebt(dec(24423422357345049, 12));
      th.assertIsApproximatelyEqual(
        res3,
        USDH_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12)))
      );
    });
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        const troveUSDHAmount = await getOpenTroveUSDHAmount(troveTotalDebt);
        await borrowerOperations.openTrove(th._100pct, troveUSDHAmount, alice, alice, {
          from: alice,
          value: troveColl
        });
        await borrowerOperations.openTrove(th._100pct, troveUSDHAmount, bob, bob, {
          from: bob,
          value: troveColl
        });
    ...
    if (!withProxy) {
      it("closeTrove(): fails if owner cannot receive ETH", async () => {
        ...
        // Alice sends USDH to NonPayable so its USDH balance covers its debt
        await usdhToken.transfer(nonPayable.address, dec(10000, 18), { from: alice });
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdrawUSDH, repayUSDH, etc. Can split them up and put them with
