...
contract("All Liquity functions with onlyOwner modifier", async accounts => {
  ...
  describe("CommunityIssuance", async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      const params = [halalToken.address, stabilityPool.address];
  ...
  describe("HALALStaking", async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      await testSetAddresses(halalStaking, 5);
    });
  });

  describe("LockupContractFactory", async accounts => {
    it("setHALALAddress(): reverts when called by non-owner, with wrong address, or twice", async () => {
      await th.assertRevert(
        lockupContractFactory.setHALALTokenAddress(halalToken.address, { from: alice })
      );

      const params = [halalToken.address];

      // Attempt to use zero address
      await testZeroAddress(lockupContractFactory, params, "setHALALTokenAddress");
      // Attempt to use non contract
      await testNonContractAddress(lockupContractFactory, params, "setHALALTokenAddress");

      // Owner can successfully set any address
      const txOwner = await lockupContractFactory.setHALALTokenAddress(halalToken.address, {
        from: owner
      });
      ...
      // fails if called twice
      await th.assertRevert(
        lockupContractFactory.setHALALTokenAddress(halalToken.address, { from: owner })
      );
    });
  });
});
