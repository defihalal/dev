...
const USDHTokenTester = artifacts.require("./USDHTokenTester.sol");
...
contract("TroveManager", async accounts => {
  ...
  const getOpenTroveTotalDebt = async usdhAmount => th.getOpenTroveTotalDebt(contracts, usdhAmount);
  ...
  const withdrawUSDH = async params => th.withdrawUSDH(contracts, params);

  beforeEach(async () => {
    contracts = await deploymentHelper.deployLiquityCore();
    contracts.troveManager = await TroveManagerTester.new();
    contracts.usdhToken = await USDHTokenTester.new(
  ...
  it("liquidate(): closes a Trove that has ICR < MCR", async () => {
    ...
    // Alice increases debt to 180 USDH, lowering her ICR to 1.11
    const A_USDHWithdrawal = await getNetBorrowingAmount(dec(130, 18));

    const targetICR = toBN("1111111111111111111");
    await withdrawUSDH({ ICR: targetICR, extraParams: { from: alice } });
    ...
    // price drops to 1ETH:100USDH, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool ETH and USDHDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool ETH and USDH debt before
    ...
    const activePool_USDHDebt_Before = (await activePool.getUSDHDebt()).toString();
    ...
    th.assertIsApproximatelyEqual(activePool_USDHDebt_Before, A_totalDebt.add(B_totalDebt));

    // price drops to 1ETH:100USDH, reducing Bob's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and USDH, 
    leaving Alice’s ether and USDH debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool ETH and USDH debt
    ...
    const activePool_USDHDebt_After = (await activePool.getUSDHDebt()).toString();

    assert.equal(activePool_ETH_After, A_collateral);
    assert.equal(activePool_RawEther_After, A_collateral);
    th.assertIsApproximatelyEqual(activePool_USDHDebt_After, A_totalDebt);
  });

  it("liquidate(): increases DefaultPool ETH and USDH debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(4, 18)),
      extraParams: { from: alice }
    ...
    // check DefaultPool ETH and USDH debt before
    ...
    const defaultPool_USDHDebt_Before = (await defaultPool.getUSDHDebt()).toString();

    assert.equal(defaultPool_ETH_Before, "0");
    assert.equal(defaultPool_RawEther_Before, "0");
    assert.equal(defaultPool_USDHDebt_Before, "0");
    ...
    const defaultPool_USDHDebt_After = (await defaultPool.getUSDHDebt()).toString();
    ...
    th.assertIsApproximatelyEqual(defaultPool_USDHDebt_After, B_totalDebt);
  });
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and USDH should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): updates the L_ETH and L_USDHDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(8, 18)),
      extraParams: { from: alice }
    ...
    // price drops to 1ETH:100USDH, reducing Carols's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");
    ...
    // Carol's ether*0.995 and USDH should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManager.L_ETH();
    const L_USDHDebt_AfterCarolLiquidated = await troveManager.L_USDHDebt();

    const L_ETH_expected_1 = th
      .applyLiquidationFee(C_collateral)
      .mul(mv._1e18BN)
      .div(A_collateral.add(B_collateral));
    const L_USDHDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral));
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated, L_ETH_expected_1), 100);
    assert.isAtMost(th.getDifference(L_USDHDebt_AfterCarolLiquidated, L_USDHDebt_expected_1), 100);

    // Bob now withdraws USDH, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawUSDH({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: bob }
    });
    ...
    // price drops to 1ETH:50USDH, reducing Bob's ICR below MCR
   ...
   L_USDHDebt = (180 / 20) + (890 / 10) = 98 USDH */
    const L_ETH_AfterBobLiquidated = await troveManager.L_ETH();
    const L_USDHDebt_AfterBobLiquidated = await troveManager.L_USDHDebt();
    ...
    const L_USDHDebt_expected_2 = L_USDHDebt_expected_1.add(
      B_totalDebt.add(B_increasedTotalDebt)
        .add(B_collateral.mul(L_USDHDebt_expected_1).div(mv._1e18BN))
        .mul(mv._1e18BN)
        .div(A_collateral)
    );
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100);
    assert.isAtMost(th.getDifference(L_USDHDebt_AfterBobLiquidated, L_USDHDebt_expected_2), 100);
  });
  ...
  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    ...
    // Alice creates a single trove with 0.7 ETH and a debt of 70 USDH, and provides 10 USDH to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraParams: { from: alice }
    });

    // Alice proves 10 USDH to SP
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides USDH to SP
    const spDeposit = toBN(dec(100, 24));
    await openTrove({
      ICR: toBN(dec(4, 18)),
      extraUSDHAmount: spDeposit,
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    const { C_totalDebt, C_collateral } = await openTrove({
      ICR: toBN(dec(218, 16)),
      extraUSDHAmount: toBN(dec(100, 18)),
      extraParams: { from: carol }
    });

    // Bob sends tokens to Dennis, who has no trove
    await usdhToken.transfer(dennis, spDeposit, { from: bob });

    //Dennis provides USDH to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedUSDHDeposit(dennis)).toString();
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedUSDHDeposit(dennis)).toString();
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
    ...
    const bob_Deposit_Before = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
    ...
    const bob_Deposit_After = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated ETH
    const bob_Deposit_Before = await stabilityPool.getCompoundedUSDHDeposit(bob);
    ...
    /* Alice's USDH Loss = (300 / 400) * 200 = 150 USDH
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's USDHLoss = (100 / 400) * 200 = 50 USDH
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 USDH, and his ETH gain has increased to 1.5 ETH. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedUSDHDeposit(alice)).toString();
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedUSDHDeposit(bob);
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
    const { usdhAmount: A_usdhAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDHAmount: toBN(dec(300, 18)),
      extraParams: { from: alice }
    });
    const { usdhAmount: B_usdhAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDHAmount: toBN(dec(200, 18)),
      extraParams: { from: bob }
    });
    const { usdhAmount: C_usdhAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDHAmount: toBN(dec(100, 18)),
      extraParams: { from: carol }
    });
    ...
    // Liquidate A, B and C
    const activeUSDHDebt_0 = await activePool.getUSDHDebt();
    const defaultUSDHDebt_0 = await defaultPool.getUSDHDebt();

    await troveManager.liquidate(alice);
    const activeUSDHDebt_A = await activePool.getUSDHDebt();
    const defaultUSDHDebt_A = await defaultPool.getUSDHDebt();

    await troveManager.liquidate(bob);
    const activeUSDHDebt_B = await activePool.getUSDHDebt();
    const defaultUSDHDebt_B = await defaultPool.getUSDHDebt();
    ...
    assert.equal((await usdhToken.balanceOf(alice)).toString(), A_usdhAmount);
    assert.equal((await usdhToken.balanceOf(bob)).toString(), B_usdhAmount);
    assert.equal((await usdhToken.balanceOf(carol)).toString(), C_usdhAmount);
  });
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Defaulter opens with 60 USDH, 0.6 ETH
    ...
    /* Liquidate defaulter. 30 USDH and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 USDH, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 USDH, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 USDH, and (0.3*1/4) = 0.075 ETH
    */
    await troveManager.liquidate(defaulter_1);
  ...
  it("liquidate(): when SP > 0, triggers HALAL reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalUSDHDeposits(), dec(100, 18));

    const G_Before = await stabilityPool.epochToScaleToG(0, 0);

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);

    // Price drops to 1ETH:100USDH, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
    assert.isFalse(await th.checkRecoveryMode(contracts));
    ...
    // Expect G has increased from the HALAL reward event triggered
  ...
  it("liquidate(): when SP is empty, doesn't update G", async () => {
    ...
    // Check SP is empty
    assert.equal(await stabilityPool.getTotalUSDHDeposits(), "0");
  ...
  it("liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
    ...
    // A adds 10 USDH to the SP, but less than C's debt
    ...
    await borrowerOperations.repayUSDH(dec(1, 18), D, D, { from: D });
    await borrowerOperations.repayUSDH(dec(1, 18), E, E, { from: E });
    ...
    const pendingUSDHDebt_C = await troveManager.getPendingUSDHDebtReward(C);
    const defaultPoolETH = await defaultPool.getETH();
    const defaultPoolUSDHDebt = await defaultPool.getUSDHDebt();
    assert.isTrue(pendingETH_C.lte(defaultPoolETH));
    assert.isTrue(pendingUSDHDebt_C.lte(defaultPoolUSDHDebt));
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000);
    assert.isAtMost(th.getDifference(pendingUSDHDebt_C, defaultPoolUSDHDebt), 1000);
  ...
  it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
    ...
    // Price drops to 1ETH:100USDH, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 USDH and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 USDH, 0.1 ETH
    await troveManager.liquidate(defaulter_1);
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await usdhToken.balanceOf(dennis);
    const E_balanceBefore = await usdhToken.balanceOf(erin);
    const F_balanceBefore = await usdhToken.balanceOf(flyn);
    ...
    assert.equal((await usdhToken.balanceOf(dennis)).toString(), D_balanceBefore);
    assert.equal((await usdhToken.balanceOf(erin)).toString(), E_balanceBefore);
    assert.equal((await usdhToken.balanceOf(flyn)).toString(), F_balanceBefore);
  });

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 USDH to SP
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraUSDHAmount: toBN(dec(500, 18)),
      extraParams: { from: whale }
    });
    ...
    // Check pool has 500 USDH
    assert.equal((await stabilityPool.getTotalUSDHDeposits()).toString(), dec(500, 18));
    ...
    assert.equal((await stabilityPool.getTotalUSDHDeposits()).toString(), "0");
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides 400 USDH to the SP
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDHAmount: A_deposit,
      extraParams: { from: alice }
    });
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDHAmount: B_deposit,
      extraParams: { from: bob }
    });
    const { collateral: C_coll, totalDebt: C_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
    ...
    // Check 800 USDH in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit);
    assert.equal((await stabilityPool.getTotalUSDHDeposits()).toString(), totalDeposits);
    ...
    Total USDH in Pool: 800 USDH

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 USDH
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale usdh loss: 650 * (400/800) = 325 usdh
    alice usdh loss:  650 *(100/800) = 81.25 usdh
    bob usdh loss: 650 * (300/800) = 243.75 usdh

    whale remaining deposit: (400 - 325) = 75 usdh
    alice remaining deposit: (100 - 81.25) = 18.75 usdh
    bob remaining deposit: (300 - 243.75) = 56.25 usdh
    ...
    Total remaining deposits: 150 USDH
    Total ETH gain: 4.975 ETH */

    // Check remaining USDH Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedUSDHDeposit(whale);
    const alice_Deposit_After = await stabilityPool.getCompoundedUSDHDeposit(alice);
    const bob_Deposit_After = await stabilityPool.getCompoundedUSDHDeposit(bob);
    ...
    const total_USDHinSP = (await stabilityPool.getTotalUSDHDeposits()).toString();
    const total_ETHinSP = (await stabilityPool.getETH()).toString();

    assert.isAtMost(th.getDifference(total_USDHinSP, totalDeposits.sub(liquidatedDebt)), 1000);
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000);
  });

  it("liquidateTroves(): when SP > 0, triggers HALAL reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } });
  ...
  it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
    ...
    // Price drops to 1ETH:100USDH, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    th.assertIsApproximatelyEqual(
      (await stabilityPool.getTotalUSDHDeposits()).toString(),
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await usdhToken.transfer(carol, dec(100, 18), { from: whale });
  ...
  it("batchLiquidateTroves: when SP > 0, triggers HALAL reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } });
  ...
  it("getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraUSDHAmount: partialRedemptionAmount,
      extraParams: { from: alice }
    });
  ...
  it("getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations", async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra USDH. At least 3 iterations are needed
  ...
  it("redeemCollateral(): cancels the provided USDH with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraUSDHAmount: dec(10, 18),
      extraParams: { from: alice }
    });
    const { netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(290, 16)),
      extraUSDHAmount: dec(8, 18),
      extraParams: { from: bob }
    });
    ...
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraUSDHAmount: redemptionAmount,
      extraParams: { from: dennis }
    });

    const dennis_ETHBalance_Before = toBN(await web3.eth.getBalance(dennis));

    const dennis_USDHBalance_Before = await usdhToken.balanceOf(dennis);
    ...
    // Find hints for redeeming 20 USDH
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      redemptionAmount,
      price,
      0
    ...
    /* check that Dennis' redeemed 20 USDH has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) USDH debt + 50 for gas compensation. */
    ...
    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)); // convert redemptionAmount USDH to ETH, at ETH:USD price 200
    ...
    const dennis_USDHBalance_After = (await usdhToken.balanceOf(dennis)).toString();
    assert.equal(dennis_USDHBalance_After, dennis_USDHBalance_Before.sub(redemptionAmount));
  });
  ...
  it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
    ...
    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 USDH), 20000%
    const { usdhAmount: F_usdhAmount } = await openTrove({
      ICR: toBN(dec(200, 18)),
      extraUSDHAmount: redemptionAmount.mul(toBN(2)),
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 USDH
    const flynBalance = await usdhToken.balanceOf(flyn);
    th.assertIsApproximatelyEqual(flynBalance, F_usdhAmount.sub(redemptionAmount));
  ...
  it("redeemCollateral(): ends the redemption sequence when max iterations have been reached", async () => {
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 USDH
    const flynBalance = (await usdhToken.balanceOf(flyn)).toString();
    th.assertIsApproximatelyEqual(flynBalance, F_usdhAmount.sub(redemptionAmount));
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    ...
    await usdhToken.transfer(B, await usdhToken.balanceOf(A), { from: A });
    await usdhToken.transfer(B, await usdhToken.balanceOf(C), { from: C });
    ...
    // USDH redemption is 55000 US
    const USDHRedemption = dec(55000, 18);
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, USDHRedemption, th._100pct);
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(
      th._100pct,
      await getOpenTroveUSDHAmount(dec(6000, 18)),
      A,
      A,
      { from: A, value: dec(1000, "ether") }
    );
  ...
  it("redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date", async () => {
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 USDH from Carol's Trove
      await troveManager.redeemCollateral(
        frontRunRedepmtion,
        firstRedemptionHint,
        upperPartialRedemptionHint,
    ...
    // Dennis tries to redeem 20 USDH
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    // Since Alice already redeemed 1 USDH from Carol's Trove, Dennis was  able to redeem:
    //  - 9 USDH from Carol's
    //  - 8 USDH from Bob's
    // for a total of 17 USDH.

    // Dennis calculated his hint for redeeming 2 USDH from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 USDH to fully complete his redemption of 20 USDH.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)); // redempted USDH converted to ETH, at ETH:USD price 200
    ...
    const dennis_USDHBalance_After = (await usdhToken.balanceOf(dennis)).toString();
    th.assertIsApproximatelyEqual(
      dennis_USDHBalance_After,
      dennis_USDHBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion))
    );
  });
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveUSDHAmount(dec(110, 18));
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } });
    await openTrove({
      ICR: toBN(dec(133, 16)),
      extraUSDHAmount: amount,
      extraParams: { from: bob }
    });

    await usdhToken.transfer(carol, amount, { from: bob });
    ...
    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)); // convert 100 USDH to ETH at ETH:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee);

    const receivedETH = carol_ETHBalance_After.sub(carol_ETHBalance_Before);
    assert.isTrue(expectedReceivedETH.eq(receivedETH));

    const carol_USDHBalance_After = (await usdhToken.balanceOf(carol)).toString();
    assert.equal(carol_USDHBalance_After, "0");
  });
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { usdhAmount: B_usdhAmount, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraUSDHAmount: A_debt,
      extraParams: { from: bob }
    });

    await usdhToken.transfer(carol, B_usdhAmount, { from: bob });
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500USDH to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraUSDHAmount: dec(500, 18),
      extraParams: { from: alice }
    });
    await usdhToken.transfer(erin, dec(500, 18), { from: alice });
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDHAmount: dec(30, 18),
      extraParams: { from: C }
    });
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDHAmount: dec(40, 18),
      extraParams: { from: D }
    });
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDHAmount: dec(80, 18),
      extraParams: { from: A }
    });
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDHAmount: dec(90, 18),
      extraParams: { from: B }
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDHAmount: dec(100, 18),
      extraParams: { from: C }
    });
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt);

    // Check total USDH supply
    ...
    // USDH redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedUSDHRedemption = expectedTotalSupply.div(toBN(10));

    // Max fee is <5%
    const lessThan5pct = "49999999999999999";
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDHRedemption, lessThan5pct),
    ...
    // Max fee is 1%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDHRedemption, dec(1, 16)),
      "Fee exceeded provided maximum"
    );
    ...
    // Max fee is 3.754%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDHRedemption, dec(3754, 13)),
      "Fee exceeded provided maximum"
    );
    ...
    // Max fee is 0.5%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDHRedemption, dec(5, 15)),
      "Fee exceeded provided maximum"
    );
  });
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDHAmount: dec(9500, 18),
      extraParams: { from: A }
    });
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(395, 16)),
      extraUSDHAmount: dec(9000, 18),
      extraParams: { from: B }
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(390, 16)),
      extraUSDHAmount: dec(10000, 18),
      extraParams: { from: C }
    });
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt);
    ...
    // USDH redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedUSDHRedemption = expectedTotalSupply.div(toBN(10));

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice();
    const ETHDrawn = attemptedUSDHRedemption.mul(mv._1e18BN).div(price);
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(195, 16)),
      extraUSDHAmount: dec(200, 18),
      extraParams: { from: carol }
    });
    const { totalDebt: D_totalDebt } = await openTrove({
      ICR: toBN(dec(190, 16)),
      extraUSDHAmount: dec(400, 18),
      extraParams: { from: dennis }
    });
    ...
    // Alice opens trove and transfers USDH to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDHAmount: redemptionAmount,
      extraParams: { from: alice }
    });
    await usdhToken.transfer(erin, redemptionAmount, { from: alice });
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedUSDHDeposit(carol)).toString();
    const dennis_SPDeposit_before = (
      await stabilityPool.getCompoundedUSDHDeposit(dennis)
    ).toString();
    ...
    // Check the remaining USDH and ETH in Stability Pool after liquidation is non-zero
    const USDHinSP = await stabilityPool.getTotalUSDHDeposits();
    const ETHinSP = await stabilityPool.getETH();
    assert.isTrue(USDHinSP.gte(mv._zeroBN));
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedUSDHDeposit(carol)).toString();
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedUSDHDeposit(dennis)).toString();
  ...
  it("redeemCollateral(): caller can redeem their entire USDHToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale }
    });

    // Alice opens trove and transfers 400 USDH to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDHAmount: dec(400, 18),
      extraParams: { from: alice }
    });
    await usdhToken.transfer(erin, dec(400, 18), { from: alice });

    // Check Erin's balance before
    const erin_balance_before = await usdhToken.balanceOf(erin);
    assert.equal(erin_balance_before, dec(400, 18));

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDHAmount: dec(590, 18),
      extraParams: { from: bob }
    });
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDHAmount: dec(1990, 18),
      extraParams: { from: carol }
    });
    ...
    // Erin attempts to redeem 400 USDH
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      dec(400, 18),
      price,
      0
    ...
    // Check activePool debt reduced by  400 USDH
    const activePool_debt_after = await activePool.getUSDHDebt();
    ...
    const erin_balance_after = (await usdhToken.balanceOf(erin)).toString();
    assert.equal(erin_balance_after, "0");
  });

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's USDH token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale }
    });
    ...
    // Erin tries to redeem 1000 USDH
    try {
      ...
      assert.include(
        error.message,
        "Requested redemption amount must be <= user's USDH token balance"
      );
    }

    // Erin tries to redeem 401 USDH
    try {
      ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
    ...
    // Erin tries to redeem 239482309 USDH
    try {
      ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
    ...
    // Erin tries to redeem 2^256 - 1 USDH
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed USDH (assuming 1 USDH has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale }
    });

    // Alice opens trove and transfers 1000 USDH each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDHAmount: dec(4990, 18),
      extraParams: { from: alice }
    });
    await usdhToken.transfer(erin, dec(1000, 18), { from: alice });
    await usdhToken.transfer(flyn, dec(1000, 18), { from: alice });
    await usdhToken.transfer(graham, dec(1000, 18), { from: alice });

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDHAmount: dec(1590, 18),
      extraParams: { from: bob }
    });
    const { collateral: C_coll } = await openTrove({
      ICR: toBN(dec(600, 16)),
      extraUSDHAmount: dec(1090, 18),
      extraParams: { from: carol }
    });
    ...
    const _120_USDH = "120000000000000000000";
    const _373_USDH = "373000000000000000000";
    const _950_USDH = "950000000000000000000";
    ...
    /* 120 USDH redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200, 
    ...
    assert.equal(
      ...
      activeETH_0.sub(toBN(_120_USDH).mul(mv._1e18BN).div(price))
    );

    // Flyn redeems 373 USDH
    ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      _373_USDH,
      price,
      0
    ...
    /* 373 USDH redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200, 
    ...
    assert.equal(
      ...
      activeETH_1.sub(toBN(_373_USDH).mul(mv._1e18BN).div(price))
    );

    // Graham redeems 950 USDH
    ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      _950_USDH,
      price,
      0
    ...
    /* 950 USDH redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200, 
    ...
    assert.equal(
      ...
      activeETH_2.sub(toBN(_950_USDH).mul(mv._1e18BN).div(price))
    );
  });
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint USDH to Bob
    await usdhToken.unprotectedMint(bob, dec(100, 18));

    assert.equal(await usdhToken.balanceOf(bob), dec(100, 18));
    ...
    // Bob tries to redeem his illegally obtained USDH
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint USDH to Bob
    await usdhToken.unprotectedMint(bob, "101000000000000000000");

    assert.equal(await usdhToken.balanceOf(bob), "101000000000000000000");
    ...
    th.assertIsApproximatelyEqual((await activePool.getUSDHDebt()).toString(), totalDebt);
    ...
    // Bob attempts to redeem his ill-gotten 101 USDH, from a system that has 100 USDH outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    const A_balanceBefore = await usdhToken.balanceOf(A);

    await th.redeemCollateral(A, contracts, dec(10, 18));

    // Check A's balance has decreased by 10 USDH
    assert.equal(await usdhToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString());
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await usdhToken.balanceOf(A);
    const B_balanceBefore = await usdhToken.balanceOf(B);
    ...
    // Check B's balance has decreased by 10 USDH
    assert.equal(await usdhToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString());
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 USDH
    assert.equal(A_balanceBefore.sub(await usdhToken.balanceOf(A)), dec(10, 18));
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to HALAL staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 HALAL
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
    await halalStaking.stake(dec(1, 18), { from: multisig });
    ...
    // Check HALAL Staking contract balance before is zero
    const halalStakingBalance_Before = await web3.eth.getBalance(halalStaking.address);
    assert.equal(halalStakingBalance_Before, "0");
    ...
    // Check HALAL Staking contract balance after is non-zero
    const halalStakingBalance_After = toBN(await web3.eth.getBalance(halalStaking.address));
    assert.isTrue(halalStakingBalance_After.gt(toBN("0")));
  });

  it("redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-HALAL-staked in HALAL Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 HALAL
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
    await halalStaking.stake(dec(1, 18), { from: multisig });
    ...
    // Check HALAL Staking ETH-fees-per-HALAL-staked before is zero
    const F_ETH_Before = await halalStaking.F_ETH();
    ...
    // Check HALAL Staking ETH-fees-per-HALAL-staked after is non-zero
    const F_ETH_After = await halalStaking.F_ETH();
    assert.isTrue(F_ETH_After.gt("0"));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to HALAL staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 HALAL
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
    await halalStaking.stake(dec(1, 18), { from: multisig });
    ...
    const halalStakingBalance_Before = toBN(await web3.eth.getBalance(halalStaking.address));
    ...
    // check HALAL Staking balance has increased
    assert.isTrue(halalStakingBalance_After.gt(halalStakingBalance_Before));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-HALAL-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 HALAL
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await halalToken.approve(halalStaking.address, dec(1, 18), { from: multisig });
    await halalStaking.stake(dec(1, 18), { from: multisig });
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    // Check total USDH supply
    const activeUSDH = await activePool.getUSDHDebt();
    const defaultUSDH = await defaultPool.getUSDHDebt();

    const totalUSDHSupply = activeUSDH.add(defaultUSDH);
    th.assertIsApproximatelyEqual(totalUSDHSupply, totalDebt);
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    // whale redeems 360 USDH.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    // whale redeems USDH.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount);
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    // whale redeems USDH.  Expect this to fully redeem A, B, C, and partially redeem 15 USDH from D.
  ...
  it("redeemCollateral(): reverts if fee eats up all returned collateral", async () => {
    // --- SETUP ---
    const { usdhAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraUSDHAmount: dec(1, 24),
      extraParams: { from: alice }
    });
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(usdhAmount, price, 0);
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } });
      await borrowerOperations.adjustTrove(th._100pct, 0, usdhAmount, true, alice, alice, {
        from: alice,
        value: usdhAmount.mul(mv._1e18BN).div(price)
      });
    }
  ...
  it("getPendingUSDHDebtReward(): Returns 0 if there is no pending USDHDebt reward", async () => {
    // Make some troves
    ...
    const current_L_USDHDebt = await troveManager.L_USDHDebt();
    assert.equal(current_L_USDHDebt, 0);

    const carolSnapshot_L_USDHDebt = (await troveManager.rewardSnapshots(carol))[1];
    assert.equal(carolSnapshot_L_USDHDebt, 0);

    const carol_PendingUSDHDebtReward = await troveManager.getPendingUSDHDebtReward(carol);
    assert.equal(carol_PendingUSDHDebtReward, 0);
  });
  ...
  it("computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 USDH", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
    const debt = dec(100, 18);
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 USDH", async () => {
    const price = dec(100, 18);
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 USDH", async () => {
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 USDH", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(150, 16)),
      extraUSDHAmount: B_totalDebt,
      extraParams: { from: A }
    });

    // to be able to repay:
    await usdhToken.transfer(B, B_totalDebt, { from: A });
