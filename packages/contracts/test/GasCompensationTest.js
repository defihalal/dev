...
contract("Gas compensation tests", async accounts => {
  ...
  it("_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value", async () => {
    ...
    debt = 10 USDH
    0.5% of coll = 0.04995 ETH. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 USDH*/
    ...
    /* ETH:USD price = 200
     ...
     debt = 0 USDH
     0.5% of coll = 0.000275 ETH. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 USDH*/
    ...
    // debt = 200 USDH
    // 0.5% of coll = 0.004995 ETH. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  USDH */
  ...
  it("getCompositeDebt(): returns (debt + 50) collateral = $10 in value", async () => {
    ...
    debt = 123.45 USDH
    0.5% of coll = 0.5 ETH. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 USDH  */
  ...
  it("getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value", async () => {
    ...
    coll = 100 ETH  
    debt = 2000 USDH
    -> Expect composite debt = (2000 + 200) = 2200 USDH  */
    ...
    coll = 37.5 ETH  
    debt = 500 USDH
    -> Expect composite debt = (500 + 200) = 700 USDH  */
    ...
    coll = 94758.230582309850 ETH  
    debt = 1 billion USDH
    -> Expect composite debt = (1000000000 + 200) = 1000000200 USDH  */
    await priceFeed.setPrice("45323545420000000000000");
    const price_2 = await priceFeed.getPrice();
    ...
    ETH:USD price = 1000000 $/E (1 million)
    coll = 300000000 ETH   (300 million)
    debt = 54321.123456789 USDH
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 USDH */
    await priceFeed.setPrice(dec(1, 24));
    const price_3 = await priceFeed.getPrice();
  ...
  it("getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves", async () => {
    ...
    // E opens with 4405.45 ETH, 32598.35 USDH
  ...
  it("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDHAmount: B_totalDebt.add(C_totalDebt),
      extraParams: { from: erin }
    });

    // D, E each provide USDH to SP
    ...
    const USDHinSP_0 = await stabilityPool.getTotalUSDHDeposits();
    ...
    // Check SP USDH has decreased due to the liquidation
    const USDHinSP_A = await stabilityPool.getTotalUSDHDeposits();
    assert.isTrue(USDHinSP_A.lte(USDHinSP_0));
    ...
    // Check SP USDH has decreased due to the liquidation of B
    const USDHinSP_B = await stabilityPool.getTotalUSDHDeposits();
    assert.isTrue(USDHinSP_B.lt(USDHinSP_A));
    ...
    // Check SP USDH has decreased due to the liquidation of C
    const USDHinSP_C = await stabilityPool.getTotalUSDHDeposits();
    assert.isTrue(USDHinSP_C.lt(USDHinSP_B));
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(60, 18)),
      extraUSDHAmount: dec(600, 18),
      extraParams: { from: carol }
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
      extraUSDHAmount: dec(1, 23),
      extraParams: { from: dennis }
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
    ...
    // D, E each provide 10000 USDH to SP
    ...
    // Check SP USDH has decreased due to the liquidation of A
    const USDHinSP_A = await stabilityPool.getTotalUSDHDeposits();
    assert.isTrue(USDHinSP_A.lt(USDHinSP_0));
  ...
  it("liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    */
    ...
    // Check USDH in SP has decreased
    const USDHinSP_1 = await stabilityPool.getTotalUSDHDeposits();
    assert.isTrue(USDHinSP_1.lt(USDHinSP_0));
  ...
  it("liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    const USDHinDefaultPool_0 = await defaultPool.getUSDHDebt();
    ...
    */
    ...
    // Check USDH in DefaultPool has decreased
    const USDHinDefaultPool_1 = await defaultPool.getUSDHDebt();
    assert.isTrue(USDHinDefaultPool_1.gt(USDHinDefaultPool_0));
  ...
  it("liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 USDH
  ...
  it("Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
    ...
    let coll = 50;
    // create 20 troves, increasing collateral, constant debt = 100USDH
