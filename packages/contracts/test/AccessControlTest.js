...
contract(
  ...
  async accounts => {
    ...
    before(async () => {
      ...
      coreContracts = await deploymentHelper.deployUSDHTokenTester(coreContracts);
      ...
      const bal = await halalToken.balanceOf(communityIssuance.address);
    ...
    describe("ActivePool", async accounts => {
      ...
      // increaseUSDH
      it("increaseUSDHDebt(): reverts when called by an account that is not BO nor TroveM", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.increaseUSDHDebt(100, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      // decreaseUSDH
      it("decreaseUSDHDebt(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
        // Attempt call from alice
        try {
          const txAlice = await activePool.decreaseUSDHDebt(100, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
          assert.include(
            err.message,
    ...
    describe("DefaultPool", async accounts => {
      ...
      // increaseUSDH
      it("increaseUSDHDebt(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.increaseUSDHDebt(100, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
          assert.include(err.message, "Caller is not the TroveManager");
        }
      ...
      // decreaseUSDH
      it("decreaseUSDH(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.decreaseUSDHDebt(100, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
          assert.include(err.message, "Caller is not the TroveManager");
        }
    ...
    describe("USDHToken", async accounts => {
      //    mint
      it("mint(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        const txAlice = usdhToken.mint(bob, 100, { from: alice });
      ...
      it("burn(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
        // Attempt call from alice
        try {
          const txAlice = await usdhToken.burn(bob, 100, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("sendToPool(): reverts when called by an account that is not StabilityPool", async () => {
        // Attempt call from alice
        try {
          const txAlice = await usdhToken.sendToPool(bob, activePool.address, 100, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("returnFromPool(): reverts when called by an account that is not TroveManager nor StabilityPool", async () => {
        // Attempt call from alice
        try {
          const txAlice = await usdhToken.returnFromPool(activePool.address, bob, 100, {
            from: alice
          });
        } catch (err) {
          assert.include(err.message, "revert");
    ...
    describe("LockupContract", async accounts => {
      it("withdrawHALAL(): reverts when caller is not beneficiary", async () => {
        // deploy new LC with Carol as beneficiary
        const unlockTime = (await halalToken.getDeploymentStartTime()).add(
        ...
        // HALAL Multisig funds the LC
        await halalToken.transfer(LC.address, dec(100, 18), { from: multisig });

        // Fast-forward one year, so that beneficiary can withdraw
        await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);

        // Bob attempts to withdraw HALAL
        try {
          const txBob = await LC.withdrawHALAL({ from: bob });
        } catch (err) {
          assert.include(err.message, "revert");
        }

        // Confirm beneficiary, Carol, can withdraw
        const txCarol = await LC.withdrawHALAL({ from: carol });
        assert.isTrue(txCarol.receipt.status);
      });
    });

    describe("HALALStaking", async accounts => {
      it("increaseF_USDH(): reverts when caller is not TroveManager", async () => {
        try {
          const txAlice = await halalStaking.increaseF_USDH(dec(1, 18), { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
        }
      });
    ...
    describe("HALALToken", async accounts => {
      it("sendToHALALStaking(): reverts when caller is not the HALALSstaking", async () => {
        // Check multisig has some HALAL
        assert.isTrue((await halalToken.balanceOf(multisig)).gt(toBN("0")));

        // multisig tries to call it
        try {
          const tx = await halalToken.sendToHALALStaking(multisig, 1, { from: multisig });
        } catch (err) {
          assert.include(err.message, "revert");
        }
        ...
        // Owner transfers 1 HALAL to bob
        await halalToken.transfer(bob, dec(1, 18), { from: multisig });
        assert.equal(await halalToken.balanceOf(bob), dec(1, 18));

        // Bob tries to call it
        try {
          const tx = await halalToken.sendToHALALStaking(bob, dec(1, 18), { from: bob });
        } catch (err) {
          assert.include(err.message, "revert");
        }
      });
    ...
    describe("CommunityIssuance", async accounts => {
      it("sendHALAL(): reverts when caller is not the StabilityPool", async () => {
        const tx1 = communityIssuance.sendHALAL(alice, dec(100, 18), { from: alice });
        const tx2 = communityIssuance.sendHALAL(bob, dec(100, 18), { from: alice });
        const tx3 = communityIssuance.sendHALAL(stabilityPool.address, dec(100, 18), {
          from: alice
        });
      ...
      it("issueHALAL(): reverts when caller is not the StabilityPool", async () => {
        const tx1 = communityIssuance.issueHALAL({ from: alice });
