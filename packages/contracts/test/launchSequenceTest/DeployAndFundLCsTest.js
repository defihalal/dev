...
contract("Deploying and funding One Year Lockup Contracts", async accounts => {
  ...
  describe("Deploying LCs", async accounts => {
    ...
    it("Direct deployment of LC sets the unlockTime in the LC", async () => {
      ...
      const LC_B = await LockupContract.new(halalToken.address, B, "230582305895235", { from: B });
      const LC_C = await LockupContract.new(halalToken.address, C, dec(20, 18), { from: E });
    ...
    it("Direct deployment of LC reverts when the unlockTime is < 1 year from system deployment", async () => {
      ...
      const LCDeploymentPromise_A = LockupContract.new(halalToken.address, A, nearlyOneYear, {
        from: liquityAG
      });
      const LCDeploymentPromise_B = LockupContract.new(halalToken.address, B, "37", { from: B });
      const LCDeploymentPromise_C = LockupContract.new(halalToken.address, C, "43200", { from: E });
  ...
  describe("Funding LCs", async accounts => {
    it("HALAL transfer from HALAL deployer to their deployed LC increases the HALAL balance of the LC", async () => {
      // Deploy 5 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(
        A,
        oneYearFromSystemDeployment,
      ...
      assert.equal(await halalToken.balanceOf(LCAddress_D), "0");
      assert.equal(await halalToken.balanceOf(LCAddress_E), "0");

      // Multisig transfers HALAL to each LC
      await halalToken.transfer(LCAddress_A, HALALEntitlement_A, { from: multisig });
      await halalToken.transfer(LCAddress_B, HALALEntitlement_B, { from: multisig });
      await halalToken.transfer(LCAddress_C, HALALEntitlement_C, { from: multisig });
      await halalToken.transfer(LCAddress_D, HALALEntitlement_D, { from: multisig });
      await halalToken.transfer(LCAddress_E, HALALEntitlement_E, { from: multisig });

      assert.equal(await halalToken.balanceOf(LCAddress_A), HALALEntitlement_A);
      assert.equal(await halalToken.balanceOf(LCAddress_B), HALALEntitlement_B);
      assert.equal(await halalToken.balanceOf(LCAddress_C), HALALEntitlement_C);
      assert.equal(await halalToken.balanceOf(LCAddress_D), HALALEntitlement_D);
      assert.equal(await halalToken.balanceOf(LCAddress_E), HALALEntitlement_E);
    });

    it("HALAL Multisig can transfer HALAL to LCs deployed through the factory by anyone", async () => {
      ...
      // Multisig transfers HALAL to each LC
      await halalToken.transfer(LCAddress_A, dec(1, 18), { from: multisig });
      await halalToken.transfer(LCAddress_B, dec(2, 18), { from: multisig });
      await halalToken.transfer(LCAddress_C, dec(3, 18), { from: multisig });
      await halalToken.transfer(LCAddress_D, dec(4, 18), { from: multisig });
      await halalToken.transfer(LCAddress_E, dec(5, 18), { from: multisig });

      assert.equal(await halalToken.balanceOf(LCAddress_A), dec(1, 18));
      assert.equal(await halalToken.balanceOf(LCAddress_B), dec(2, 18));
      assert.equal(await halalToken.balanceOf(LCAddress_C), dec(3, 18));
      assert.equal(await halalToken.balanceOf(LCAddress_D), dec(4, 18));
      assert.equal(await halalToken.balanceOf(LCAddress_E), dec(5, 18));
    });

    // can't transfer HALAL to any LCs that were deployed directly
  });
  ...
  describe("Withdrawal attempts on funded, inactive LCs immediately after funding", async accounts => {
    it("Beneficiary can't withdraw from their funded LC", async () => {
      ...
      // Multisig transfers HALAL to each LC
      await halalToken.transfer(LC_A.address, HALALEntitlement_A, { from: multisig });
      await halalToken.transfer(LC_B.address, HALALEntitlement_B, { from: multisig });
      await halalToken.transfer(LC_C.address, HALALEntitlement_C, { from: multisig });

      assert.equal(await halalToken.balanceOf(LC_A.address), HALALEntitlement_A);
      assert.equal(await halalToken.balanceOf(LC_B.address), HALALEntitlement_B);
      assert.equal(await halalToken.balanceOf(LC_C.address), HALALEntitlement_C);
      ...
      for (LC of LCs) {
        try {
          ...
          const withdrawalAttemptTx = await LC.withdrawHALAL({ from: beneficiary });
    ...
    it("HALAL multisig can't withraw from a LC which it funded", async () => {
      // Deploy 3 LCs
      const deployedLCtx_A = await lockupContractFactory.deployLockupContract(
        A,
        oneYearFromSystemDeployment,
      ...
      // HALAL multisig attempts to withdraw from LCs
      for (LC of LCs) {
        try {
          const withdrawalAttemptTx = await LC.withdrawHALAL({ from: multisig });
          assert.isFalse(withdrawalAttemptTx.receipt.status);
        } catch (error) {
          assert.include(error.message, "revert");
        }
    ...
    it("No one can withraw from a LC", async () => {
      ...
      // LiquityAG transfers HALAL to the LC
      await halalToken.transfer(LC_A.address, HALALEntitlement_A, { from: multisig });
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawHALAL({ from: G });
        assert.isFalse(withdrawalAttemptTx.receipt.status);
      } catch (error) {
        assert.include(error.message, "revert");
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawHALAL({ from: H });
        assert.isFalse(withdrawalAttemptTx.receipt.status);
      } catch (error) {
        assert.include(error.message, "revert");
      }
      ...
      try {
        const withdrawalAttemptTx = await LC_A.withdrawHALAL({ from: I });
        assert.isFalse(withdrawalAttemptTx.receipt.status);
      } catch (error) {
        assert.include(error.message, "revert");
      }
