...
contract("StabilityPool", async accounts => {
  ...
  */
  ...
  it("getTotalUSDHDeposits(): gets the recorded USDH balance", async () => {
    const recordedETHBalance = await stabilityPool.getTotalUSDHDeposits();
    assert.equal(recordedETHBalance, 0);
  });
});
...
contract("ActivePool", async accounts => {
  ...
  it("getUSDHDebt(): gets the recorded USDH balance", async () => {
    const recordedETHBalance = await activePool.getUSDHDebt();
    assert.equal(recordedETHBalance, 0);
  });

  it("increaseUSDH(): increases the recorded USDH balance by the correct amount", async () => {
    const recordedUSDH_balanceBefore = await activePool.getUSDHDebt();
    assert.equal(recordedUSDH_balanceBefore, 0);

    // await activePool.increaseUSDHDebt(100, { from: mockBorrowerOperationsAddress })
    const increaseUSDHDebtData = th.getTransactionData("increaseUSDHDebt(uint256)", ["0x64"]);
    const tx = await mockBorrowerOperations.forward(activePool.address, increaseUSDHDebtData);
    assert.isTrue(tx.receipt.status);
    const recordedUSDH_balanceAfter = await activePool.getUSDHDebt();
    assert.equal(recordedUSDH_balanceAfter, 100);
  });
  // Decrease
  it("decreaseUSDH(): decreases the recorded USDH balance by the correct amount", async () => {
    // start the pool on 100 wei
    //await activePool.increaseUSDHDebt(100, { from: mockBorrowerOperationsAddress })
    const increaseUSDHDebtData = th.getTransactionData("increaseUSDHDebt(uint256)", ["0x64"]);
    const tx1 = await mockBorrowerOperations.forward(activePool.address, increaseUSDHDebtData);
    assert.isTrue(tx1.receipt.status);

    const recordedUSDH_balanceBefore = await activePool.getUSDHDebt();
    assert.equal(recordedUSDH_balanceBefore, 100);

    //await activePool.decreaseUSDHDebt(100, { from: mockBorrowerOperationsAddress })
    const decreaseUSDHDebtData = th.getTransactionData("decreaseUSDHDebt(uint256)", ["0x64"]);
    const tx2 = await mockBorrowerOperations.forward(activePool.address, decreaseUSDHDebtData);
    assert.isTrue(tx2.receipt.status);
    const recordedUSDH_balanceAfter = await activePool.getUSDHDebt();
    assert.equal(recordedUSDH_balanceAfter, 0);
  });
...
contract("DefaultPool", async accounts => {
  ...
  it("getETH(): gets the recorded USDH balance", async () => {
    const recordedETHBalance = await defaultPool.getETH();
    assert.equal(recordedETHBalance, 0);
  });

  it("getUSDHDebt(): gets the recorded USDH balance", async () => {
    const recordedETHBalance = await defaultPool.getUSDHDebt();
    assert.equal(recordedETHBalance, 0);
  });

  it("increaseUSDH(): increases the recorded USDH balance by the correct amount", async () => {
    const recordedUSDH_balanceBefore = await defaultPool.getUSDHDebt();
    assert.equal(recordedUSDH_balanceBefore, 0);

    // await defaultPool.increaseUSDHDebt(100, { from: mockTroveManagerAddress })
    const increaseUSDHDebtData = th.getTransactionData("increaseUSDHDebt(uint256)", ["0x64"]);
    const tx = await mockTroveManager.forward(defaultPool.address, increaseUSDHDebtData);
    assert.isTrue(tx.receipt.status);

    const recordedUSDH_balanceAfter = await defaultPool.getUSDHDebt();
    assert.equal(recordedUSDH_balanceAfter, 100);
  });
  ...
  it("decreaseUSDH(): decreases the recorded USDH balance by the correct amount", async () => {
    ...
    //await defaultPool.increaseUSDHDebt(100, { from: mockTroveManagerAddress })
    const increaseUSDHDebtData = th.getTransactionData("increaseUSDHDebt(uint256)", ["0x64"]);
    const tx1 = await mockTroveManager.forward(defaultPool.address, increaseUSDHDebtData);
    assert.isTrue(tx1.receipt.status);

    const recordedUSDH_balanceBefore = await defaultPool.getUSDHDebt();
    assert.equal(recordedUSDH_balanceBefore, 100);

    // await defaultPool.decreaseUSDHDebt(100, { from: mockTroveManagerAddress })
    const decreaseUSDHDebtData = th.getTransactionData("decreaseUSDHDebt(uint256)", ["0x64"]);
    const tx2 = await mockTroveManager.forward(defaultPool.address, decreaseUSDHDebtData);
    assert.isTrue(tx2.receipt.status);
