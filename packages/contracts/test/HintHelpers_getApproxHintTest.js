...
contract("HintHelpers", async accounts => {
  ...
  /* Open a Trove for each account. USDH debt is 200 USDH each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

  // Open Troves in parallel, then withdraw USDH in parallel
  const makeTrovesInParallel = async (accounts, n) => {
    ...
    const withdrawUSDHpromises = activeAccounts.map(account => withdrawUSDHfromTrove(account));
    await Promise.all(withdrawUSDHpromises);
  ...
  const withdrawUSDHfromTrove = async account => {
    await borrowerOperations.withdrawUSDH(th._100pct, "100000000000000000000", account, account, {
      from: account
    });
  };

  // Sequentially add coll and withdraw USDH, 1 account at a time
