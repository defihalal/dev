...
contract("StabilityPool", async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    // --- provideToSP() ---
    // increases recorded USDH at Stability Pool
    it("provideToSP(): increases the Stability Pool USDH balance", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({
        extraUSDHAmount: toBN(200),
        ICR: toBN(dec(2, 18)),
      ...
      // check USDH balances after
      const stabilityPool_USDH_After = await stabilityPool.getTotalUSDHDeposits();
      assert.equal(stabilityPool_USDH_After, 200);
    });
    ...
    it("provideToSP(): reduces the user's USDH balance by the correct amount", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({
        extraUSDHAmount: toBN(200),
        ICR: toBN(dec(2, 18)),
      ...
      // check user's USDH balance change
      const alice_USDHBalance_After = await usdhToken.balanceOf(alice);
      assert.equal(alice_USDHBalance_Before.sub(alice_USDHBalance_After), "200");
    });

    it("provideToSP(): increases totalUSDHDeposits by correct amount", async () => {
      // --- SETUP ---

      // Whale opens Trove with 50 ETH, adds 2000 USDH to StabilityPool
      await openTrove({
        extraUSDHAmount: toBN(dec(2000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale }
      });
      await stabilityPool.provideToSP(dec(2000, 18), frontEnd_1, { from: whale });

      const totalUSDHDeposits = await stabilityPool.getTotalUSDHDeposits();
      assert.equal(totalUSDHDeposits, dec(2000, 18));
    });
    ...
    it("provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked", async () => {
      ...
      const whaleUSDH = await usdhToken.balanceOf(whale);
      await stabilityPool.provideToSP(whaleUSDH, frontEnd_1, { from: whale });
      ...
      // Alice makes Trove and withdraws 100 USDH
      await openTrove({
      ...
      const SPUSDH_Before = await stabilityPool.getTotalUSDHDeposits();
      ...
      const SPUSDH_After = await stabilityPool.getTotalUSDHDeposits();
      assert.isTrue(SPUSDH_After.lt(SPUSDH_Before));
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      // 3 Troves opened. Two users withdraw 160 USDH each
      await openTrove({
        extraUSDHAmount: 0,
        ICR: toBN(dec(2, 18)),
      ...
      // Alice makes deposit #1: 150 USDH
      await openTrove({
        extraUSDHAmount: toBN(dec(250, 18)),
        ICR: toBN(dec(3, 18)),
        extraParams: { from: alice }
      });
      ...
      // 2 users with Trove with 180 USDH drawn are closed
      await troveManager.liquidate(defaulter_1, { from: owner }); // 180 USDH closed
      await troveManager.liquidate(defaulter_2, { from: owner }); // 180 USDH closed

      const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedUSDHDeposit(alice);
      ...
      // Bob withdraws USDH and deposits to StabilityPool
      await openTrove({
        extraUSDHAmount: toBN(dec(1000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedUSDHDeposit(alice);
    ...
    it("provideToSP(): reverts if user tries to provide more than their USDH balance", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") }
      ...
      const aliceUSDHbal = await usdhToken.balanceOf(alice);
      const bobUSDHbal = await usdhToken.balanceOf(bob);

      // Alice, attempts to deposit 1 wei more than her balance

      const aliceTxPromise = stabilityPool.provideToSP(aliceUSDHbal.add(toBN(1)), frontEnd_1, {
        from: alice
      });
      ...
      const bobTxPromise = stabilityPool.provideToSP(
        bobUSDHbal.add(toBN(dec(235534, 18))),
        frontEnd_1,
        { from: bob }
      );
    ...
    it("provideToSP(): reverts if user tries to provide 2^256-1 USDH, which exceeds their balance", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") }
      ...
      // Alice attempts to deposit 2^256-1 USDH
      try {
    ...
    it("provideToSP(): reverts if cannot receive ETH Gain", async () => {
      ...
      await usdhToken.transfer(nonPayable.address, dec(250, 18), { from: whale });

      // NonPayable makes deposit #1: 150 USDH
      ...
      // NonPayable tries to make deposit #2: 100USDH (which also attempts to withdraw ETH gain)
    ...
    it("provideToSP(): doesn't impact other users' deposits or ETH gains", async () => {
      ...
      const alice_USDHDeposit_Before = (
        await stabilityPool.getCompoundedUSDHDeposit(alice)
      ).toString();
      const bob_USDHDeposit_Before = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
      const carol_USDHDeposit_Before = (
        await stabilityPool.getCompoundedUSDHDeposit(carol)
      ).toString();
      ...
      //check non-zero USDH and ETHGain in the Stability Pool
      const USDHinSP = await stabilityPool.getTotalUSDHDeposits();
      const ETHinSP = await stabilityPool.getETH();
      assert.isTrue(USDHinSP.gt(mv._zeroBN));
      assert.isTrue(ETHinSP.gt(mv._zeroBN));

      // D makes an SP deposit
      await stabilityPool.provideToSP(dec(1000, 18), frontEnd_1, { from: dennis });
      assert.equal((await stabilityPool.getCompoundedUSDHDeposit(dennis)).toString(), dec(1000, 18));

      const alice_USDHDeposit_After = (
        await stabilityPool.getCompoundedUSDHDeposit(alice)
      ).toString();
      const bob_USDHDeposit_After = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
      const carol_USDHDeposit_After = (
        await stabilityPool.getCompoundedUSDHDeposit(carol)
      ).toString();
      ...
      assert.equal(alice_USDHDeposit_Before, alice_USDHDeposit_After);
      assert.equal(bob_USDHDeposit_Before, bob_USDHDeposit_After);
      assert.equal(carol_USDHDeposit_Before, carol_USDHDeposit_After);
    ...
    it("provideToSP(): doesn't impact system debt, collateral or TCR", async () => {
      ...
      const activeDebt_Before = (await activePool.getUSDHDebt()).toString();
      const defaultedDebt_Before = (await defaultPool.getUSDHDebt()).toString();
      ...
      const activeDebt_After = (await activePool.getUSDHDebt()).toString();
      const defaultedDebt_After = (await defaultPool.getUSDHDebt()).toString();
    ...
    it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
      ...
      assert.equal((await stabilityPool.getCompoundedUSDHDeposit(bob)).toString(), dec(1000, 18));
    ...
    it("provideToSP(): providing 0 USDH reverts", async () => {
      // --- SETUP ---
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // A, B, C provides 100, 50, 30 USDH to SP
      await stabilityPool.provideToSP(dec(100, 18), frontEnd_1, { from: alice });
      ...
      const USDHinSP_Before = (await stabilityPool.getTotalUSDHDeposits()).toString();

      assert.equal(USDHinSP_Before, dec(180, 18));

      // Bob provides 0 USDH to the Stability Pool
      const txPromise_B = stabilityPool.provideToSP(0, frontEnd_1, { from: bob });
      await th.assertRevert(txPromise_B);
    });

    // --- HALAL functionality ---
    it("provideToSP(), new deposit: when SP > 0, triggers HALAL reward event - increases the sum G", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") }
    ...
    it("provideToSP(), new deposit: depositor does not receive any HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") }
      ...
      // Get A, B, C HALAL balances before and confirm they're zero
      const A_HALALBalance_Before = await halalToken.balanceOf(A);
      const B_HALALBalance_Before = await halalToken.balanceOf(B);

      assert.equal(A_HALALBalance_Before, "0");
      assert.equal(B_HALALBalance_Before, "0");
      ...
      // Get A, B, C HALAL balances after, and confirm they're still zero
      const A_HALALBalance_After = await halalToken.balanceOf(A);
      const B_HALALBalance_After = await halalToken.balanceOf(B);

      assert.equal(A_HALALBalance_After, "0");
      assert.equal(B_HALALBalance_After, "0");
    });

    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive any HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(4000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: D }
      });
      ...
      const initialDeposit_A = await usdhToken.balanceOf(A);
      const initialDeposit_B = await usdhToken.balanceOf(B);
      // A, B provide to SP
      ...
      // Get A, B, C HALAL balances before and confirm they're non-zero
      const A_HALALBalance_Before = await halalToken.balanceOf(A);
      const B_HALALBalance_Before = await halalToken.balanceOf(B);
      assert.isTrue(A_HALALBalance_Before.gt(toBN("0")));
      assert.isTrue(B_HALALBalance_Before.gt(toBN("0")));
      ...
      // Get A, B, C HALAL balances after, and confirm they have not changed
      const A_HALALBalance_After = await halalToken.balanceOf(A);
      const B_HALALBalance_After = await halalToken.balanceOf(B);

      assert.isTrue(A_HALALBalance_After.eq(A_HALALBalance_Before));
      assert.isTrue(B_HALALBalance_After.eq(B_HALALBalance_Before));
    });

    it("provideToSP(), new eligible deposit: tagged front end receives HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get F1, F2, F3 HALAL balances before, and confirm they're zero
      const frontEnd_1_HALALBalance_Before = await halalToken.balanceOf(frontEnd_1);
      const frontEnd_2_HALALBalance_Before = await halalToken.balanceOf(frontEnd_2);
      const frontEnd_3_HALALBalance_Before = await halalToken.balanceOf(frontEnd_3);

      assert.equal(frontEnd_1_HALALBalance_Before, "0");
      assert.equal(frontEnd_2_HALALBalance_Before, "0");
      assert.equal(frontEnd_3_HALALBalance_Before, "0");

      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);

      // console.log(`HALALSupplyCap before: ${await communityIssuance.HALALSupplyCap()}`)
      // console.log(`totalHALALIssued before: ${await communityIssuance.totalHALALIssued()}`)
      // console.log(`HALAL balance of CI before: ${await halalToken.balanceOf(communityIssuance.address)}`)
      ...
      // console.log(`HALALSupplyCap after: ${await communityIssuance.HALALSupplyCap()}`)
      // console.log(`totalHALALIssued after: ${await communityIssuance.totalHALALIssued()}`)
      // console.log(`HALAL balance of CI after: ${await halalToken.balanceOf(communityIssuance.address)}`)

      // Get F1, F2, F3 HALAL balances after, and confirm they have increased
      const frontEnd_1_HALALBalance_After = await halalToken.balanceOf(frontEnd_1);
      const frontEnd_2_HALALBalance_After = await halalToken.balanceOf(frontEnd_2);
      const frontEnd_3_HALALBalance_After = await halalToken.balanceOf(frontEnd_3);

      assert.isTrue(frontEnd_1_HALALBalance_After.gt(frontEnd_1_HALALBalance_Before));
      assert.isTrue(frontEnd_2_HALALBalance_After.gt(frontEnd_2_HALALBalance_Before));
      assert.isTrue(frontEnd_3_HALALBalance_After.gt(frontEnd_3_HALALBalance_Before));
    });
    ...
    it("provideToSP(), new deposit: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers USDH to A, B
      await usdhToken.transfer(A, dec(100, 18), { from: whale });
      await usdhToken.transfer(B, dec(200, 18), { from: whale });
    ...
    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers USDH to A, B
      await usdhToken.transfer(A, dec(1000, 18), { from: whale });
      await usdhToken.transfer(B, dec(1000, 18), { from: whale });
    ...
    it("provideToSP(), topup: triggers HALAL reward event - increases the sum G", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Expect G has increased from the HALAL reward event triggered by B's topup
      assert.isTrue(G_After.gt(G_Before));
    });
    ...
    it("provideToSP(), topup from different front end: doesn't change the front end tag", async () => {
      ...
      await usdhToken.transfer(D, dec(100, 18), { from: whale });
      await usdhToken.transfer(E, dec(200, 18), { from: whale });
    ...
    it("provideToSP(), topup: depositor receives HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get A, B, C HALAL balance before
      const A_HALALBalance_Before = await halalToken.balanceOf(A);
      const B_HALALBalance_Before = await halalToken.balanceOf(B);
      const C_HALALBalance_Before = await halalToken.balanceOf(C);
      ...
      // Get HALAL balance after
      const A_HALALBalance_After = await halalToken.balanceOf(A);
      const B_HALALBalance_After = await halalToken.balanceOf(B);
      const C_HALALBalance_After = await halalToken.balanceOf(C);

      // Check HALAL Balance of A, B, C has increased
      assert.isTrue(A_HALALBalance_After.gt(A_HALALBalance_Before));
      assert.isTrue(B_HALALBalance_After.gt(B_HALALBalance_Before));
      assert.isTrue(C_HALALBalance_After.gt(C_HALALBalance_Before));
    });

    it("provideToSP(), topup: tagged front end receives HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get front ends' HALAL balance before
      const F1_HALALBalance_Before = await halalToken.balanceOf(frontEnd_1);
      const F2_HALALBalance_Before = await halalToken.balanceOf(frontEnd_2);
      const F3_HALALBalance_Before = await halalToken.balanceOf(frontEnd_3);
      ...
      // Get front ends' HALAL balance after
      const F1_HALALBalance_After = await halalToken.balanceOf(A);
      const F2_HALALBalance_After = await halalToken.balanceOf(B);
      const F3_HALALBalance_After = await halalToken.balanceOf(C);

      // Check HALAL Balance of front ends has increased
      assert.isTrue(F1_HALALBalance_After.gt(F1_HALALBalance_Before));
      assert.isTrue(F2_HALALBalance_After.gt(F2_HALALBalance_Before));
      assert.isTrue(F3_HALALBalance_After.gt(F3_HALALBalance_Before));
    });
    ...
    it("provideToSP(), topup: tagged front end's snapshots update", async () => {
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(400, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: B }
      });
      await openTrove({
        extraUSDHAmount: toBN(dec(600, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
      ...
      await stabilityPool.provideToSP(await usdhToken.balanceOf(D), ZERO_ADDRESS, { from: D });
      ...
      // between topups, because some block.timestamp time passes (and HALAL is issued) between ops
      const G1 = await stabilityPool.epochToScaleToG(currentScale, currentEpoch);
      await stabilityPool.provideToSP(deposit_A, frontEnd_1, { from: A });
    ...
    it("provideToSP(): reverts when amount is zero", async () => {
      ...
      // Whale transfers USDH to C, D
      await usdhToken.transfer(C, dec(100, 18), { from: whale });
      await usdhToken.transfer(D, dec(100, 18), { from: whale });
    ...
    it("withdrawFromSP(): partial retrieval - retrieves correct USDH amount and the entire ETH Gain, and updates deposit", async () => {
      // --- SETUP ---
      // Whale deposits 185000 USDH in StabilityPool
      await openTrove({
        extraUSDHAmount: toBN(dec(1, 24)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      });
      ...
      // Alice makes deposit #1: 15000 USDH
      await openTrove({
      ...
      // 2 users with Trove with 170 USDH drawn are closed
      const liquidationTX_1 = await troveManager.liquidate(defaulter_1, { from: owner }); // 170 USDH closed
      const liquidationTX_2 = await troveManager.liquidate(defaulter_2, { from: owner }); // 170 USDH closed

      const [liquidatedDebt_1] = await th.getEmittedLiquidationValues(liquidationTX_1);
      const [liquidatedDebt_2] = await th.getEmittedLiquidationValues(liquidationTX_2);

      // Alice USDHLoss is ((15000/200000) * liquidatedDebt), for each liquidation
      const expectedUSDHLoss_A = liquidatedDebt_1
        .mul(toBN(dec(15000, 18)))
        .div(toBN(dec(200000, 18)))
        .add(liquidatedDebt_2.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))));

      const expectedCompoundedUSDHDeposit_A = toBN(dec(15000, 18)).sub(expectedUSDHLoss_A);
      ...
      // Alice retrieves part of her entitled USDH: 9000 USDH
      await stabilityPool.withdrawFromSP(dec(9000, 18), { from: alice });

      const expectedNewDeposit_A = compoundedUSDHDeposit_A.sub(toBN(dec(9000, 18)));
    ...
    it("withdrawFromSP(): partial retrieval - leaves the correct amount of USDH in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 USDH in StabilityPool
      await openTrove({
        extraUSDHAmount: toBN(dec(1, 24)),
      ...
      const SP_USDH_Before = await stabilityPool.getTotalUSDHDeposits();
      assert.equal(SP_USDH_Before, dec(200000, 18));
      ...
      Expect USDH in SP = (200000 - liquidatedDebt_1 - liquidatedDebt_2 - 9000) */
      const expectedSPUSDH = toBN(dec(200000, 18))
        .sub(toBN(liquidatedDebt_1))
        .sub(toBN(liquidatedDebt_2))
        .sub(toBN(dec(9000, 18)));

      const SP_USDH_After = (await stabilityPool.getTotalUSDHDeposits()).toString();

      th.assertIsApproximatelyEqual(SP_USDH_After, expectedSPUSDH);
    });

    it("withdrawFromSP(): full retrieval - leaves the correct amount of USDH in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 USDH in StabilityPool
      await openTrove({
      ...
      const USDHinSPBefore = await stabilityPool.getTotalUSDHDeposits();

      // Alice retrieves all of her entitled USDH:
      await stabilityPool.withdrawFromSP(dec(15000, 18), { from: alice });

      const expectedUSDHinSPAfter = USDHinSPBefore.sub(compoundedUSDHDeposit_A);

      const USDHinSPAfter = await stabilityPool.getTotalUSDHDeposits();
      assert.isAtMost(th.getDifference(expectedUSDHinSPAfter, USDHinSPAfter), 100000);
    });
    ...
    it("withdrawFromSP(): it correctly updates the user's USDH and ETH snapshots of entitled reward per unit staked", async () => {
      // --- SETUP ---
      // Whale deposits 185000 USDH in StabilityPool
      await openTrove({
        extraUSDHAmount: toBN(dec(1000000, 18)),
    ...
    it("withdrawFromSP(): decreases StabilityPool ETH", async () => {
      ...
      const liquidationTx_1 = await troveManager.liquidate(defaulter_1, { from: owner }); // 180 USDH closed
    ...
    it("withdrawFromSP(): All depositors are able to withdraw from the SP to their account", async () => {
      ...
      const totalDeposits = (await stabilityPool.getTotalUSDHDeposits()).toString();

      assert.isAtMost(th.getDifference(totalDeposits, "0"), 100000);
    });

    it("withdrawFromSP(): increases depositor's USDH token balance by the expected amount", async () => {
      // Whale opens trove
      await openTrove({
      ...
      const aliceBalBefore = await usdhToken.balanceOf(alice);
      const bobBalBefore = await usdhToken.balanceOf(bob);

      /* From an offset of 10000 USDH, each depositor receives
      USDHLoss = 1666.6666666666666666 USDH

      and thus with a deposit of 10000 USDH, each should withdraw 8333.3333333333333333 USDH (in practice, slightly less due to rounding error)
      */

      // Price bounces back to $200 per ETH
      await priceFeed.setPrice(dec(200, 18));

      // Bob issues a further 5000 USDH from his trove
      await borrowerOperations.withdrawUSDH(th._100pct, dec(5000, 18), bob, bob, { from: bob });

      // Expect Alice's USDH balance increase be very close to 8333.3333333333333333 USDH
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice });
      const aliceBalance = await usdhToken.balanceOf(alice);
      ...
      // expect Bob's USDH balance increase to be very close to  13333.33333333333333333 USDH
      await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob });
      const bobBalance = await usdhToken.balanceOf(bob);
      assert.isAtMost(
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const A_HALALBalBefore = await halalToken.balanceOf(A);

      // Check Alice has gains to withdraw
      const A_pendingETHGain = await stabilityPool.getDepositorETHGain(A);
      const A_pendingHALALGain = await stabilityPool.getDepositorHALALGain(A);
      assert.isTrue(A_pendingETHGain.gt(toBN("0")));
      assert.isTrue(A_pendingHALALGain.gt(toBN("0")));
      ...
      const A_HALALBalAfter = await halalToken.balanceOf(A);
      const A_HALALBalDiff = A_HALALBalAfter.sub(A_HALALBalBefore);

      // Check A's ETH and HALAL balances have increased correctly
      assert.isTrue(A_ETHBalAfter.sub(A_ETHBalBefore).eq(A_pendingETHGain));
      assert.isAtMost(th.getDifference(A_HALALBalDiff, A_pendingHALALGain), 1000);
    });

    it("withdrawFromSP(): withdrawing 0 USDH doesn't alter the caller's deposit or the total USDH in the Stability Pool", async () => {
      // --- SETUP ---
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      // Bob withdraws 0 USDH from the Stability Pool
      await stabilityPool.withdrawFromSP(0, { from: bob });

      // check Bob's deposit and total USDH in Stability Pool has not changed
      const bob_Deposit_After = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
      const USDHinSP_After = (await stabilityPool.getTotalUSDHDeposits()).toString();

      assert.equal(bob_Deposit_Before, bob_Deposit_After);
      assert.equal(USDHinSP_Before, USDHinSP_After);
    });
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      const alice_USDH_Balance_Before = await usdhToken.balanceOf(alice);
      const bob_USDH_Balance_Before = await usdhToken.balanceOf(bob);

      const alice_Deposit_Before = await stabilityPool.getCompoundedUSDHDeposit(alice);
      const bob_Deposit_Before = await stabilityPool.getCompoundedUSDHDeposit(bob);

      const USDHinSP_Before = await stabilityPool.getTotalUSDHDeposits();
      ...
      // Check Bob's USDH balance has risen by only the value of his compounded deposit
      const bob_expectedUSDHBalance = bob_USDH_Balance_Before.add(bob_Deposit_Before).toString();
      const bob_USDH_Balance_After = (await usdhToken.balanceOf(bob)).toString();
      assert.equal(bob_USDH_Balance_After, bob_expectedUSDHBalance);

      // Alice attempts to withdraws 2309842309.000000000000000000 USDH from the Stability Pool
      await stabilityPool.withdrawFromSP("2309842309000000000000000000", { from: alice });

      // Check Alice's USDH balance has risen by only the value of her compounded deposit
      const alice_expectedUSDHBalance = alice_USDH_Balance_Before
        .add(alice_Deposit_Before)
        .toString();
      const alice_USDH_Balance_After = (await usdhToken.balanceOf(alice)).toString();
      assert.equal(alice_USDH_Balance_After, alice_expectedUSDHBalance);

      // Check USDH in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
      const expectedUSDHinSP = USDHinSP_Before.sub(alice_Deposit_Before)
        .sub(bob_Deposit_Before)
        .toString();
      const USDHinSP_After = (await stabilityPool.getTotalUSDHDeposits()).toString();
      assert.equal(USDHinSP_After, expectedUSDHinSP);
    });

    it("withdrawFromSP(): Request to withdraw 2^256-1 USDH only withdraws the caller's compounded deposit", async () => {
      // --- SETUP ---
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      // Bob attempts to withdraws maxBytes32 USDH from the Stability Pool
      ...
      // Check USDH in Stability Pool has been reduced by only  Bob's compounded deposit
      const expectedUSDHinSP = USDHinSP_Before.sub(bob_Deposit_Before).toString();
      const USDHinSP_After = (await stabilityPool.getTotalUSDHDeposits()).toString();
      assert.equal(USDHinSP_After, expectedUSDHinSP);
    });
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode", async () => {
      ...
      // A, B, C provides 10000, 5000, 3000 USDH to SP
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: alice });
      ...
      const alice_USDH_Balance_Before = await usdhToken.balanceOf(alice);
      const bob_USDH_Balance_Before = await usdhToken.balanceOf(bob);
      const carol_USDH_Balance_Before = await usdhToken.balanceOf(carol);
      ...
      const alice_Deposit_Before = await stabilityPool.getCompoundedUSDHDeposit(alice);
      const bob_Deposit_Before = await stabilityPool.getCompoundedUSDHDeposit(bob);
      const carol_Deposit_Before = await stabilityPool.getCompoundedUSDHDeposit(carol);
      ...
      // Check USDH balances of A, B, C have risen by the value of their compounded deposits, respectively
      const alice_expectedUSDHBalance = alice_USDH_Balance_Before
        .add(alice_Deposit_Before)
        .toString();

      const bob_expectedUSDHBalance = bob_USDH_Balance_Before.add(bob_Deposit_Before).toString();
      const carol_expectedUSDHBalance = carol_USDH_Balance_Before
      ...
      const bob_USDH_Balance_After = (await usdhToken.balanceOf(bob)).toString();
      const carol_USDH_Balance_After = (await usdhToken.balanceOf(carol)).toString();

      assert.equal(alice_USDH_Balance_After, alice_expectedUSDHBalance);
      assert.equal(bob_USDH_Balance_After, bob_expectedUSDHBalance);
      assert.equal(carol_USDH_Balance_After, carol_expectedUSDHBalance);
      ...
      // Check USDH in Stability Pool has been reduced by A, B and C's compounded deposit
      const expectedUSDHinSP = USDHinSP_Before.sub(alice_Deposit_Before)
        .sub(bob_Deposit_Before)
    ...
    it("getDepositorETHGain(): depositor does not earn further ETH gains from liquidations while their compounded deposit == 0: ", async () => {
      ...
      // A, B, provide 10000, 5000 USDH to SP
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: alice });
      await stabilityPool.provideToSP(dec(5000, 18), frontEnd_1, { from: bob });
      ...
      assert.equal(USDHinSP, "0");

      // Check Stability deposits have been fully cancelled with debt, and are now all zero
      const alice_Deposit = (await stabilityPool.getCompoundedUSDHDeposit(alice)).toString();
      const bob_Deposit = (await stabilityPool.getCompoundedUSDHDeposit(bob)).toString();
      ...
      // Whale deposits 10000 USDH to Stability Pool
    ...
    // --- HALAL functionality ---
    it("withdrawFromSP(): triggers HALAL reward event - increases the sum G", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(1, 24)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawFromSP(), partial withdrawal: depositor receives HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end receives HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's snapshots update", async () => {
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(60000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C }
      });
    ...
    it("withdrawFromSP(), full withdrawal: removes deposit's front end tag", async () => {
      ...
      await usdhToken.transfer(A, dec(10000, 18), { from: whale });
      await usdhToken.transfer(B, dec(20000, 18), { from: whale });
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      // Fast-forward time and make a second deposit, to trigger HALAL reward and make G > 0
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E });
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      await openTrove({
        extraUSDHAmount: toBN(dec(10100, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A }
      });
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: A });
      ...
      //  SETUP: Execute a series of operations to trigger HALAL and ETH rewards for depositor A
    ...
    it("withdrawETHGainToTrove(): Applies USDHLoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
      // --- SETUP ---
      // Whale deposits 185000 USDH in StabilityPool
      await openTrove({
        extraUSDHAmount: toBN(dec(1000000, 18)),
      ...
      const compoundedDeposit_A = await stabilityPool.getCompoundedUSDHDeposit(alice);
      ...
      const expectedUSDHLoss_A = liquidatedDebt.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)));
      const expectedCompoundedDeposit_A = toBN(dec(15000, 18)).sub(expectedUSDHLoss_A);
      ...
      // check Alice's USDHLoss has been applied to her deposit expectedCompoundedDeposit_A
    ...
    it("withdrawETHGainToTrove(): reverts if user has no trove", async () => {
      ...
      // A transfers USDH to D
      await usdhToken.transfer(dennis, dec(10000, 18), { from: alice });
    ...
    it("withdrawETHGainToTrove(): triggers HALAL reward event - increases the sum G", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawETHGainToTrove(), eligible deposit: depositor receives HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
    ...
    it("withdrawETHGainToTrove(), eligible deposit: tagged front end receives HALAL rewards", async () => {
      await openTrove({
        extraUSDHAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale }
      ...
      // Get front ends' HALAL balance after
      const F1_HALALBalance_After = await halalToken.balanceOf(frontEnd_1);
      const F2_HALALBalance_After = await halalToken.balanceOf(frontEnd_2);
      const F3_HALALBalance_After = await halalToken.balanceOf(frontEnd_3);
    ...
    it("withdrawETHGainToTrove(): reverts when depositor has no ETH gain", async () => {
      ...
      // fastforward time, and E makes a deposit, creating HALAL rewards for all
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);
      await openTrove({
        extraUSDHAmount: toBN(dec(3000, 18)),
        ICR: toBN(dec(2, 18)),
