...
contract("TroveManager - in Recovery Mode - back to normal mode in 1 tx", async accounts => {
  ...
  context("Batch liquidations", () => {
    ...
    it("Stability Pool profit matches", async () => {
      ...
      const spUsdhBefore = await stabilityPool.getTotalUSDHDeposits();
      ...
      const spUsdhAfter = await stabilityPool.getTotalUSDHDeposits();
      ...
      const expectedGainInUSDH = expectedCollateralLiquidatedA
        .mul(price)
        .div(mv._1e18BN)
        .sub(A_totalDebt);
      const realGainInUSDH = spEthAfter
        .sub(spEthBefore)
        .mul(price)
        .div(mv._1e18BN)
        .sub(spUsdhBefore.sub(spUsdhAfter));
      ...
      assert.equal(
        spUsdhBefore.sub(spUsdhAfter).toString(),
        A_totalDebt.toString(),
        "Stability Pool USDH doesn’t match"
      );
      assert.equal(
        realGainInUSDH.toString(),
        expectedGainInUSDH.toString(),
