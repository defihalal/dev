...
contract("TroveManager - Redistribution reward calculations", async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    */
    ...
    // check USDH gas compensation
    assert.equal((await usdhToken.balanceOf(owner)).toString(), dec(400, 18));
  });
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
    ...
    */
    ...
    // check USDH gas compensation
    assert.equal((await usdhToken.balanceOf(owner)).toString(), dec(1000, 18));
  });
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    // Expect Bob now holds all Ether and USDHDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 USDH (10 each for gas compensation)
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingETHReward(bob))
      .toString();

    const bob_USDHDebt = (await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingUSDHDebtReward(bob))
      .toString();
    ...
    assert.isAtMost(
      th.getDifference(bob_USDHDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)),
      1000
    );
  });
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 1/2*0.995 ETH, 55 USDH
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 USDHDebt

    coll: 3.1195 ETH
    debt: 233.77 USDHDebt

     Alice rewards:
    L1 1/2*0.995 ETH, 55 USDH
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 USDHDebt

    coll: 1.8705 ETH
    debt: 146.23 USDHDebt

    totalColl: 4.99 ETH
    totalDebt 380 USDH (includes 50 each for gas compensation)
    */
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingETHReward(bob))
      .toString();
    ...
    const alice_USDHDebt = (await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingUSDHDebtReward(alice))
      .toString();
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000);
    assert.isAtMost(th.getDifference(bob_USDHDebt, expected_B_debt), 10000);
    ...
    assert.isAtMost(th.getDifference(alice_USDHDebt, expected_A_debt), 10000);
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    */
    ...
    // check USDH gas compensation
    th.assertIsApproximatelyEqual((await usdhToken.balanceOf(owner)).toString(), dec(400, 18));
  });
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    // Expect Bob now holds all Ether and USDHDebt in the system: 2.5 Ether and 300 USDH
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 ETH, 55 USDH
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 USDHDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 USDHDebt 

     Alice rewards:
    L1 0.4975, 55 USDH
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 USDHDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 ETH
    debt: (50 + 55 + 66.022) = 171.022 USDH Debt

    totalColl: 3.49 ETH
    totalDebt 380 USDH (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingETHReward(bob))
      .toString();
    ...
    const entireSystemDebt = (await activePool.getUSDHDebt()).add(await defaultPool.getUSDHDebt());
    th.assertIsApproximatelyEqual(
      entireSystemDebt,
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    ...
    // check USDH gas compensation
    assert.equal((await usdhToken.balanceOf(owner)).toString(), dec(600, 18));
  });
