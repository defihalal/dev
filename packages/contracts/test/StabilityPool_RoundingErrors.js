...
contract("Pool Manager: Sum-Product rounding errors", async accounts => {
  ...
  it.skip("Rounding errors: 100 deposits of 100USDH into SP, then 200 liquidations of 49USDH", async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalUSDHDeposits();
    const SP_ETH = await stabilityPool.getETH();
    const compoundedDeposit = await stabilityPool.getCompoundedUSDHDeposit(depositors[0]);
