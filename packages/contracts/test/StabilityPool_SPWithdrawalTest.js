  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
        whale,
        whale,
        { from: whale, value: dec(100000, "ether") }
      );

      // Whale transfers 10k USDH to A, B and C who then deposit it to the SP
      const depositors = [alice, bob, carol];
      for (account of depositors) {
        await usdhToken.transfer(account, dec(10000, 18), { from: whale });
        await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: account });
      }

      // Defaulter opens trove with 200% ICR and 10k USDH net debt
      await borrowerOperations.openTrove(
        th._100pct,
      ...
      // Check depositors' compounded deposit is 6666.66 USDH and ETH Gain is 33.16 ETH
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "6666666666666666666666"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "6666666666666666666666"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "6666666666666666666666"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 3333.33 USDH and ETH Gain is 66.33 ETH
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice });
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob });
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol });
      // Grab the ETH gain from the emitted event in the tx log
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "3333333333333333333333"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "3333333333333333333333"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "3333333333333333333333"),
        10000
      );
    ...
    it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 0 USDH and ETH Gain is 99.5 ETH
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice });
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob });
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol });
      ...
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(bob)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(carol)).toString(), "0"), 10000);
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing USDH", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(5000, 18)),
        defaulter_1,
        defaulter_1,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(7000, 18)),
        defaulter_2,
        defaulter_2,
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "6000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "6000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "6000000000000000000000"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing USDH", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "4000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "4000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "4000000000000000000000"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      // Whale transfers 10k, 20k, 30k USDH to A, B and C respectively who then deposit it to the SP
      ...
      await usdhToken.transfer(bob, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: bob });
      await usdhToken.transfer(carol, dec(30000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "13333333333333333333333"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "20000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "5000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "10000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "15000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(1000000, 18)),
        whale,
        whale,
      ...
      Bob:  456000 USDH
      Carol: 13100 USDH */
      // Whale transfers USDH to  A, B and C respectively who then deposit it to the SP
      await usdhToken.transfer(alice, dec(2000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(2000, 18), ZERO_ADDRESS, { from: alice });
      await usdhToken.transfer(bob, dec(456000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(456000, 18), ZERO_ADDRESS, { from: bob });
      await usdhToken.transfer(carol, dec(13100, 18), { from: whale });
      await stabilityPool.provideToSP(dec(13100, 18), ZERO_ADDRESS, { from: carol });

      /* Defaulters open troves
     
      Defaulter 1: 207000 USDH & 2160 ETH
      Defaulter 2: 5000 USDH & 50 ETH
      Defaulter 3: 46700 USDH & 500 ETH
      */
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("207000000000000000000000"),
        defaulter_1,
        defaulter_1,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(5, 21)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("46700000000000000000000"),
        defaulter_3,
        defaulter_3,
      ...
      // ()
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "901719380174061000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "205592018679686000000000"),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "5906261940140100000000"),
        10000000000
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      await usdhToken.transfer(dennis, dec(10000, 18), { from: whale });
      ...
      console.log();
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "1666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "1666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "1666666666666666666666"),
        100000
      );

      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "0"), 100000);
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct USDH deposit and ETH Gain", async () => {
      // Whale opens Trove with 1m ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(1000000, 18)),
      ...
      */
      // Whale transfers USDH to  A, B and C respectively who then deposit it to the SP
      await usdhToken.transfer(alice, dec(60000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(60000, 18), ZERO_ADDRESS, { from: alice });
      await usdhToken.transfer(bob, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: bob });
      await usdhToken.transfer(carol, dec(15000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(15000, 18), ZERO_ADDRESS, { from: carol });

      /* Defaulters open troves:
      Defaulter 1:  10000 USDH, 100 ETH
      Defaulter 2:  25000 USDH, 250 ETH
      Defaulter 3:  5000 USDH, 50 ETH
      Defaulter 4:  40000 USDH, 400 ETH
      */
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(10000, 18)),
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(25000, 18)),
        defaulter_2,
        defaulter_2,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(40000, 18)),
        defaulter_4,
        defaulter_4,
      ...
      // Dennis provides 25000 USDH
      await usdhToken.transfer(dennis, dec(25000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "17832817337461300000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "5944272445820430000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "4458204334365320000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "11764705882352900000000"),
        100000000000
      );
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 USDH.  A, B, C, D withdraw correct USDH deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(alice)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(bob)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(carol)).toString(), "0"), 1000);
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct USDH deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      Dennis: 40000 USDH
      */
      // Whale transfers USDH to  A, B,C and D respectively who then deposit it to the SP
      await usdhToken.transfer(alice, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: alice });
      await usdhToken.transfer(bob, dec(25000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(25000, 18), ZERO_ADDRESS, { from: bob });
      await usdhToken.transfer(carol, dec(12500, 18), { from: whale });
      await stabilityPool.provideToSP(dec(12500, 18), ZERO_ADDRESS, { from: carol });
      await usdhToken.transfer(dennis, dec(40000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(40000, 18), ZERO_ADDRESS, { from: dennis });

      /* Defaulters open troves:
      Defaulter 1: 10000 USDH
      Defaulter 2: 20000 USDH
      Defaulter 3: 30000 USDH
      Defaulter 4: 5000 USDH
      */
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(10000, 18)),
      ...
      const dennis_ETHWithdrawn = th.getEventArgByName(txD, "ETHGainWithdrawn", "_ETH").toString();
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "27692307692307700000000"),
        100000000000
      );
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "1672240802675590000000"),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "2090301003344480000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "1045150501672240000000"),
        100000000000
      );
    ...
    it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 USDH. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct USDH deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      // Whale transfers 10k USDH to A, B and D who then deposit it to the SP
      ...
      // Carol makes deposit
      await usdhToken.transfer(carol, dec(10000, 18), { from: whale });
      ...
      const dennis_ETHWithdrawn = th.getEventArgByName(txD, "ETHGainWithdrawn", "_ETH").toString();
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "1666666666666666666666"),
        100000
      );
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(alice)).toString(), "666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "2000000000000000000000"),
        100000
      );
    ...
    // A, B withdraw 0USDH & 100e
    // C, D withdraw 5000USDH  & 500e
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Whale transfers 10k USDH to A, B who then deposit it to the SP
      ...
      // Defaulter 1 liquidated. 20000 USDH fully offset with pool.
      await troveManager.liquidate(defaulter_1, { from: owner });

      // Carol, Dennis each deposit 10000 USDH
      ...
      // Defaulter 2 liquidated. 10000 USDH offset
      await troveManager.liquidate(defaulter_2, { from: owner });
      ...
      // Expect Alice And Bob's compounded deposit to be 0 USDH
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(bob)).toString(), "0"), 10000);
      ...
      // Expect Carol And Dennis' compounded deposit to be 50 USDH
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "5000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
    ...
    it("withdrawFromSP(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
      ...
      // Defaulter 1 liquidated. 10--0 USDH fully offset, Pool remains non-zero
      await troveManager.liquidate(defaulter_1, { from: owner });
      ...
      // Defaulter 2 liquidated. 1--00 USDH, empties pool
      await troveManager.liquidate(defaulter_2, { from: owner });
      ...
      // Defaulter 3 liquidated. 10000 USDH fully offset, Pool remains non-zero
      await troveManager.liquidate(defaulter_3, { from: owner });
      ...
      // Defaulter 4 liquidated. 10000 USDH, empties pool
      await troveManager.liquidate(defaulter_4, { from: owner });
    ...
    // A, B withdraw 0 USDH & 100e
    // C, D withdraw 5000 USDH  & 50e
    it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Carol, Dennis, Erin each deposit 10000, 20000, 30000 USDH respectively
      await usdhToken.transfer(carol, dec(10000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: carol });

      await usdhToken.transfer(dennis, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: dennis });

      await usdhToken.transfer(erin, dec(30000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), "8333333333333333333333"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "16666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(erin)).toString(), "25000000000000000000000"),
        100000
      );
    ...
    // A deposits 10000
    // L1, L2, L3 liquidated with 10000 USDH each
    // A withdraws all
    ...
    it("withdrawFromSP(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the ETH Gain from one liquidation", async () => {
      ...
      // Defaulter 1,2,3 withdraw 10000 USDH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(10000, 18)),
        defaulter_1,
    ...
    // A,B deposit 10000 USDH
    // L1 cancels 20000 USDH, 2E
    // B,C deposits 10000 USDH
    // L2 cancels 20000 USDH, 2E
    // E,F deposit 10000 USDH
    // L3 cancels 20000, 200E
    // G,H deposits 10000
    // L4 cancels 20000, 200E

    // Expect all depositors withdraw 0 USDH and 100 ETH
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Alice, Bob each deposit 10k USDH
      ...
      // Erin, Flyn each deposit 10000 USDH
      ...
      // Defaulter 3 liquidated. 10000 USDH offset
      await troveManager.liquidate(defaulter_3, { from: owner });

      // Graham, Harriet each deposit 10000 USDH
      ...
      // Defaulter 4 liquidated. 10k USDH offset
      await troveManager.liquidate(defaulter_4, { from: owner });
      ...
      // Expect all deposits to be 0 USDH
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(dennis)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(erin)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(flyn)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdhToken.balanceOf(graham)).toString(), "0"), 100000);
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(harriet)).toString(), "0"),
        100000
      );
    ...
    // A withdraws all
    // B deposits 10000
    // L2 of 9900 USDH, should bring P slightly past boundary i.e. 1e-9 -> 1e-10
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10000 USDH:  9999.99991 USDH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("9999999910000000000000"),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, "ether") }
      );
      ...
      // Defaulter 2 withdraws 9900 USDH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(9900, 18)),
        defaulter_2,
        defaulter_2,
      ...
      // Defaulter 2 liquidated.  9900 USDH liquidated. P altered by a factor of 1-(9900/10000) = 0.01.  Scale changed.
      await troveManager.liquidate(defaulter_2, { from: owner });
      ...
      // Expect Bob to withdraw 1% of initial deposit (100 USDH) and all the liquidated ETH (60 ether)
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), "100000000000000000000"),
        100000
      );
    ...
    // A deposits 10000
    // L1 brings P close to boundary, i.e. 9e-9: liquidate 9999.99991 USDH
    // A withdraws all
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10k USDH.
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("9999999910000000000000"),
        defaulter_1,
      ...
      // Defaulter 2 withdraws 59400 USDH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("59400000000000000000000"),
        defaulter_2,
        defaulter_2,
      ...
      await usdhToken.transfer(carol, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: carol });

      await usdhToken.transfer(dennis, dec(30000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(30000, 18), ZERO_ADDRESS, { from: dennis });

      // 54000 USDH liquidated.  P altered by a factor of 1-(59400/60000) = 0.01. Scale changed.
      ...
      Bob:  1000 USDH, 55 Ether
      Carol:  2000 USDH, 110 Ether
      Dennis:  3000 USDH, 165 Ether
     
      Total: 6000 USDH, 300 Ether
      */
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), dec(100, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), dec(200, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), dec(300, 18)),
        100000
      );
    ...
    // A make deposit 10000 USDH
    // L1 brings P to 1e-5*P. L1:  9999.9000000000000000 USDH
    // A withdraws
    // B makes deposit 10000 USDH
    // L2 decreases P again by 1e-5, over the scale boundary: 9999.9000000000000000 (near to the 10000 USDH total deposits)
    // B withdraws
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 and default 2 each withdraw 9999.999999999 USDH
      await borrowerOperations.openTrove(
        th._100pct,
      ...
      // Bob should withdraw 1e-5 of initial deposit: 0.1 USDH and the full ETH gain of 100 ether
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), dec(1, 17)),
        100000
      );
    ...
    // L2 decreases P again by 1e-5, over boundary. L2: 59999.4000000000000000  (near to the 60000 USDH total deposits)
    // B withdraws
    // expect d(B) = d0(B) * 1e-5
    // expect B gets entire ETH gain from L2
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 and default 2 withdraw up to debt of 9999.9 USDH and 59999.4 USDH
      await borrowerOperations.openTrove(
        th._100pct,
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("59999400000000000000000"),
        defaulter_2,
        defaulter_2,
      ...
      // B, C, D deposit 10000, 20000, 30000 USDH
      await usdhToken.transfer(bob, dec(10000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: bob });
      ...
      // {B, C, D} should have a compounded deposit of {0.1, 0.2, 0.3} USDH
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(bob)).toString(), dec(1, 17)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(carol)).toString(), dec(2, 17)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), dec(3, 17)),
        100000
      );
    ...
    // A make deposit 10000 USDH
    // L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 USDH
    ...
    it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      // Defaulters 1 withdraws 9999.9999999 USDH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("9999999999900000000000"),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(100, "ether") }
      );
      ...
      const aliceDeposit = (await stabilityPool.getCompoundedUSDHDeposit(alice)).toString();
    ...
    /* A make deposit 10000 USDH
    L1 brings P to 0.0001P. L1:  9999.900000000000000000 USDH, 1 ETH
    B makes deposit 9999.9, brings SP to 10k
    L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 USDH, 1 ETH
    C makes deposit 9999.9, brings SP to 10k
    L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 USDH, 1 ETH
    D makes deposit 9999.9, brings SP to 10k
    L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 USDH, 1 ETH
    expect A, B, C, D each withdraw ~100 Ether
    */
    it("withdrawFromSP(): Several deposits of 10000 USDH span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(100000, 18)),
      ...
      // Defaulters 1-4 each withdraw 9999.9 USDH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount("9999900000000000000000"),
        defaulter_1,
      ...
      // B deposits 9999.9 USDH
      await usdhToken.transfer(bob, dec(99999, 17), { from: whale });
      ...
      // C deposits 9999.9 USDH
      await usdhToken.transfer(carol, dec(99999, 17), { from: whale });
      ...
      // D deposits 9999.9 USDH
      await usdhToken.transfer(dennis, dec(99999, 17), { from: whale });
      ...
      assert.equal(await usdhToken.balanceOf(alice), "0");
      assert.equal(await usdhToken.balanceOf(alice), "0");
      assert.equal(await usdhToken.balanceOf(alice), "0");
      // D should withdraw around 0.9999 USDH, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(
        th.getDifference((await usdhToken.balanceOf(dennis)).toString(), dec(99999, 12)),
        100000
      );
    ...
    it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      // Defaulters 1-3 each withdraw 24100, 24300, 24500 USDH (inc gas comp)
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(24100, 18)),
        defaulter_1,
        defaulter_1,
        { from: defaulter_1, value: dec(200, "ether") }
      );
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(24300, 18)),
        defaulter_2,
        defaulter_2,
        { from: defaulter_2, value: dec(200, "ether") }
      );
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(24500, 18)),
        defaulter_3,
        defaulter_3,
        { from: defaulter_3, value: dec(200, "ether") }
      );
      ...
      // A, B provide 10k USDH
      await usdhToken.transfer(A, dec(10000, 18), { from: whale });
      await usdhToken.transfer(B, dec(10000, 18), { from: whale });
      ...
      const A_deposit = await stabilityPool.getCompoundedUSDHDeposit(A);
      const B_deposit = await stabilityPool.getCompoundedUSDHDeposit(B);
      ...
      const USDHinSP_1 = await stabilityPool.getTotalUSDHDeposits();
      // console.log(`USDHinSP_1: ${USDHinSP_1}`)
      assert.equal(USDHinSP_1, "0");

      // Check SP USDH balance is zero
      const SPUSDHBalance_1 = await usdhToken.balanceOf(stabilityPool.address);
      // console.log(`SPUSDHBalance_1: ${SPUSDHBalance_1}`)
      assert.equal(SPUSDHBalance_1, "0");
      ...
      // C, D provide 10k USDH
      await usdhToken.transfer(C, dec(10000, 18), { from: whale });
      await usdhToken.transfer(D, dec(10000, 18), { from: whale });
      ...
      // Check compounded deposits
      const C_deposit = await stabilityPool.getCompoundedUSDHDeposit(C);
      const D_deposit = await stabilityPool.getCompoundedUSDHDeposit(D);
      ...
      // Check SP tracker is zero
      const USDHinSP_2 = await stabilityPool.getTotalUSDHDeposits();
      // console.log(`USDHinSP_2: ${USDHinSP_2}`)
      assert.equal(USDHinSP_2, "0");

      // Check SP USDH balance is zero
      const SPUSDHBalance_2 = await usdhToken.balanceOf(stabilityPool.address);
      // console.log(`SPUSDHBalance_2: ${SPUSDHBalance_2}`)
      assert.equal(SPUSDHBalance_2, "0");
      ...
      // E, F provide 10k USDH
      await usdhToken.transfer(E, dec(10000, 18), { from: whale });
      await usdhToken.transfer(F, dec(10000, 18), { from: whale });
      ...
      // Check compounded deposits
      const E_deposit = await stabilityPool.getCompoundedUSDHDeposit(E);
      const F_deposit = await stabilityPool.getCompoundedUSDHDeposit(F);
      ...
      // Check SP tracker is zero
      const USDHinSP_3 = await stabilityPool.getTotalUSDHDeposits();
      assert.equal(USDHinSP_3, "0");

      // Check SP USDH balance is zero
      const SPUSDHBalance_3 = await usdhToken.balanceOf(stabilityPool.address);
      // console.log(`SPUSDHBalance_3: ${SPUSDHBalance_3}`)
      assert.equal(SPUSDHBalance_3, "0");
    ...
    it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
      ...
      // Defaulters 1-5 each withdraw up to debt of 9999.9999999 USDH
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(99999, 17)),
        defaulter_1,
      ...
      // E deposits 9999.9 USDH
      await usdhToken.transfer(erin, dec(99999, 17), { from: whale });
    ...
    it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(1, 36)),
        defaulter_1,
        defaulter_1,
      ...
      // Check USDH balances
      const aliceUSDHBalance = await usdhToken.balanceOf(alice);
      const aliceExpectedUSDHBalance = web3.utils.toBN(dec(5, 35));
      const aliceUSDHBalDiff = aliceUSDHBalance.sub(aliceExpectedUSDHBalance).abs();

      assert.isTrue(aliceUSDHBalDiff.lte(toBN(dec(1, 18)))); // error tolerance of 1e18

      const bobUSDHBalance = await usdhToken.balanceOf(bob);
      const bobExpectedUSDHBalance = toBN(dec(5, 35));
      const bobUSDHBalDiff = bobUSDHBalance.sub(bobExpectedUSDHBalance).abs();

      assert.isTrue(bobUSDHBalDiff.lte(toBN(dec(1, 18))));
    ...
    it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      // Defaulter opens trove with 50e-7 ETH and  5000 USDH. 200% ICR
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveUSDHAmount(dec(5000, 18)),
        defaulter_1,
      ...
      const aliceUSDHBalance = await usdhToken.balanceOf(alice);
      const aliceExpectedUSDHBalance = toBN("99999999999999997500000000000000000000");
      const aliceUSDHBalDiff = aliceUSDHBalance.sub(aliceExpectedUSDHBalance).abs();

      assert.isTrue(aliceUSDHBalDiff.lte(toBN(dec(1, 18))));

      const bobUSDHBalance = await usdhToken.balanceOf(bob);
      const bobExpectedUSDHBalance = toBN("99999999999999997500000000000000000000");
      const bobUSDHBalDiff = bobUSDHBalance.sub(bobExpectedUSDHBalance).abs();

      assert.isTrue(bobUSDHBalDiff.lte(toBN("100000000000000000000")));
