...
def liquidate_troves(troves, index, data):
  ...
  price_USDH_previous = data.loc[index-1,'Price_USDH']
  price_HALAL_previous = data.loc[index-1,'price_HALAL']
  ...
  liquidation_gain = ether_liquidated*price_ether_current - debt_liquidated*price_USDH_previous
  airdrop_gain = price_HALAL_previous * quantity_HALAL_airdrop
  ...
  if index <= day:
   return_stability = initial_return*(1+shock_return)
    ...
    return_stability = min(0.5, 365*(data.loc[index-day:index, 'liquidation_gain'].sum()+data.loc[index-day:index, 'airdrop_gain'].sum())/(price_USDH_previous*stability_pool_previous))
  else:
    return_stability = (365/30)*(data.loc[index-month:index, 'liquidation_gain'].sum()+data.loc[index-month:index, 'airdrop_gain'].sum())/(price_USDH_previous*stability_pool_previous)
  
  return[troves, return_stability, debt_liquidated, ether_liquidated, liquidation_gain, airdrop_gain, n_liquidate]

"""Close Troves"""

def close_troves(troves, index2, price_USDH_previous):
  ...
  elif price_USDH_previous >=1:
    number_closetroves = max(0, n_steady * (1+shock_closetroves))
  else:
    number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_USDH_previous)*n_troves
...
def adjust_troves(troves, index):
  ...
  #A part of the troves are adjusted by adjusting debt
    if p >= ratio:
      ...
      if check>2:
        ...
        issuance_USDH_adjust = issuance_USDH_adjust + rate_issuance * (supply_new - working_trove['Supply'])
  ...
  return[troves, issuance_USDH_adjust]

"""Open Troves"""

def open_troves(troves, index1, price_USDH_previous):
  ...
  elif price_USDH_previous <=1 + rate_issuance:
    number_opentroves = max(0, n_steady * (1+shock_opentroves))
  else:
    number_opentroves = max(0, n_steady * (1+shock_opentroves)) + alpha*(price_USDH_previous-rate_issuance-1)*n_troves
  ...
  return[troves, number_opentroves, issuance_USDH_open]
...
#Calculating Price
  ...
  price_USDH_previous = float(data['Price_USDH'][index-1])
  price_USDH_current= price_USDH_previous*((supply-stability_pool)/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
...
#Stabilizer
  ...
  if price_USDH_current > 1.1 + rate_issuance:
    ...
    supply_wanted=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1.1+rate_issuance)/price_USDH_previous)**delta
  ...
  if price_USDH_current < 1 - rate_redemption:
    ...
    #supply_current = sum(troves['Supply'])
    supply_target=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1-rate_redemption)/price_USDH_previous)**delta
    ...
    if supply_diff < redemption_ratio * liquidity_pool:
      ...
      price_USDH_current= price_USDH_previous * (liquidity_pool/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
  ...
  troves = troves.reset_index(drop=True)
  return[price_USDH_current, liquidity_pool, troves, issuance_USDH_stabilizer, redemption_fee, n_redempt, redemption_pool, n_open]
...
def HALAL_market(index, data):
  quantity_HALAL = (100000000/3)*(1-0.5**(index/period))
  ...
  return[price_HALAL_current, annualized_earning, MC_HALAL_current]

"""# Simulation Program"""

#Defining Initials
initials = {"Price_USDH":[1.00], "Price_Ether":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0], 
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_USDH":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_HALAL":[price_HALAL_initial], "MC_HALAL":[0], "annualized_earning":[0]}
...
result_open = open_troves(troves, 0, data['Price_USDH'][0])
troves = result_open[0]
issuance_USDH_open = result_open[2]
data.loc[0,'issuance_fee'] = issuance_USDH_open * initials["Price_USDH"][0]
data.loc[0,'supply_USDH'] = troves["Supply"].sum()
...
#close troves
  result_close = close_troves(troves, index, price_USDH_previous)
...
#adjust troves
  ...
  issuance_USDH_adjust = result_adjustment[1]

#open troves
  result_open = open_troves(troves, index, price_USDH_previous)
  troves = result_open[0]
...
#Calculating Price, Liquidity Pool, and Redemption
  ...
  price_USDH_current = result_price[0]
  liquidity_pool = result_price[1]
  troves = result_price[2]
  issuance_USDH_stabilizer = result_price[3]
...
#Summary
  issuance_fee = price_USDH_current * (issuance_USDH_adjust + issuance_USDH_open + issuance_USDH_stabilizer)
  n_troves = troves.shape[0]
  supply_USDH = troves['Supply'].sum()
  if index >= month:
    price_HALAL.append(price_HALAL_current)

  new_row = {"Price_USDH":float(price_USDH_current), "Price_Ether":float(price_ether_current), "n_open":float(n_open), "n_close":float(n_close), 
             "n_liquidate":float(n_liquidate), "n_redempt": float(n_redempt), "n_troves":float(n_troves),
              "stability":float(stability_pool), "liquidity":float(liquidity_pool), "redemption_pool":float(redemption_pool), "supply_USDH":float(supply_USDH),
             ...
             "annualized_earning":float(annualized_earning), "MC_HALAL":float(MC_HALAL_current), "price_HALAL":float(price_HALAL_current)
             }
  data = data.append(new_row, ignore_index=True)
  if price_USDH_current < 0:
    break
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['Price_USDH'], name="USDH Price"),
    secondary_y=False,
)
fig.add_trace(
...
fig.update_layout(
    title_text="Price Dynamics of USDH and Ether"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="USDH Price", secondary_y=False)
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['supply_USDH'], name="USDH Supply"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and USDH Supply"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="Number of Troves", secondary_y=False)
fig.update_yaxes(title_text="USDH Supply", secondary_y=True)
fig.show()
...
fig = make_subplots(specs=[[{"secondary_y": True}]])
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['price_HALAL'], name="HALAL Price"),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['MC_HALAL'], name="HALAL Market Cap"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of HALAL"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="HALAL Price", secondary_y=False)
fig.update_yaxes(title_text="HALAL Market Cap", secondary_y=True)
fig.show()
...
initials = {"Price_USDH":[1.00], "Price_Ether":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0], 
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_USDH":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_HALAL":[price_HALAL_initial], "MC_HALAL":[0], "annualized_earning":[0], "base_rate":[base_rate_initial]}
data2 = pd.DataFrame(initials)
troves2= pd.DataFrame({"Ether_Price":[], "Ether_Quantity":[], "CR_initial":[], 
              "Supply":[], "Rational_inattention":[], "CR_current":[]})
result_open = open_troves(troves2, 0, data2['Price_USDH'][0])
troves2 = result_open[0]
issuance_USDH_open = result_open[2]
data2.loc[0,'issuance_fee'] = issuance_USDH_open * initials["Price_USDH"][0]
data2.loc[0,'supply_USDH'] = troves2["Supply"].sum()
...
#exogenous ether price input
  ...
  price_USDH_previous = data2.loc[index-1,'Price_USDH']
  price_HALAL_previous = data2.loc[index-1,'price_HALAL']
...
#close troves
  result_close = close_troves(troves2, index, price_USDH_previous)
...
#open troves
  result_open = open_troves(troves2, index, price_USDH_previous)
  troves2 = result_open[0]
  n_open = result_open[1]  
  issuance_USDH_open = result_open[2]
...
#HALAL Market
  result_HALAL = HALAL_market(index, data2)
  price_HALAL_current = result_HALAL[0]
  annualized_earning = result_HALAL[1]
  MC_HALAL_current = result_HALAL[2]
...
#Summary
  ...
  supply_USDH = troves2['Supply'].sum()
  if index >= month:
    price_HALAL.append(price_HALAL_current)
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['Price_USDH'], name="USDH Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.update_layout(
    title_text="Price Dynamics of USDH and Ether"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['supply_USDH'], name="USDH Supply New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and USDH Supply"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['price_HALAL'], name="HALAL Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['MC_HALAL'], name="HALAL Market Cap New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of HALAL"
