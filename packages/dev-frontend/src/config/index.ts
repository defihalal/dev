...
const parseConfig = (json: unknown): LiquityFrontendConfig => {
  ...
  if (typeof json === "object" && json !== null) {
    ...
    if (hasKey(json, "maticNode") && json.maticNode !== "") {
      const { maticNode } = json;

      if (typeof maticNode === "string") {
        config.maticNode = maticNode;
      } else {
        console.error("Malformed maticNode:");
        console.log(maticNode);
      }
    }

    if (hasKey(json, "mumbaiNode") && json.mumbaiNode !== "") {
      const { mumbaiNode } = json;

      if (typeof mumbaiNode === "string") {
        config.mumbaiNode = mumbaiNode;
      } else {
        console.error("Malformed mumbaiNode:");
        console.log(mumbaiNode);
      }
    }
