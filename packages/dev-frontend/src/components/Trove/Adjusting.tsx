...
const GAS_ROOM_MATIC = Decimal.from(0.1);

const feeFrom = (original: Trove, edited: Trove, borrowingRate: Decimal): Decimal => {
  const change = original.whatChanged(edited, borrowingRate);

  if (change && change.type !== "invalidCreation" && change.params.borrowUSDH) {
    return change.params.borrowUSDH.mul(borrowingRate);
  } else {
    return Decimal.ZERO;
  }
};
...
export const Adjusting: React.FC = () => {
  ...
  const fee = isDebtIncrease
    ...
    : Decimal.ZERO;
  const totalDebt = netDebt.add(USDH_LIQUIDATION_RESERVE).add(fee);
  const maxBorrowingRate = borrowingRate.add(0.005);
  const updatedTrove = isDirty ? new Trove(collateral, totalDebt) : trove;
  const feePct = new Percent(borrowingRate);
  const availableEth = accountBalance.gt(GAS_ROOM_MATIC)
    ? accountBalance.sub(GAS_ROOM_MATIC)
    : Decimal.ZERO;
    ...
    <Card>
      ...
      <Box sx={{ p: [2, 3] }}>
        ...
        <StaticRow
          ...
          amount={`${USDH_LIQUIDATION_RESERVE}`}
          unit={COIN}
          infoIcon={
            <InfoIcon
              tooltip={
        ...
        <StaticRow
          ...
          infoIcon={
            <InfoIcon
              tooltip={
                <Card variant="tooltip" sx={{ width: "240px" }}>
                  The total amount of USDH your Trove will hold.{" "}
                  {isDirty && (
                    <>
                      You will need to repay {totalDebt.sub(USDH_LIQUIDATION_RESERVE).prettify(2)}{" "}
                      USDH to reclaim your collateral ({USDH_LIQUIDATION_RESERVE.toString()} USDH
