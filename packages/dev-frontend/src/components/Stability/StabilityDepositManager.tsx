...
const init = ({ stabilityDeposit }: LiquityStoreState) => ({
  ...
  editedUSDH: stabilityDeposit.currentUSDH,
  changePending: false
});
...
const reduce = (
  ...
  const { originalDeposit, editedUSDH, changePending } = state;
  ...
  switch (action.type) {
    ...
    case "setDeposit":
      return { ...state, editedUSDH: Decimal.from(action.newValue) };

    case "revert":
      return { ...state, editedUSDH: originalDeposit.currentUSDH };
    ...
    case "updateStore": {
      ...
      const changeCommitted =
        !updatedDeposit.initialUSDH.eq(originalDeposit.initialUSDH) ||
        updatedDeposit.currentUSDH.gt(originalDeposit.currentUSDH) ||
        updatedDeposit.collateralGain.lt(originalDeposit.collateralGain) ||
        updatedDeposit.halalReward.lt(originalDeposit.halalReward);
      ...
      return {
        ...newState,
        editedUSDH: updatedDeposit.apply(originalDeposit.whatChanged(editedUSDH))
      };
    }
  }
};
...
export const StabilityDepositManager: React.FC = () => {
  const [{ originalDeposit, editedUSDH, changePending }, dispatch] = useLiquityReducer(reduce, init);
