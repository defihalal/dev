...
const selector = ({ stabilityDeposit, trove, usdhInStabilityPool }: LiquityStoreState) => ({
  stabilityDeposit,
  trove,
  usdhInStabilityPool
});
...
export const ActiveDeposit: React.FC = () => {
  ...
  const { stabilityDeposit, trove, usdhInStabilityPool } = useLiquitySelector(selector);

  const poolShare = stabilityDeposit.currentUSDH.mulDiv(100, usdhInStabilityPool);

  const handleAdjustDeposit = useCallback(() => {
    dispatchEvent("ADJUST_DEPOSIT_PRESSED");
  }, [dispatchEvent]);

  const hasReward = !stabilityDeposit.halalReward.isZero;
    ...
    <Card>
      ...
      <Box sx={{ p: [2, 3] }}>
        <Box>
          <DisabledEditableRow
            label="Deposit"
            inputId="deposit-usdh"
            amount={stabilityDeposit.currentUSDH.prettify()}
            unit={COIN}
          />
          ...
          <Flex sx={{ alignItems: "center" }}>
            <StaticRow
              ...
              amount={stabilityDeposit.halalReward.prettify()}
              color={stabilityDeposit.halalReward.nonZero && "success"}
              unit={GT}
              infoIcon={
                <InfoIcon
                  tooltip={
        ...
        <Flex variant="layout.actions">
          ...
          <ClaimRewards disabled={!hasGain && !hasReward}>Claim MATIC and HALAL</ClaimRewards>
        </Flex>

        {hasTrove && (
          <ClaimAndMove disabled={!hasGain}>Claim HALAL and move MATIC to Trove</ClaimAndMove>
        )}
      </Box>
