...
const supportedNetworks = ["maticmum", "matic"];
...
export const LiquityProvider: React.FC<LiquityProviderProps> = ({
  ...
  const connection = useMemo(() => {
    if (config && provider && account && chainId) {
      try {
        let x = _connectByChainId(provider, provider.getSigner(account), chainId, {
          userAddress: account,
      ...
      if (isWebSocketAugmentedProvider(provider)) {
        ...
        if (
          ...
          provider.openWebSocket(
            ...wsParams(network.name, config.infuraApiKey, config.maticNode, config.mumbaiNode)
          );
