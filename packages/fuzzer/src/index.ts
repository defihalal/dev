  ...
  .command(
    ...
    async ({ troves }) => {
      ...
      for (let i = 1; i <= troves; ++i) {
        ...
        const debt = USDH_LIQUIDATION_RESERVE.add(99999 * Math.random());
        ...
        if (i % 4 === 0) {
          ...
          await liquity.depositUSDHInStabilityPool(usdhBalance);
        }
      ...
      for (let i = 1; i <= numberOfRounds; ++i) {
        ...
        for (let i = 0; i < randomUsers.length; ++i) {
          ...
          if (x < 0.5) {
            ...
            if (deposit.initialUSDH.isZero || x < 0.6) {
            ...
            const stake = await liquity.getHALALStake();

            if (stake.stakedHALAL.isZero || x < 0.8) {
          ...
          // await fixture.sweepUSDH(liquity);
          await fixture.sweepHALAL(liquity);
  ...
  .command(
    ...
    async () => {
      ...
      if (firstTroveOwner !== funder.address) {
        ...
        const usdhBalance = await funderLiquity.getUSDHBalance();

        if (usdhBalance.lt(trove.netDebt)) {
          const [randomUser] = createRandomWallets(1, provider);
          const randomLiquity = await Liquity.connect(randomUser);

          const usdhNeeded = trove.netDebt.sub(usdhBalance);
          const tempTrove = {
            depositCollateral: USDH_LIQUIDATION_RESERVE.add(usdhNeeded).div(initialPrice).mul(3),
            borrowUSDH: usdhNeeded
          };
          ...
          await randomLiquity.sendUSDH(funder.address, usdhNeeded, { gasPrice: 0 });
        }

        await funderLiquity.repayUSDH(trove.netDebt);
      }
