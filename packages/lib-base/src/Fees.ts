...
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in USDH, multiply the borrowed USDH amount by the borrowing rate.
   *
   * @example
   * ```typescript
   * const fees = await liquity.getFees();
   *
   * const borrowedUSDHAmount = 100;
   * const borrowingRate = fees.borrowingRate();
   * const borrowingFeeUSDH = borrowingRate.mul(borrowedUSDHAmount);
   * ```
   */
   ...
   *
   * @param redeemedFractionOfSupply - The amount of USDH being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total USDH supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in USDH, multiply the redeemed USDH amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
   ...
   *
   * const redeemedUSDHAmount = Decimal.from(100);
   * const redeemedFractionOfSupply = redeemedUSDHAmount.div(total.debt);
   * const redemptionRate = fees.redemptionRate(redeemedFractionOfSupply);
   * const redemptionFeeUSDH = redemptionRate.mul(redeemedUSDHAmount);
   * ```
   */
