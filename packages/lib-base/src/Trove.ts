...
/** @internal */ export type _USDHBorrowing<T> = { borrowUSDH: T };
/** @internal */ export type _USDHRepayment<T> = { repayUSDH: T };
...
/** @internal */ export type _NoUSDHBorrowing = Partial<_USDHBorrowing<undefined>>;
/** @internal */ export type _NoUSDHRepayment = Partial<_USDHRepayment<undefined>>;
...
export type _DebtChange<T> =
  | (_USDHBorrowing<T> & _NoUSDHRepayment)
  | (_USDHRepayment<T> & _NoUSDHBorrowing);

/** @internal */
export type _NoDebtChange = _NoUSDHBorrowing & _NoUSDHRepayment;
 ...
 *     <td> borrowUSDH </td>
 *     <td> T </td>
 *     <td> The amount of USDH that's borrowed. </td>
 *   </tr>
 *
 * </table>
 *
 ...
 *     <td> repayUSDH? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of USDH that's repaid. </td>
 *   </tr>
 *
 * </table>
 *
 ...
 * least one.
 *
 * Defining both `depositCollateral` and `withdrawCollateral`, or both `borrowUSDH` and `repayUSDH`
 ...
 *     <td> borrowUSDH? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of USDH that's borrowed. </td>
 *   </tr>
 *
 *   <tr>
 *     <td> repayUSDH? </td>
...
const debtChangeFrom = <T>({
  ...
  if (borrowUSDH !== undefined && repayUSDH !== undefined) {
    throw new Error(
      "TroveAdjustmentParams: 'borrowUSDH' and 'repayUSDH' can't be present at the same time"
    );
  }

  if (borrowUSDH !== undefined) {
    return { borrowUSDH };
  }

  if (repayUSDH !== undefined) {
    return { repayUSDH };
  }
};
...
export class Trove {
  ...
  /**
   * Amount of USDH that must be repaid to close this Trove.
   *
   * @remarks
   * This doesn't include the liquidation reserve, which is refunded in case of normal closure.
   */
  get netDebt(): Decimal {
    if (this.debt.lt(USDH_LIQUIDATION_RESERVE)) {
      throw new Error(`netDebt should not be used when debt < ${USDH_LIQUIDATION_RESERVE}`);
    }

    return this.debt.sub(USDH_LIQUIDATION_RESERVE);
  }
  ...
  private _debtChange({ debt }: Trove, borrowingRate: Decimalish): _DebtChange<Decimal> {
    return debt.gt(this.debt)
      ? { borrowUSDH: unapplyFee(borrowingRate, debt.sub(this.debt)) }
      : { repayUSDH: this.debt.sub(debt) };
  }
    ...
    if (that.isEmpty) {
      return troveClosure(
        this.netDebt.nonZero
          ? { withdrawCollateral: this.collateral, repayUSDH: this.netDebt }
  ...
  apply(
    ...
    switch (change.type) {
      ...
      case "creation": {
        ...
        const { depositCollateral, borrowUSDH } = change.params;

        return new Trove(
          depositCollateral,
          USDH_LIQUIDATION_RESERVE.add(applyFee(borrowingRate, borrowUSDH))
        );
      }
      ...
      case "adjustment": {
        const {
          setToZero,
          params: { depositCollateral, withdrawCollateral, borrowUSDH, repayUSDH }
        } = change;

        const collateralDecrease = withdrawCollateral ?? Decimal.ZERO;
        const collateralIncrease = depositCollateral ?? Decimal.ZERO;
        const debtDecrease = repayUSDH ?? Decimal.ZERO;
        const debtIncrease = borrowUSDH ? applyFee(borrowingRate, borrowUSDH) : Decimal.ZERO;
