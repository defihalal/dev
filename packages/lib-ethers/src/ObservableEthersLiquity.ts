...
export class ObservableEthersLiquity implements ObservableLiquity {
  ...
  watchUSDHInStabilityPool(
    onUSDHInStabilityPoolChanged: (usdhInStabilityPool: Decimal) => void
  ): () => void {
    const { usdhToken, stabilityPool } = _getContracts(this._readable.connection);
    const { Transfer } = usdhToken.filters;

    const transferUSDHFromStabilityPool = Transfer(stabilityPool.address);
    const transferUSDHToStabilityPool = Transfer(null, stabilityPool.address);

    const stabilityPoolUSDHFilters = [transferUSDHFromStabilityPool, transferUSDHToStabilityPool];

    const stabilityPoolUSDHListener = debounce((blockTag: number) => {
      this._readable.getUSDHInStabilityPool({ blockTag }).then(onUSDHInStabilityPoolChanged);
    });

    stabilityPoolUSDHFilters.forEach(filter => usdhToken.on(filter, stabilityPoolUSDHListener));

    return () =>
      stabilityPoolUSDHFilters.forEach(filter =>
        usdhToken.removeListener(filter, stabilityPoolUSDHListener)
      );
  }

  watchUSDHBalance(onUSDHBalanceChanged: (balance: Decimal) => void, address?: string): () => void {
    address ??= _requireAddress(this._readable.connection);

    const { usdhToken } = _getContracts(this._readable.connection);
    const { Transfer } = usdhToken.filters;
    const transferUSDHFromUser = Transfer(address);
    const transferUSDHToUser = Transfer(null, address);

    const usdhTransferFilters = [transferUSDHFromUser, transferUSDHToUser];

    const usdhTransferListener = debounce((blockTag: number) => {
      this._readable.getUSDHBalance(address, { blockTag }).then(onUSDHBalanceChanged);
    });

    usdhTransferFilters.forEach(filter => usdhToken.on(filter, usdhTransferListener));

    return () =>
      usdhTransferFilters.forEach(filter => usdhToken.removeListener(filter, usdhTransferListener));
  }
}
