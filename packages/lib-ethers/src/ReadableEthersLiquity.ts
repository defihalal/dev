...
export class ReadableEthersLiquity implements ReadableLiquity {
  ...
  async getTotalRedistributed(overrides?: EthersCallOverrides): Promise<Trove> {
    ...
    const [collateral, debt] = await Promise.all([
      ...
      troveManager.L_USDHDebt({ ...overrides }).then(decimalify)
    ]);
  ...
  async getTroveBeforeRedistribution(
    ...
    if (trove.status === BackendTroveStatus.active) {
      return new TroveWithPendingRedistribution(
        ...
        new Trove(decimalify(snapshot.ETH), decimalify(snapshot.USDHDebt))
      );
    } else {
  ...
  async _getActivePool(overrides?: EthersCallOverrides): Promise<Trove> {
    ...
    const [activeCollateral, activeDebt] = await Promise.all(
      [
        ...
        activePool.getUSDHDebt({ ...overrides })
  ...
  async _getDefaultPool(overrides?: EthersCallOverrides): Promise<Trove> {
    ...
    const [liquidatedCollateral, closedDebt] = await Promise.all(
      [
        ...
        defaultPool.getUSDHDebt({ ...overrides })
      ].map(getBigNumber => getBigNumber.then(decimalify))
    );
  ...
  async getStabilityDeposit(
    ...
    const [
      ...
      stabilityPool.getCompoundedUSDHDeposit(address, { ...overrides }),
      stabilityPool.getDepositorETHGain(address, { ...overrides }),
      stabilityPool.getDepositorHALALGain(address, { ...overrides })
    ]);

    return new StabilityDeposit(
      decimalify(initialValue),
      decimalify(currentUSDH),
      decimalify(collateralGain),
      decimalify(halalReward),
      frontEndTag
    );
  }

  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getRemainingStabilityPoolHALALReward} */
  async getRemainingStabilityPoolHALALReward(overrides?: EthersCallOverrides): Promise<Decimal> {
    const { communityIssuance } = _getContracts(this.connection);

    const issuanceCap = this.connection.totalStabilityPoolHALALReward;
    const totalHALALIssued = decimalify(await communityIssuance.totalHALALIssued({ ...overrides }));

    // totalHALALIssued approaches but never reaches issuanceCap
    return issuanceCap.sub(totalHALALIssued);
  }

  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getUSDHInStabilityPool} */
  getUSDHInStabilityPool(overrides?: EthersCallOverrides): Promise<Decimal> {
    const { stabilityPool } = _getContracts(this.connection);

    return stabilityPool.getTotalUSDHDeposits({ ...overrides }).then(decimalify);
  }

  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getUSDHBalance} */
  getUSDHBalance(address?: string, overrides?: EthersCallOverrides): Promise<Decimal> {
    address ??= _requireAddress(this.connection);
    const { usdhToken } = _getContracts(this.connection);

    return usdhToken.balanceOf(address, { ...overrides }).then(decimalify);
  }

  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getHALALBalance} */
  getHALALBalance(address?: string, overrides?: EthersCallOverrides): Promise<Decimal> {
    address ??= _requireAddress(this.connection);
    const { halalToken } = _getContracts(this.connection);

    return halalToken.balanceOf(address, { ...overrides }).then(decimalify);
  }
  ...
  /** @internal */
  async _getRemainingLiquidityMiningHALALRewardCalculator(
    overrides?: EthersCallOverrides
  ...
  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getRemainingLiquidityMiningHALALReward} */
  async getRemainingLiquidityMiningHALALReward(overrides?: EthersCallOverrides): Promise<Decimal> {
    const [calculateRemainingHALAL, blockTimestamp] = await Promise.all([
      this._getRemainingLiquidityMiningHALALRewardCalculator(overrides),
      this._getBlockTimestamp(overrides?.blockTag)
    ]);

    return calculateRemainingHALAL(blockTimestamp);
  }
  ...
  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getLiquidityMiningHALALReward} */
  getLiquidityMiningHALALReward(
    address?: string,
    overrides?: EthersCallOverrides
  ): Promise<Decimal> {
    address ??= _requireAddress(this.connection);
  ...
  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getHALALStake} */
  async getHALALStake(address?: string, overrides?: EthersCallOverrides): Promise<HALALStake> {
    address ??= _requireAddress(this.connection);
    const { halalStaking } = _getContracts(this.connection);

    const [stakedHALAL, collateralGain, usdhGain] = await Promise.all(
      [
        halalStaking.stakes(address, { ...overrides }),
        halalStaking.getPendingETHGain(address, { ...overrides }),
        halalStaking.getPendingUSDHGain(address, { ...overrides })
      ].map(getBigNumber => getBigNumber.then(decimalify))
    );

    return new HALALStake(stakedHALAL, collateralGain, usdhGain);
  }

  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getTotalStakedHALAL} */
  async getTotalStakedHALAL(overrides?: EthersCallOverrides): Promise<Decimal> {
    const { halalStaking } = _getContracts(this.connection);

    return halalStaking.totalHALALStaked({ ...overrides }).then(decimalify);
  }
  ...
  troves.map(
    trove =>
      new TroveWithPendingRedistribution(
        ...
        decimalify(trove.debt),
        decimalify(trove.stake),
        new Trove(decimalify(trove.snapshotETH), decimalify(trove.snapshotUSDHDebt))
      )
  );
...
class _BlockPolledReadableEthersLiquity
  ...
  async getRemainingStabilityPoolHALALReward(overrides?: EthersCallOverrides): Promise<Decimal> {
    return this._blockHit(overrides)
      ? this.store.state.remainingStabilityPoolHALALReward
      : this._readable.getRemainingStabilityPoolHALALReward(overrides);
  }

  async getUSDHInStabilityPool(overrides?: EthersCallOverrides): Promise<Decimal> {
    return this._blockHit(overrides)
      ? this.store.state.usdhInStabilityPool
      : this._readable.getUSDHInStabilityPool(overrides);
  }

  async getUSDHBalance(address?: string, overrides?: EthersCallOverrides): Promise<Decimal> {
    return this._userHit(address, overrides)
      ? this.store.state.usdhBalance
      : this._readable.getUSDHBalance(address, overrides);
  }

  async getHALALBalance(address?: string, overrides?: EthersCallOverrides): Promise<Decimal> {
    return this._userHit(address, overrides)
      ? this.store.state.halalBalance
      : this._readable.getHALALBalance(address, overrides);
  }
  ...
  async getRemainingLiquidityMiningHALALReward(overrides?: EthersCallOverrides): Promise<Decimal> {
    return this._blockHit(overrides)
      ? this.store.state.remainingLiquidityMiningHALALReward
      : this._readable.getRemainingLiquidityMiningHALALReward(overrides);
  }
  ...
  async getLiquidityMiningHALALReward(
    address?: string,
    overrides?: EthersCallOverrides
  ): Promise<Decimal> {
    return this._userHit(address, overrides)
      ? this.store.state.liquidityMiningHALALReward
      : this._readable.getLiquidityMiningHALALReward(address, overrides);
  }
  ...
  async getHALALStake(address?: string, overrides?: EthersCallOverrides): Promise<HALALStake> {
    return this._userHit(address, overrides)
      ? this.store.state.halalStake
      : this._readable.getHALALStake(address, overrides);
  }

  async getTotalStakedHALAL(overrides?: EthersCallOverrides): Promise<Decimal> {
    return this._blockHit(overrides)
      ? this.store.state.totalStakedHALAL
      : this._readable.getTotalStakedHALAL(overrides);
  }
  ...
  _getRemainingLiquidityMiningHALALRewardCalculator(): Promise<(blockTimestamp: number) => Decimal> {
    throw new Error("Method not implemented.");
  }
}
