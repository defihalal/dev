...
const addGasForHALALIssuance = (gas: BigNumber) => gas.add(50000);
...
/**
 * Optional parameters of a transaction that borrows USDH.
 *
 * @public
 */
export interface BorrowingOperationOptionalParams {
   ...
   *
   * @remarks
   * Transactions that borrow USDH must pay a variable borrowing fee, which is added to the Trove's
...
export class PopulatedEthersRedemption
  ...
  implements
    PopulatedRedemption<
      ...
      EthersTransactionReceipt
    > {
  /** {@inheritDoc @liquity/lib-base#PopulatedRedemption.attemptedUSDHAmount} */
  readonly attemptedUSDHAmount: Decimal;

  /** {@inheritDoc @liquity/lib-base#PopulatedRedemption.redeemableUSDHAmount} */
    ...
    super(
      ...
      ({ logs }) =>
        troveManager
          ...
          .map(({ args: { _ETHSent, _ETHFee, _actualUSDHAmount, _attemptedUSDHAmount } }) => ({
            attemptedUSDHAmount: decimalify(_attemptedUSDHAmount),
            actualUSDHAmount: decimalify(_actualUSDHAmount),
            collateralTaken: decimalify(_ETHSent),
            fee: decimalify(_ETHFee)
          }))[0]
    );

    this.attemptedUSDHAmount = attemptedUSDHAmount;
    this.redeemableUSDHAmount = redeemableUSDHAmount;
    this.isTruncated = redeemableUSDHAmount.lt(attemptedUSDHAmount);
...
export class PopulatableEthersLiquity
    ...
    return new PopulatedEthersLiquityTransaction(
      ...
      ({ logs }) => {
        ...
        const [fee] = borrowerOperations
          .extractEvents(logs, "USDHBorrowingFeePaid")
          .map(({ args: { _USDHFee } }) => decimalify(_USDHFee));
  ...
  private async _wrapTroveClosure(
    ...
    const { activePool, usdhToken } = _getContracts(this._readable.connection);
    ...
    return new PopulatedEthersLiquityTransaction(
      ...
      ({ logs, from: userAddress }) => {
        ...
        return {
          params: repayUSDH.nonZero ? { withdrawCollateral, repayUSDH } : { withdrawCollateral }
        };
      }
    );
  }
    ...
    return new PopulatedEthersLiquityTransaction(
      ...
      ({ logs }) => {
        ...
        const [totals] = troveManager
          ...
          .map(
            ({
              args: { _USDHGasCompensation, _collGasCompensation, _liquidatedColl, _liquidatedDebt }
            }) => ({
              collateralGasCompensation: decimalify(_collGasCompensation),
              usdhGasCompensation: decimalify(_USDHGasCompensation),
  ...
  private _extractStabilityPoolGainsWithdrawalDetails(
    ...
    const [newUSDHDeposit] = stabilityPool
      .extractEvents(logs, "UserDepositChanged")
      .map(({ args: { _newDeposit } }) => decimalify(_newDeposit));

    const [[collateralGain, usdhLoss]] = stabilityPool
      .extractEvents(logs, "ETHGainWithdrawn")
      .map(({ args: { _ETH, _USDHLoss } }) => [decimalify(_ETH), decimalify(_USDHLoss)]);

    const [halalReward] = stabilityPool
      .extractEvents(logs, "HALALPaidToDepositor")
      .map(({ args: { _HALAL } }) => decimalify(_HALAL));
  ...
  private async _wrapStabilityDepositWithdrawal(
    ...
    const { stabilityPool, usdhToken } = _getContracts(this._readable.connection);
    ...
    return new PopulatedEthersLiquityTransaction(
      ...
      ({ logs, from: userAddress }) => {
        ...
        return {
          ...
          change: { withdrawUSDH, withdrawAllUSDH: gainsWithdrawalDetails.newUSDHDeposit.isZero }
        };
      }
    );
  }
  ...
  > {
    ...
    return [
      decimalify(truncatedUSDHamount),
      firstRedemptionHint,
      partialRedemptionUpperHint,
      partialRedemptionLowerHint,
      partialRedemptionHintNICR
  ...
  async openTrove(
    ...
    const { depositCollateral, borrowUSDH } = normalizedParams;
    ...
    const txParams = (borrowUSDH: Decimal): Parameters<typeof borrowerOperations.openTrove> => [
    ...
    if (overrides?.gasLimit === undefined) {
      ...
      const { borrowUSDH: borrowUSDHSimulatingDecay } = Trove.recreate(
        decayedTrove,
        currentBorrowingRate
      );

      if (decayedTrove.debt.lt(USDH_MINIMUM_DEBT)) {
        throw new Error(
          `Trove's debt might fall below ${USDH_MINIMUM_DEBT} ` +
            `within ${borrowingFeeDecayToleranceMinutes} minutes`
        );
      }

      const [gasNow, gasLater] = await Promise.all([
        borrowerOperations.estimateGas.openTrove(...txParams(borrowUSDH)),
        borrowerOperations.estimateGas.openTrove(...txParams(borrowUSDHSimulatingDecay))
      ]);
    ...
    return this._wrapTroveChangeWithFees(
      normalizedParams,
      await borrowerOperations.populateTransaction.openTrove(...txParams(borrowUSDH)),
      gasHeadroom
    );
  }
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.borrowUSDH} */
  borrowUSDH(
    amount: Decimalish,
    maxBorrowingRate?: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<TroveAdjustmentDetails>> {
    return this.adjustTrove({ borrowUSDH: amount }, maxBorrowingRate, overrides);
  }

  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.repayUSDH} */
  repayUSDH(
    amount: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<TroveAdjustmentDetails>> {
    return this.adjustTrove({ repayUSDH: amount }, undefined, overrides);
  }
  ...
  async adjustTrove(
    ...
    const { depositCollateral, withdrawCollateral, borrowUSDH, repayUSDH } = normalizedParams;
    ...
    const txParams = (borrowUSDH?: Decimal): Parameters<typeof borrowerOperations.adjustTrove> => [
      maxBorrowingRate.hex,
      (withdrawCollateral ?? Decimal.ZERO).hex,
      (borrowUSDH ?? repayUSDH ?? Decimal.ZERO).hex,
      !!borrowUSDH,
      ...hints,
      { value: depositCollateral?.hex, ...overrides }
    ];
    ...
    if (overrides?.gasLimit === undefined) {
      ...
      const { borrowUSDH: borrowUSDHSimulatingDecay } = trove.adjustTo(
        decayedTrove,
        currentBorrowingRate
      );
      ...
      const [gasNow, gasLater] = await Promise.all([
        borrowerOperations.estimateGas.adjustTrove(...txParams(borrowUSDH)),
        borrowUSDH &&
          borrowerOperations.estimateGas.adjustTrove(...txParams(borrowUSDHSimulatingDecay))
      ]);
    ...
    return this._wrapTroveChangeWithFees(
      normalizedParams,
      await borrowerOperations.populateTransaction.adjustTrove(...txParams(borrowUSDH)),
      gasHeadroom
    );
  }
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.depositUSDHInStabilityPool} */
  async depositUSDHInStabilityPool(
    amount: Decimalish,
    frontendTag?: string,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<StabilityDepositChangeDetails>> {
    const { stabilityPool } = _getContracts(this._readable.connection);
    const depositUSDH = Decimal.from(amount);
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.withdrawUSDHFromStabilityPool} */
  async withdrawUSDHFromStabilityPool(
    amount: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<StabilityDepositChangeDetails>> {
    const { stabilityPool } = _getContracts(this._readable.connection);
  ...
  async transferCollateralGainToTrove(
    ...
    return this._wrapCollateralGainTransfer(
      await stabilityPool.estimateAndPopulate.withdrawETHGainToTrove(
        { ...overrides },
        compose(addGasForPotentialListTraversal, addGasForHALALIssuance),
        ...(await this._findHints(finalTrove, address))
      )
    );
  }

  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.sendUSDH} */
  async sendUSDH(
    toAddress: string,
    amount: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    const { usdhToken } = _getContracts(this._readable.connection);

    return this._wrapSimpleTransaction(
      await usdhToken.estimateAndPopulate.transfer(
        { ...overrides },
        id,
        toAddress,
        Decimal.from(amount).hex
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.sendHALAL} */
  async sendHALAL(
    toAddress: string,
    amount: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    const { halalToken } = _getContracts(this._readable.connection);

    return this._wrapSimpleTransaction(
      await halalToken.estimateAndPopulate.transfer(
        { ...overrides },
        id,
        toAddress,
        Decimal.from(amount).hex
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.redeemUSDH} */
  async redeemUSDH(
    amount: Decimalish,
    maxRedemptionRate?: Decimalish,
    overrides?: EthersTransactionOverrides
    ...
    const { troveManager } = _getContracts(this._readable.connection);
    const attemptedUSDHAmount = Decimal.from(amount);
    ...
    const [
      ...
      this._readable.getTotal(),
      this._findRedemptionHints(attemptedUSDHAmount)
    ]);

    if (truncatedAmount.isZero) {
      throw new Error(
        `redeemUSDH: amount too low to redeem (try at least ${USDH_MINIMUM_NET_DEBT})`
      );
    }
    ...
    const populateRedemption = async (
      attemptedUSDHAmount: Decimal,
      maxRedemptionRate?: Decimalish,
      truncatedAmount: Decimal = attemptedUSDHAmount,
      ...
      return new PopulatedEthersRedemption(
        ...
        truncatedAmount.lt(attemptedUSDHAmount)
          ? newMaxRedemptionRate =>
              populateRedemption(
                truncatedAmount.add(USDH_MINIMUM_NET_DEBT),
    ...
    return populateRedemption(attemptedUSDHAmount, maxRedemptionRate, truncatedAmount, partialHints);
  }

  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.stakeHALAL} */
  async stakeHALAL(
    amount: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    const { halalStaking } = _getContracts(this._readable.connection);

    return this._wrapSimpleTransaction(
      await halalStaking.estimateAndPopulate.stake({ ...overrides }, id, Decimal.from(amount).hex)
    );
  }

  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.unstakeHALAL} */
  async unstakeHALAL(
    amount: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    ...
    return this._wrapSimpleTransaction(
      await halalStaking.estimateAndPopulate.unstake({ ...overrides }, id, Decimal.from(amount).hex)
    );
  }
  ...
  withdrawGainsFromStaking(
    ...
    return this.unstakeHALAL(Decimal.ZERO, overrides);
  }
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.withdrawHALALRewardFromLiquidityMining} */
  async withdrawHALALRewardFromLiquidityMining(
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    const { unipool } = _getContracts(this._readable.connection);
