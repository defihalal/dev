...
describe("EthersLiquity", () => {
  ...
  describe("findHintForCollateralRatio", () => {
    it("should pick the closest approx hint", async () => {
      ...
      const params = Trove.recreate(new Trove(Decimal.from(1), USDH_MINIMUM_DEBT));
  ...
  describe("Trove", () => {
    ...
    it("should fail to create an undercollateralized Trove", async () => {
      const price = await liquity.getPrice();
      const undercollateralized = new Trove(USDH_MINIMUM_DEBT.div(price), USDH_MINIMUM_DEBT);
    ...
    it("should fail to create a Trove with too little debt", async () => {
      const withTooLittleDebt = new Trove(Decimal.from(50), USDH_MINIMUM_DEBT.sub(1));

      await expect(liquity.openTrove(Trove.recreate(withTooLittleDebt))).to.eventually.be.rejected;
    });

    const withSomeBorrowing = { depositCollateral: 50, borrowUSDH: USDH_MINIMUM_NET_DEBT.add(100) };
    ...
    it("should create a Trove with some borrowing", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(withSomeBorrowing.borrowUSDH)}`);
    });
    ...
    const repaySomeDebt = { repayUSDH: 10 };

    it("should repay some debt", async () => {
      const { newTrove, fee } = await liquity.repayUSDH(repaySomeDebt.repayUSDH);
      expect(newTrove).to.deep.equal(Trove.create(withSomeBorrowing).adjust(repaySomeDebt));
      expect(`${fee}`).to.equal("0");
    });

    const borrowSomeMore = { borrowUSDH: 20 };

    it("should borrow some more", async () => {
      const { newTrove, fee } = await liquity.borrowUSDH(borrowSomeMore.borrowUSDH);
      expect(newTrove).to.deep.equal(
        Trove.create(withSomeBorrowing).adjust(repaySomeDebt).adjust(borrowSomeMore)
      );
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowSomeMore.borrowUSDH)}`);
    });
    ...
    const repayAndWithdraw = { repayUSDH: 60, withdrawCollateral: 0.5 };
    ...
    const borrowAndDeposit = { borrowUSDH: 60, depositCollateral: 0.5 };
    ...
    it("should borrow more and deposit some collateral at the same time", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowAndDeposit.borrowUSDH)}`);
    ...
    it("should close the Trove with some USDH from another user", async () => {
      const price = await liquity.getPrice();
      const initialTrove = await liquity.getTrove();
      const usdhBalance = await liquity.getHALALBalance();
      const usdhShortage = initialTrove.netDebt.sub(usdhBalance);

      let funderTrove = Trove.create({ depositCollateral: 1, borrowUSDH: usdhShortage });
      funderTrove = funderTrove.setDebt(Decimal.max(funderTrove.debt, USDH_MINIMUM_DEBT));
      ...
      await funderLiquity.sendUSDH(await user.getAddress(), usdhShortage);

      const { params } = await liquity.closeTrove();

      expect(params).to.deep.equal({
        withdrawCollateral: initialTrove.collateral,
        repayUSDH: initialTrove.netDebt
      });
  ...
  describe("SendableEthersLiquity", () => {
    it("should parse failed transactions without throwing", async () => {
      ...
      const tx = await liquity.send.openTrove(
        { depositCollateral: 0.01, borrowUSDH: 0.01 },
        undefined,
        { gasLimit: 1e6 }
      );
  ...
  describe("Frontend", () => {
    ...
    it("other user's deposit should be tagged with the frontend's address", async () => {
      ...
      await otherLiquity.openTrove({ depositCollateral: 20, borrowUSDH: USDH_MINIMUM_DEBT });

      await otherLiquity.depositUSDHInStabilityPool(USDH_MINIMUM_DEBT);
  ...
  describe("StabilityPool", () => {
    before(async () => {
      ...
      await funder.sendTransaction({
        to: otherUsers[0].getAddress(),
        value: USDH_MINIMUM_DEBT.div(170).hex
      });
    });

    const initialTroveOfDepositor = Trove.create({
      // depositCollateral: USDH_MINIMUM_DEBT.div(100),
      depositCollateral: USDH_MINIMUM_DEBT.div(1),
      borrowUSDH: USDH_MINIMUM_NET_DEBT
    });
    ...
    it("should make a small stability deposit", async () => {
      ...
      const details = await liquity.depositUSDHInStabilityPool(smallStabilityDeposit);

      expect(details).to.deep.equal({
        usdhLoss: Decimal.from(0),
        newUSDHDeposit: smallStabilityDeposit,
        collateralGain: Decimal.from(0),
        halalReward: Decimal.from(0),

        change: {
          depositUSDH: smallStabilityDeposit
        }
      });
    });

    const troveWithVeryLowICR = Trove.create({
      // depositCollateral: USDH_MINIMUM_DEBT.div(180),
      depositCollateral: USDH_MINIMUM_DEBT.div(1),
      borrowUSDH: USDH_MINIMUM_NET_DEBT
    });
    ...
    it("should liquidate other user's Trove", async () => {
      ...
      expect(details).to.deep.equal({
        ...
        usdhGasCompensation: USDH_LIQUIDATION_RESERVE,
    ...
    it("should transfer the gains to the Trove", async () => {
      ...
      expect(details).to.deep.equal({
        usdhLoss: smallStabilityDeposit,
        newUSDHDeposit: Decimal.ZERO,
        halalReward: Decimal.ZERO,
    ...
    describe("when people overstay", () => {
      before(async () => {
        ...
        // Use this account to print USDH
        await liquity.openTrove({ depositCollateral: 50, borrowUSDH: 5000 });

        // otherLiquities[0-2] will be independent stability depositors
        await liquity.sendUSDH(await otherUsers[0].getAddress(), 3000);
        await liquity.sendUSDH(await otherUsers[1].getAddress(), 1000);
        await liquity.sendUSDH(await otherUsers[2].getAddress(), 1000);

        // otherLiquities[3-4] will be Trove owners whose Troves get liquidated
        await otherLiquities[3].openTrove({ depositCollateral: 21, borrowUSDH: 2900 });
        await otherLiquities[4].openTrove({ depositCollateral: 21, borrowUSDH: 2900 });

        await otherLiquities[0].depositUSDHInStabilityPool(3000);
        await otherLiquities[1].depositUSDHInStabilityPool(1000);
        ...
        await otherLiquities[2].depositUSDHInStabilityPool(1000);
        ...
        expect(`${await liquity.getUSDHInStabilityPool()}`).to.equal("0");
      });
      ...
      it("should still be able to withdraw remaining deposit", async () => {
        for (const l of [otherLiquities[0], otherLiquities[1], otherLiquities[2]]) {
          ...
          await l.withdrawUSDHFromStabilityPool(stabilityDeposit.currentUSDH);
        }
      });
    });
  });
  ...
  describe("Redemption", () => {
    const troveCreations = [
      { depositCollateral: 99, borrowUSDH: 4600 },
      { depositCollateral: 20, borrowUSDH: 2000 }, // net debt: 2010
      { depositCollateral: 20, borrowUSDH: 2100 }, // net debt: 2110.5
      { depositCollateral: 20, borrowUSDH: 2200 } //  net debt: 2211
    ];
    ...
    it("should fail to redeem during the bootstrap phase", async () => {
      ...
      await expect(liquity.redeemUSDH(4326.5)).to.eventually.be.rejected;
    });

    const someUSDH = Decimal.from(4326.5);

    it("should redeem some USDH after the bootstrap phase", async () => {
      ...
      const expectedDetails = {
        attemptedUSDHAmount: someUSDH,
        actualUSDHAmount: someUSDH,
        collateralTaken: someUSDH.div(200),
        fee: new Fees(0, 0.99, 2, new Date(), new Date(), false)
          .redemptionRate(someUSDH.div(total.debt))
          .mul(someUSDH.div(200))
      };

      const { rawReceipt, details } = await waitForSuccess(liquity.send.redeemUSDH(someUSDH));
      ...
      expect(`${await liquity.getUSDHBalance()}`).to.equal("273.5");
    ...
    it("borrowing rate should be maxed out now", async () => {
      const borrowUSDH = Decimal.from(10);

      const { fee, newTrove } = await liquity.borrowUSDH(borrowUSDH);
      expect(`${fee}`).to.equal(`${borrowUSDH.mul(MAXIMUM_BORROWING_RATE)}`);

      expect(newTrove).to.deep.equal(
        Trove.create(troveCreations[0]).adjust({ borrowUSDH }, MAXIMUM_BORROWING_RATE)
      );
    });
  });

  describe("Redemption (truncation)", () => {
    const troveCreationParams = { depositCollateral: 20, borrowUSDH: 2000 };
    const netDebtPerTrove = Trove.create(troveCreationParams).netDebt;
    const amountToAttempt = Decimal.from(3000);
    const expectedRedeemable = netDebtPerTrove.mul(2).sub(USDH_MINIMUM_NET_DEBT);
    ...
    beforeEach(async () => {
      ...
      await liquity.openTrove({ depositCollateral: 99, borrowUSDH: 5000 });
    ...
    it("should truncate the amount if it would put the last Trove below the min debt", async () => {
      const redemption = await liquity.populate.redeemUSDH(amountToAttempt);
      expect(`${redemption.attemptedUSDHAmount}`).to.equal(`${amountToAttempt}`);
      expect(`${redemption.redeemableUSDHAmount}`).to.equal(`${expectedRedeemable}`);
      expect(redemption.isTruncated).to.be.true;

      const { details } = await waitForSuccess(redemption.send());
      expect(`${details.attemptedUSDHAmount}`).to.equal(`${expectedRedeemable}`);
      expect(`${details.actualUSDHAmount}`).to.equal(`${expectedRedeemable}`);
    });

    it("should increase the amount to the next lowest redeemable value", async () => {
      const increasedRedeemable = expectedRedeemable.add(USDH_MINIMUM_NET_DEBT);

      const initialRedemption = await liquity.populate.redeemUSDH(amountToAttempt);
      const increasedRedemption = await initialRedemption.increaseAmountByMinimumNetDebt();
      expect(`${increasedRedemption.attemptedUSDHAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${increasedRedemption.redeemableUSDHAmount}`).to.equal(`${increasedRedeemable}`);
      expect(increasedRedemption.isTruncated).to.be.false;

      const { details } = await waitForSuccess(increasedRedemption.send());
      expect(`${details.attemptedUSDHAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${details.actualUSDHAmount}`).to.equal(`${increasedRedeemable}`);
    });

    it("should fail to increase the amount if it's not truncated", async () => {
      const redemption = await liquity.populate.redeemUSDH(netDebtPerTrove);
  ...
  describe("Redemption (gas checks)", function () {
    ...
    before(async function () {
      ...
      for (const otherLiquity of otherLiquities) {
        await otherLiquity.openTrove({
          ...
          borrowUSDH: amountToBorrowPerTrove
        });
      }
    ...
    it("should redeem using the maximum iterations and almost all gas", async () => {
      await liquity.openTrove({
        ...
        borrowUSDH: amountToRedeem
      });

      const { rawReceipt } = await waitForSuccess(liquity.send.redeemUSDH(amountToRedeem));
  ...
  describe("Liquidity mining", () => {
    ...
    it("should have an HALAL reward after some time has passed", async function () {
      this.timeout("20s");
      ...
      const halalReward = Number(await liquity.getLiquidityMiningHALALReward());
      expect(halalReward).to.be.at.least(1); // ~0.2572 per second [(4e6/3) / (60*24*60*60)]

      await liquity.withdrawHALALRewardFromLiquidityMining();
      const halalBalance = Number(await liquity.getHALALBalance());
      expect(halalBalance).to.be.at.least(halalReward); // may have increased since checking
    });
    ...
    it("should unstake remaining tokens and withdraw remaining HALAL reward", async () => {
      ...
      const halalReward = await liquity.getLiquidityMiningHALALReward();
      expect(`${halalReward}`).to.equal("0");
    ...
    it("should have no more rewards after the mining period is over", async function () {
      ...
      const remainingHALALReward = await liquity.getRemainingLiquidityMiningHALALReward();
      expect(`${remainingHALALReward}`).to.equal("0");

      const halalBalance = Number(await liquity.getHALALBalance());
      expect(halalBalance).to.be.within(1333333, 1333334);
    });
  });
  ...
  describe("Hints (adjustTrove)", () => {
    ...
    before(async () => {
      ...
      await openTroves(eightOtherUsers, [
        { depositCollateral: 30, borrowUSDH: 2000 }, // 0
        { depositCollateral: 30, borrowUSDH: 2100 }, // 1
        { depositCollateral: 30, borrowUSDH: 2200 }, // 2
        { depositCollateral: 30, borrowUSDH: 2300 }, // 3
        // Test 1:           30,             2400
        { depositCollateral: 30, borrowUSDH: 2500 }, // 4
        { depositCollateral: 30, borrowUSDH: 2600 }, // 5
        { depositCollateral: 30, borrowUSDH: 2700 }, // 6
        { depositCollateral: 30, borrowUSDH: 2800 } //  7
        // Test 2:           30,             2900
    ...
    it("should not traverse the whole list when bottom Trove moves", async () => {
      ...
      const targetTrove = Trove.create({ depositCollateral: 30, borrowUSDH: 2900 });
      const interferingTrove = Trove.create({ depositCollateral: 30, borrowUSDH: 3000 });
    ...
    it("should not traverse the whole list when lowering ICR of bottom Trove", async () => {
      ...
      const targetTrove = [
        Trove.create({ depositCollateral: 30, borrowUSDH: 3100 }),
        Trove.create({ depositCollateral: 30, borrowUSDH: 3200 })
      ];
  ...
  describe("Gas estimation", () => {
    ...
    before(async function () {
      ...
      await openTroves(fiveOtherUsers, [
        { depositCollateral: 20, borrowUSDH: 2040 },
        { depositCollateral: 20, borrowUSDH: 2050 },
        { depositCollateral: 20, borrowUSDH: 2060 },
        { depositCollateral: 20, borrowUSDH: 2070 },
        { depositCollateral: 20, borrowUSDH: 2080 }
      ]);
    ...
    it("should include enough gas for updating lastFeeOperationTime", async () => {
      await liquity.openTrove({ depositCollateral: 20, borrowUSDH: 2090 });

      // We just updated lastFeeOperationTime, so this won't anticipate having to update that
      // during estimateGas
      const tx = await liquity.populate.redeemUSDH(1);
    ...
    it("should include enough gas for one extra traversal", async () => {
      ...
      expect(adjustment.borrowUSDH).to.be.undefined;
      ...
      assertDefined(rudeCreation.borrowUSDH);
      const usdhShortage = rudeTrove.debt.sub(rudeCreation.borrowUSDH);

      await liquity.sendUSDH(await rudeUser.getAddress(), usdhShortage);
    ...
    it("should include enough gas for both when borrowing", async () => {
      ...
      const adjustment = trove.adjustTo(newTrove);
      expect(adjustment.borrowUSDH).to.not.be.undefined;
  ...
  describe("Gas estimation (HALAL issuance)", () => {
    ...
    it("should include enough gas for issuing HALAL", async function () {
      this.timeout("1m");

      await liquity.openTrove({ depositCollateral: 40, borrowUSDH: 4000 });
      await liquity.depositUSDHInStabilityPool(19);

      await increaseTime(60);

      // This will issue HALAL for the first time ever. That uses a whole lotta gas, and we don't
      ...
      const deposit = await liquity.populate.depositUSDHInStabilityPool(1);
      const withdraw = await liquity.populate.withdrawUSDHFromStabilityPool(1);
      ...
      await deployerLiquity.depositUSDHInStabilityPool(creation.borrowUSDH);
    ...
    before(async function () {
      ...
      await openTroves(
        ...
        someMoreUsers.map((_, i) => ({
          ...
          borrowUSDH: USDH_MINIMUM_NET_DEBT.add(i / 10)
        }))
      );
      ...
      await Promise.all(
        otherLiquities.map(async otherLiquity =>
          otherLiquity.sendUSDH(await user.getAddress(), await otherLiquity.getUSDHBalance())
        )
      );
      ...
      const redeemedTroveDebt = await liquity
        .getUSDHBalance()
        .then(x => x.div(10).add(USDH_LIQUIDATION_RESERVE));
      ...
      const { actualUSDHAmount } = await liquity.redeemUSDH(redeemedTrove.netDebt);

      expect(`${actualUSDHAmount}`).to.equal(`${redeemedTrove.netDebt}`);
