...
export function getCurrentSystemState(): SystemState {
  ...
  if (currentSystemStateOrNull == null) {
    ...
    newSystemState.totalHALALTokensStaked = DECIMAL_ZERO;
    newSystemState.save();
...
export function updateSystemStateByHalalStakeChange(stakeChange: HalalStakeChange): void {
  let systemState = getCurrentSystemState();

  systemState.totalHALALTokensStaked = systemState.totalHALALTokensStaked.plus(
