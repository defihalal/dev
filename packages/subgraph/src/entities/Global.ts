...
export function getGlobal(): Global {
  ...
  if (globalOrNull != null) {
    ...
    newGlobal.totalNumberOfHALALStakes = 0;
    newGlobal.numberOfActiveHALALStakes = 0;
...
export function updateTotalRedistributed(L_ETH: BigInt, L_USDHDebt: BigInt): void {
  let global = getGlobal();

  global.rawTotalRedistributedCollateral = L_ETH;
  global.rawTotalRedistributedDebt = L_USDHDebt;
  global.save();
}
...
export function increaseTotalNumberOfHALALStakes(): void {
  let global = getGlobal();

  global.totalNumberOfHALALStakes++;
  global.numberOfActiveHALALStakes++;
  global.save();
}

export function increaseNumberOfActiveHALALStakes(): void {
  let global = getGlobal();
...
export function decreaseNumberOfActiveHALALStakes(): void {
  let global = getGlobal();
...
export function increaseTotalBorrowingFeesPaid(_USDHFee: BigInt): void {
  let global = getGlobal();
  global.totalBorrowingFeesPaid = global.totalBorrowingFeesPaid.plus(decimalize(_USDHFee));
  global.save();
}
