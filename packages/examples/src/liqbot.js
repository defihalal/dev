...
const { Decimal, UserTrove, USDH_LIQUIDATION_RESERVE } = require("@liquity/lib-base");
...
async function tryToLiquidate(liquity) {
  ...
  try {
    ...
    const expectedCompensation = total.collateral
      ...
      .add(USDH_LIQUIDATION_RESERVE.mul(troves.length));
    ...
    const { collateralGasCompensation, usdhGasCompensation, liquidatedAddresses } = receipt.details;
    ...
    const totalCompensation = collateralGasCompensation
      .mul(store.state.price)
      .add(usdhGasCompensation);

    success(
      `Received ${bold(`${collateralGasCompensation.toString(4)} ETH`)} + ` +
        `${bold(`${usdhGasCompensation.toString(2)} USDH`)} compensation (` +
