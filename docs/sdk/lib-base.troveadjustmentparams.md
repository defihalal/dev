...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowUSDH` and `repayUSDH` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowUSDH? </td> <td> T </td> <td> <i>(Optional)</i> The amount of USDH that's borrowed. </td> </tr>

<tr> <td> repayUSDH? </td> <td> T </td> <td> <i>(Optional)</i> The amount of USDH that's repaid. </td> </tr>
