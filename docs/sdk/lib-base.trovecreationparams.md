...
```typescript
export declare type TroveCreationParams<T = unknown> = _CollateralDeposit<T> & _NoCollateralWithdrawal & _USDHBorrowing<T> & _NoUSDHRepayment;
```
...
<tr> <td> borrowUSDH </td> <td> T </td> <td> The amount of USDH that's borrowed. </td> </tr>
